use crate::layers::{EphemeralStore, ShortTermStore, MediumTermStore, LongTermStore};
...
pub struct MemoryCoordinator {
    ...
    medium_term: Arc<MediumTermStore>,
...
impl MemoryCoordinator {
    ...
    pub async fn new(config: MemoryConfig) -> Result<Self> {
        ...
        let short_term = Arc::new(
            ShortTermStore::new(&config.sqlite_path).await
                .context("Failed to initialize short-term store")?
        );
        
        let medium_term = Arc::new(
            MediumTermStore::new(&config.sqlite_path).await
                .context("Failed to initialize medium-term store")?
        );
