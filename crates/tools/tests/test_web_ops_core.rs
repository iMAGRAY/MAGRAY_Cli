...
async fn web_fetch_file_scheme_reads_content() -> Result<()> {
    ...
    let input = ToolInput { command: "web_fetch".into(), args: HashMap::from([("url".into(), url)]), context: None, dry_run: false, timeout_ms: Some(5_000) };
    let out = fetch.execute(input).await?;
    assert!(out.success);
    assert!(out.formatted_output.as_ref().unwrap().contains("hello world"));
...
async fn web_search_dry_run_reports_provider() -> Result<()> {
    ...
    let input = ToolInput { command: "web_search".into(), args: HashMap::from([("query".into(), "rust".into())]), context: None, dry_run: true, timeout_ms: None };
    let out = search.execute(input).await?;
    assert!(out.success);
    assert_eq!(out.metadata.get("dry_run").map(|s| s.as_str()), Some("true"));
    ...
    let input2 = ToolInput { command: "web_search".into(), args: HashMap::from([("query".into(), "rust".into())]), context: None, dry_run: true, timeout_ms: None };
    let out2 = search.execute(input2).await?;
    assert!(out2.metadata.get("provider").unwrap().contains("DuckDuckGo"));
    Ok(())
}