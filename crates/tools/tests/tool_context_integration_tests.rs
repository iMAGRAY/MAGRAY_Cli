...
use std::sync::Arc;
use tools::context::{
    ToolContextBuilder, ToolSelectionRequest, ContextBuildingConfig
};
...
async fn test_tool_context_builder_creation() {
    ...
    assert!(builder_result.is_ok(), "ToolContextBuilder should be created successfully");
    
    let builder = builder_result.unwrap();
    assert!(builder.has_intelligent_tool_selection(), "Should use semantic reranking by default");
}
...
async fn test_basic_tool_selection() {
    ...
    assert!(!response.tools.is_empty() || response.tools.is_empty(), "Should handle empty registry gracefully");
    assert!(response.selection_metrics.total_time.as_millis() < 5000, "Selection should complete within 5 seconds");
}
...
async fn test_custom_configuration() {
    ...
    let builder = ToolContextBuilder::with_config(registry, config).expect("Failed to create ToolContextBuilder with config");
...
async fn test_fallback_behavior() {
    ...
    assert!(response.tools.is_empty(), "Should return empty results for non-existent categories");
}
...
async fn test_performance_requirements() {
    ...
    assert!(elapsed.as_millis() < 100, "Tool selection should complete in under 100ms for empty registry");
    
    let response = response.unwrap();
    assert!(response.selection_metrics.total_time.as_millis() < 100, "Reported metrics should be under 100ms");
}
...
async fn test_orchestrator_integration_pattern() {
    ...
    for (query, categories) in intent_patterns {
        ...
        let response = builder.build_context(request).await;
        assert!(response.is_ok(), "Orchestrator pattern should work for query: {}", query);
        
        let response = response.unwrap();
        // Verify response structure
        assert!(response.context.query == query, "Query should be preserved in context");
        assert!(response.selection_metrics.total_time.as_millis() < 1000, "Should complete quickly");
    }
}
...
async fn test_builder_methods() {
    ...
    let builder = ToolContextBuilder::new(Arc::clone(&registry)).expect("Failed to create ToolContextBuilder");
    
    // Test has_intelligent_tool_selection
    let has_intelligent = builder.has_intelligent_tool_selection();
    assert!(has_intelligent, "Should report semantic reranking by default");
    ...
    assert!(custom_builder.is_ok(), "Custom config constructor should work");
}