...
async fn shell_exec_dry_run_preview() -> Result<()> {
    ...
    let input = ToolInput { command: "shell_exec".into(), args: HashMap::from([("command".into(), "echo hello".into())]), context: None, dry_run: true, timeout_ms: None };
...
async fn shell_exec_timeout_triggers() -> Result<()> {
    let tool = ShellExec::new();
    let input = ToolInput { command: "shell_exec".into(), args: HashMap::from([("command".into(), "sleep 2".into())]), context: None, dry_run: false, timeout_ms: Some(200) };
    let out = tool.execute(input).await?;
    assert!(!out.success);
    assert!(out.result.contains("таймаут"));
    assert_eq!(out.metadata.get("timeout_ms").map(|s| s.as_str()), Some("200"));
    Ok(())
}
...
async fn shell_exec_truncates_stdout_when_exceeds_limit() -> Result<()> {
    ...
    let out = tool.execute(ToolInput { command: "shell_exec".into(), args, context: None, dry_run: false, timeout_ms: Some(5000) }).await?;
    // success true (exit code 0), but truncated
    assert!(out.success);
    assert_eq!(out.metadata.get("stdout_truncated").map(|s| s.as_str()), Some("true"));
    Ok(())
}
...
async fn shell_exec_respects_cwd() -> Result<()> {
    ...
    assert_eq!(out.metadata.get("cwd").map(|s| s.as_str()), Some(cwd.as_str()));
    Ok(())
}