...
use crate::intelligent_selector::{
    IntelligentToolSelector, ToolSelectionContext, TaskComplexity, UrgencyLevel, UserExpertise
};
use crate::execution_pipeline::{
    ToolExecutionPipeline, ExecutionStrategy, ExecutionResult
};
use crate::performance_monitor::{
    ToolPerformanceMonitor, MonitorConfig
};
...
impl EnhancedToolSystem {
    pub async fn new(config: EnhancedToolSystemConfig) -> Result<Self> {
        ...
        let execution_pipeline = Arc::new(ToolExecutionPipeline::new(Arc::clone(&intelligent_selector)));
        let performance_monitor = Arc::new(ToolPerformanceMonitor::new(config.monitor_config.clone()));
        ...
        info!("✅ Enhanced Tool System initialized with {} features", 
    ...
    pub async fn register_tool(&self, tool: Arc<dyn Tool>) -> Result<()> {
        ...
        if self.config.enable_execution_pipeline {
            self.execution_pipeline.register_tool(Arc::clone(&tool)).await;
        }
    ...
    pub async fn execute_request(
        ...
        let context = self.create_selection_context(user_query, session_context).await;
        ...
        let execution_tracker = if self.config.enable_performance_monitoring {
            Some(self.performance_monitor.execution_started("enhanced_system").await)
        } else {
            None
        };
        ...
        let execution_result = if self.config.enable_execution_pipeline {
            self.execution_pipeline.execute_tool(&context, execution_strategy).await?
        } else {
        ...
        if let Some(tracker) = execution_tracker {
            if execution_result.output.success {
                tracker.success(Some("enhanced_system_execution".to_string())).await;
            } else {
                tracker.failure(
        ...
        let performance_snapshot = if self.config.enable_performance_monitoring {
            self.get_performance_snapshot(&execution_result.tool_name).await
        } else {
            None
        };
        ...
        let optimization_suggestions = if self.config.enable_auto_optimization {
            self.generate_optimization_suggestions(&execution_result, &context).await
        } else {
            Vec::new()
        };
        ...
        // Update system stats
        {
            ...
            let total_time = stats.average_response_time * (stats.total_requests - 1) as u32 + total_execution_time;
        ...
        info!("✅ Enhanced tool request completed in {:?}", total_execution_time);
        Ok(result)
    }
    ...
    fn analyze_task_complexity(&self, query: &str) -> TaskComplexity {
        ...
        let complex_indicators = [
            "найди и", "создай и", "проанализируй", "сравни", "интегрируй",
            "оптимизируй", "рефактори", "настрой", "развернут", "миграц"
        ];
        
        let medium_indicators = [
            "создай", "обнови", "измени", "удали", "скопируй", "перемести"
        ];
        ...
        let medium_count = medium_indicators.iter()
            .filter(|&indicator| query_lower.contains(indicator))
            .count();
    ...
    fn analyze_urgency_level(&self, query: &str) -> UrgencyLevel {
        ...
        if query_lower.contains("срочно") || query_lower.contains("быстро") || 
           query_lower.contains("критично") || query_lower.contains("немедленно") {
    ...
    async fn determine_optimal_strategy(&self, context: &ToolSelectionContext) -> ExecutionStrategy {
    ...
    async fn execute_direct(&self, _context: &ToolSelectionContext) -> Result<ExecutionResult> {
        ...
        Err(anyhow::anyhow!("Direct execution not implemented - requires execution pipeline"))
    }
    ...
    async fn generate_optimization_suggestions(
        ...
        if execution_result.attempt_count > 1 {
            suggestions.push("Tool reliability could be improved with circuit breaker protection".to_string());
        }
    ...
    fn count_enabled_features(&self) -> usize {
        ...
        if self.config.enable_intelligent_selection { count += 1; }
        if self.config.enable_performance_monitoring { count += 1; }
        if self.config.enable_execution_pipeline { count += 1; }
        if self.config.enable_auto_optimization { count += 1; }
    ...
    pub async fn health_check(&self) -> Result<String> {
        ...
        let health_status = if stats.total_requests > 0 {
            let success_rate = (stats.successful_requests as f32 / stats.total_requests as f32) * 100.0;
            match success_rate {
...
impl Default for EnhancedToolSystem {
    fn default() -> Self {
        ...
        rt.block_on(async {
            Self::new(EnhancedToolSystemConfig::default()).await.unwrap()
        })
    }
}