...
impl ToolExecutionPipeline {
    ...
    pub async fn execute_tool(
        ...
        debug!(
            "🚀 Executing tool pipeline for query: '{}'",
            context.user_query
        );
        ...
        let result = match strategy {
            ExecutionStrategy::Direct => {
                self.execute_direct(&tool_candidates[0].tool_name, context)
                    .await
            }
            ExecutionStrategy::RetryWithBackoff => {
                self.execute_with_retry(&tool_candidates[0].tool_name, context)
                    .await
            }
            ...
            ExecutionStrategy::CircuitBreakerProtected => {
                self.execute_with_circuit_breaker(&tool_candidates[0].tool_name, context)
                    .await
            }
        };
        ...
        match &result {
            Ok(exec_result) => {
                self.update_success_metrics(&exec_result.tool_name, execution_time)
                    .await;
                self.tool_selector
            ...
            Err(e) => {
                ...
                if let Some(first_candidate) = tool_candidates.first() {
                    self.update_failure_metrics(&first_candidate.tool_name, execution_time)
                        .await;
                    self.tool_selector
                        .update_tool_performance(
    ...
    async fn execute_with_circuit_breaker(
        ...
        match tokio::time::timeout(timeout_duration, execution_future).await {
            ...
            Ok(Err(e)) => {
                ...
                Err(anyhow!(
                    "Circuit breaker execution failed for tool: {} - {}",
                    tool_name,
                    e
                ))
            }
    ...
    async fn execute_tool_internal(&self, tool_name: &str, input: ToolInput) -> Result<ToolOutput> {
