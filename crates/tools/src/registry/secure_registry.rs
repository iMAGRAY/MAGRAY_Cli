...
use super::tool_metadata::{SecurityLevel, ToolMetadata, ToolPermissions};
...
impl InputValidator {
    ...
    fn sanitize_based_on_permissions(
        input: &mut ToolInput,
    ...
    fn sanitize_file_path(
        path: &str,
        fs_perms: &crate::registry::tool_metadata::FileSystemPermissions,
    ) -> Result<String> {
        ...
        match fs_perms {
            ...
            FileSystemPermissions::Restricted { allowed_paths } => {
                let path_allowed = allowed_paths
                    .iter()
                    .any(|allowed| sanitized.starts_with(allowed));
                if !path_allowed {
    ...
    fn sanitize_url(
        url: &str,
        net_perms: &crate::registry::tool_metadata::NetworkPermissions,
    ) -> Result<String> {
        ...
        match net_perms {
            ...
            NetworkPermissions::Restricted { allowed_hosts } => {
                let host_allowed = allowed_hosts.iter().any(|allowed| url.contains(allowed));
                if !host_allowed {
    ...
    fn sanitize_command(
        command: &str,
        sys_perms: &crate::registry::tool_metadata::SystemPermissions,
    ) -> Result<String> {
...
impl SecureToolRegistry {
    ...
    pub async fn register_tool(
        ...
        info!(
            "🔧 Tool registered securely: {} ({})",
            metadata.name, tool_id
        );
        Ok(())
    }
    ...
    pub async fn execute_tool(
        ...
        self.start_execution_tracking(&execution_id, tool_id, &security_context.session_id)
            .await;
        ...
        self.update_tool_performance(tool_id, execution_time, success)
            .await;
    ...
    fn check_resource_requirements(
        ...
        if let Some(tool_time) = tool_reqs.max_execution_time {
            if tool_time > user_limits.max_execution_time {
                return Err(anyhow!(
                    "Tool execution time requirement exceeds user limit"
                ));
            }
        }
    ...
    async fn update_tool_performance(
        &self,
        tool_id: &str,
        ...
        if let Some((_, metadata)) = tools.get_mut(tool_id) {
            metadata
                .performance_metrics
                .update_execution(execution_time, success);
...
mod tests {
    ...
    fn test_input_validator_sql_injection() {
        ...
        assert!(InputValidator::contains_sql_injection(
