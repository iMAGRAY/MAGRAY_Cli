            ...
            for (path, plugin_id) in paths {
                if let Ok(metadata) = std::fs::metadata(&path) {
                    if let Ok(modified) = metadata.modified() {
                        ...
                        if should_reload {
                            let was_existing = last_modified.insert(path.clone(), modified).is_some();
...
impl HotReloadManager {
    ...
    pub async fn register_plugin(
        ...
        self.file_watcher.watch_path(plugin_path, plugin_id.clone()).await?;
    ...
    pub async fn trigger_reload(&self, plugin_id: &str) -> Result<()> {
        ...
        match policy {
            ...
            _ => {
                warn!("Manual reload triggered for plugin with automatic policy: {}", plugin_id);
                self.execute_reload(plugin_id).await?;
            }
        }
    ...
    async fn handle_reload_event(&self, event: ReloadEvent) -> Result<()> {
        ...
        let policies = self.reload_policies.read().await;
        let policy = policies.get(plugin_id).cloned()
...
mod tests {
    ...
    async fn test_file_watcher_creation() {
        ...
        assert!(watcher.watch_path(temp_file.clone(), "test_plugin".to_string()).await.is_ok());
