...
impl ProcessLifecycleManager {
    ...
    pub async fn start(&mut self, cmd: &str, args: Vec<String>) -> Result<(), SubprocessError> {
        ...
        let stdin = child.stdin.take().ok_or_else(|| {
            SubprocessError::CommunicationError("Failed to get stdin".to_string())
        })?;
        let stdout = child.stdout.take().ok_or_else(|| {
            SubprocessError::CommunicationError("Failed to get stdout".to_string())
        })?;
        let stderr = child.stderr.take().ok_or_else(|| {
            SubprocessError::CommunicationError("Failed to get stderr".to_string())
        })?;
    ...
    pub fn send_stdin(&self, data: String) -> Result<(), SubprocessError> {
        if let Some(ref tx) = self.stdin_tx {
            tx.send(data).map_err(|_| {
                SubprocessError::CommunicationError("Failed to send to stdin".to_string())
            })?;
            Ok(())
        } else {
...
impl SubprocessRunner {
    ...
    pub async fn recv_response(
        &self,
        process_id: &str,
    ) -> Result<Option<JsonRpcResponse>, SubprocessError> {
        let mut processes = self
            .processes
            .lock()
            .map_err(|_| SubprocessError::CommunicationError("Lock error".to_string()))?;
