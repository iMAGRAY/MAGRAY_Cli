...
impl AutoDiffEngine {
    ...
    pub async fn take_snapshot(&mut self, snapshot_id: String) -> Result<()> {
        ...
        self.scan_directory(&self.base_path.clone(), &mut files).await?;
                ...
                Some(before_snapshot) => {
                    ...
                    if before_snapshot.content_hash != after_snapshot.content_hash {
                        modified_files.push(FileChange {
                            ...
                            after: Some(after_snapshot.clone()),
                            description: self.describe_modification(before_snapshot, after_snapshot),
                        });
    ...
    pub fn format_diff(&self, diff: &DiffResult) -> String {
        ...
        output.push_str(&format!("  Added:    {} files (+{} bytes)\n", 
                                diff.summary.files_added, diff.summary.bytes_added));
        output.push_str(&format!("  Modified: {} files (~{} bytes)\n", 
                                diff.summary.files_modified, diff.summary.bytes_modified));
        output.push_str(&format!("  Deleted:  {} files (-{} bytes)\n\n", 
        ...
        if !diff.modified_files.is_empty() {
            ...
            for change in &diff.modified_files {
                output.push_str(&format!("  ~ {} ({})\n", 
                                        change.path.display(), change.description));
            }
            output.push('\n');
        }
    ...
    async fn scan_directory(&self, dir: &Path, files: &mut HashMap<PathBuf, FileSnapshot>) -> Result<()> {
        if !dir.exists() {
            return Ok(());
        }
        ...
        for entry in entries {
            ...
            if metadata.is_dir() {
                // Record directory
                files.insert(path.clone(), FileSnapshot {
                    size: 0,
    ...
    fn compute_summary(&self, added: &[FileChange], modified: &[FileChange], deleted: &[FileChange]) -> DiffSummary {
        let files_added = added.len() as u32;
        let files_modified = modified.len() as u32;
        let files_deleted = deleted.len() as u32;
        
        let bytes_added = added.iter()
            .filter_map(|change| change.after.as_ref())
            .map(|snapshot| snapshot.size)
            .sum();
        
        let bytes_removed = deleted.iter()
            .filter_map(|change| change.before.as_ref())
            .map(|snapshot| snapshot.size)
            .sum();
        
        let bytes_modified = modified.iter()
            .filter_map(|change| {
                match (&change.before, &change.after) {
                    (Some(before), Some(after)) => {
                        Some(if after.size > before.size {
