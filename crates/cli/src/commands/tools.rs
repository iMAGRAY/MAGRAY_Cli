...
use std::fs;
use tools::intelligent_selector::{
    IntelligentToolSelector, SelectorConfig, TaskComplexity, ToolSelectionContext, UrgencyLevel,
    UserExpertise,
};
...
fn apply_usage_guide_override(spec: &mut tools::ToolSpec, override_v: &serde_json::Value) {
    let mut guide = spec
        .usage_guide
        .clone()
        .unwrap_or_else(|| tools::generate_usage_guide(spec));
    if let Some(obj) = override_v.as_object() {
        if let Some(v) = obj.get("usage_title").and_then(|v| v.as_str()) {
            guide.usage_title = v.into();
        }
        if let Some(v) = obj.get("usage_summary").and_then(|v| v.as_str()) {
            guide.usage_summary = v.into();
        }
        if let Some(v) = obj.get("preconditions").and_then(|v| v.as_array()) {
            guide.preconditions = v
                .iter()
        ...
        if let Some(v) = obj.get("arguments_brief").and_then(|v| v.as_object()) {
            ...
            for (k, val) in v {
                if let Some(s) = val.as_str() {
                    ab.insert(k.clone(), s.to_string());
                }
            }
            guide.arguments_brief = ab;
        }
        if let Some(v) = obj.get("good_for").and_then(|v| v.as_array()) {
            guide.good_for = v
                .iter()
                .filter_map(|x| x.as_str().map(|s| s.to_string()))
                .collect();
        }
        if let Some(v) = obj.get("not_for").and_then(|v| v.as_array()) {
            guide.not_for = v
                .iter()
                .filter_map(|x| x.as_str().map(|s| s.to_string()))
                .collect();
        ...
        if let Some(v) = obj.get("examples").and_then(|v| v.as_array()) {
            guide.examples = v
                .iter()
                .filter_map(|x| x.as_str().map(|s| s.to_string()))
                .collect();
        }
        if let Some(v) = obj.get("platforms").and_then(|v| v.as_array()) {
            guide.platforms = v
                .iter()
                .filter_map(|x| x.as_str().map(|s| s.to_string()))
                .collect();
        }
        if let Some(v) = obj.get("cost_class").and_then(|v| v.as_str()) {
            guide.cost_class = v.into();
        }
        if let Some(v) = obj.get("latency_class").and_then(|v| v.as_str()) {
            guide.latency_class = v.into();
        }
        if let Some(v) = obj.get("side_effects").and_then(|v| v.as_array()) {
            guide.side_effects = v
                .iter()
                .filter_map(|x| x.as_str().map(|s| s.to_string()))
                .collect();
        }
        if let Some(v) = obj.get("risk_score").and_then(|v| v.as_u64()) {
            guide.risk_score = v as u8;
        }
        if let Some(v) = obj.get("capabilities").and_then(|v| v.as_array()) {
            guide.capabilities = v
                .iter()
                .filter_map(|x| x.as_str().map(|s| s.to_string()))
                .collect();
        }
...
fn map_plugin_perms_to_tools_permissions(
    p: &tools::registry::ToolPermissions,
) -> Option<tools::ToolPermissions> {
...
async fn handle_tools_command(cmd: ToolsSubcommand) -> Result<()> {
    ...
    async fn export_plugins_into_registry(registry: &mut ToolRegistry) {
        if std::env::var("MAGRAY_EXPORT_PLUGINS_AS_TOOLS")
            .ok()
            ...
            .unwrap_or(true)
        {
            ...
            let pregistry =
                tools::plugins::plugin_manager::PluginRegistry::new(plugins_dir, cfg_dir);
            ...
            for p in plugins {
                if matches!(
                    p.plugin_type,
                    tools::plugins::plugin_manager::PluginType::Wasm
                        | tools::plugins::plugin_manager::PluginType::ExternalProcess
                ) {
    ...
    match cmd {
        ToolsSubcommand::List { details, json } => {
            ...
            for spec in &mut specs {
                if let Some(ov) = guide_overrides.get(&spec.name) {
                    apply_usage_guide_override(spec, ov);
                }
            }
            if json {
                ...
                if details {
                    ...
                    if let Some(perms) = &spec.permissions {
                        if !perms.fs_read_roots.is_empty() {
                            println!("  perm.fs_read: {}", perms.fs_read_roots.join(":"));
                        }
                        if !perms.fs_write_roots.is_empty() {
                            println!("  perm.fs_write: {}", perms.fs_write_roots.join(":"));
                        }
                        if !perms.net_allowlist.is_empty() {
                            println!("  perm.net_allow: {}", perms.net_allowlist.join(","));
                        }
                        if perms.allow_shell {
                            println!("  perm.shell: allow");
                        }
                    }
                    if let Some(guide) = &spec.usage_guide {
                        ...
                        println!(
                            "  latency: {}  risk: {}",
                            guide.latency_class, guide.risk_score
                        );
                    }
                }
            }
            ...
            if json {
                ...
                if let Some(obj) = snap.get("tools").and_then(|v| v.as_object()) {
                    for (tool, v) in obj {
                        ...
                        println!(
                            "- {}: invocations={}, successes={}, asks={}, denies={}",
                            tool.bold(),
                            inv,
                            ok,
                            ask,
        ...
        ToolsSubcommand::Select {
            ...
            if json {
                ...
                for e in explained {
                    ...
                    if !e.matched.tags.is_empty() {
                        println!("  tags: {}", e.matched.tags.join(", "));
                    }
                    if !e.matched.capabilities.is_empty() {
                        println!("  caps: {}", e.matched.capabilities.join(", "));
                    }
                    if !e.matched.good_for.is_empty() {
                        println!("  good_for: {}", e.matched.good_for.join(", "));
                    }
        ...
        ToolsSubcommand::Sandbox { show, save, json } => {
            ...
            println!(
                "Использование: magray tools sandbox --show | --save --json '<SandboxConfig JSON>'"
            );
            Ok(())
        }
        ToolsSubcommand::Plugins {
            list,
            load_manifests,
            dir,
            json,
            ...
            let home = crate::util::magray_home();
            let plugins_dir = if let Some(d) = dir {
            ...
            if list {
                ...
                if json {
                    ...
                    for p in &items {
                        ...
                        println!(
                            "  perms: fs={:?} net={:?} sys={:?}",
                            p.required_permissions.file_system,
                            p.required_permissions.network,
                            p.required_permissions.system
                        );
                    }
                }
                if export_tools {
                    ...
                    for p in items {
                        let perms_opt =
                            map_plugin_perms_to_tools_permissions(&p.required_permissions);
                    ...
                    if json {
                        ...
                        println!(
                            "{}",
                            "=== Exported Plugin Tools (preview) ===".bold().cyan()
                        );
                        for s in specs {
                            ...
                            if let Some(perms) = &s.permissions {
                                ...
                                println!(
                                    ...
                                    if perms.net_allowlist.is_empty() {
                                        ...
                                        perms.net_allowlist.join(",")
                                    }
                                );
                                println!(
                                    "  perm.shell: {}",
                                    if perms.allow_shell { "allow" } else { "deny" }
                                );
                            }
                        }
                    }
        ...
        ToolsSubcommand::Run {
            ...
            if name == "web_fetch" {
                if let Some(url) = args_map.get("url").cloned() {
                    let domain = url
                        ...
                        .to_string();
                    if !domain.is_empty() {
                        args_map.insert("domain".into(), domain);
                    }
                }
            } else if name == "web_search" {
                if let Some(q) = args_map.get("query").cloned() {
                    let lowered = q.to_lowercase();
                    if lowered.contains("internal") {
                        args_map.insert("keyword".into(), "internal".into());
                    }
                    if lowered.contains("secret") {
                        args_map.insert("keyword".into(), "secret".into());
                    }
                }
            }
            ...
            if let Some(perms) = &spec.permissions {
                ...
                if let Some(pre) =
                    common::policy::precheck_permissions(&name, &simple, &sandbox_cfg)
                {
                    match pre.action {
                        common::policy::PolicyAction::Deny => {
                            anyhow::bail!(
                                "Инструмент '{}' заблокирован политикой (precheck)",
                                name
                            );
                        }
                        common::policy::PolicyAction::Ask => {
                            let non_interactive = std::env::var("MAGRAY_NONINTERACTIVE")
                                .unwrap_or_default()
                                == "true";
                            if non_interactive {
                                anyhow::bail!("Инструмент '{}' требует подтверждения (precheck), но режим non-interactive", name);
                            }
                        }
            ...
            if let Some(ov) = guide_overrides.get(&name) {
                apply_usage_guide_override(&mut spec, ov);
            }
            if matches!(decision.action, common::policy::PolicyAction::Allow)
            ...
            if matches!(decision.action, common::policy::PolicyAction::Ask)
                || require_ask_due_to_guide
            {
                let non_interactive =
                    std::env::var("MAGRAY_NONINTERACTIVE").unwrap_or_default() == "true";
                if non_interactive {
                    anyhow::bail!(
                        "Tool '{}' requires confirmation (ask), but running non-interactive",
                        name
                    );
                }
                let auto_approve =
                    std::env::var("MAGRAY_AUTO_APPROVE_ASK").unwrap_or_default() == "true";
                if !auto_approve {
                    let preview_input = tools::ToolInput {
                    ...
                    if require_ask_due_to_guide {
                        println!("Требуется подтверждение по UsageGuide (risk/side_effects)");
                    }
            ...
            if output.success {
                println!("{} {}", "✓".green(), output.result);
            } else {
                println!("{} {}", "✗".red(), output.result);
            }
