...
impl ModelsCommand {
    ...
    fn list_models(model_type_filter: Option<String>, available_only: bool) -> Result<()> {
        ...
        let filter_type = match model_type_filter.as_deref() {
            ...
            Some(unknown) => {
                warn!(
                    "⚠️ Неизвестный тип модели: {}. Доступные: embedding, reranker",
                    unknown
                );
                None
            }
        ...
        if !embedding_models.is_empty() {
            ...
            for model in embedding_models {
                ...
                let default_mark = if model.is_default {
                    " [По умолчанию]"
                } else {
                    ""
                };
    ...
    fn show_recommendations() -> Result<()> {
        ...
        info!("  • Qwen3 модели рекомендованы; поддержка BGE снята");
    ...
    fn check_models() -> Result<()> {
        ...
        let embedding_available =
            MODEL_REGISTRY.is_model_available(&ai_config.embedding.model_name);
        let reranking_available =
            MODEL_REGISTRY.is_model_available(&ai_config.reranking.model_name);
