...
use crate::agent_traits::{
    AdminResponse, AdminServiceTrait, AgentResponse, CircuitBreakerTrait, ComponentLifecycleTrait,
    RequestContext,
};
    ...
    C: CircuitBreakerTrait,
{
    ...
    pub async fn handle_admin_request(&self, context: &RequestContext) -> Result<AgentResponse> {
        ...
        debug!(
            "AdminHandler: обработка административной команды: {}",
            context.message
        );
        ...
        let response = match admin_type.as_str() {
            "system_stats" => self.handle_system_stats().await?,
            "health_check" => self.handle_health_check().await?,
            "performance_metrics" => self.handle_performance_metrics().await?,
            "custom_command" => self.handle_custom_command(context).await?,
            _ => {
    ...
    async fn handle_system_stats(&self) -> Result<AdminResponse> {
        ...
        self.circuit_breaker
            .execute(async { self.admin_service.get_system_stats().await })
            .await
    }
    ...
    async fn handle_health_check(&self) -> Result<AdminResponse> {
        ...
        self.circuit_breaker
            .execute(async { self.admin_service.check_system_health().await })
            .await
    }
    ...
    async fn handle_performance_metrics(&self) -> Result<AdminResponse> {
        ...
        self.circuit_breaker
            .execute(async { self.admin_service.get_performance_metrics().await })
            .await
    }
    ...
    async fn handle_custom_command(&self, context: &RequestContext) -> Result<AdminResponse> {
        ...
        self.circuit_breaker
            .execute(async {
                self.admin_service
                    .execute_admin_command(&command, &args)
                    .await
            })
            .await
    }
    ...
    fn classify_admin_request(&self, message: &str) -> String {
        ...
        } else if message_lower.contains("метрики")
            || message_lower.contains("metrics")
            || message_lower.contains("производительность")
...
mod tests {
    ...
    impl AdminServiceTrait for MockAdminService {
        ...
        async fn get_performance_metrics(&self) -> Result<AdminResponse> {
            Ok(AdminResponse::PerformanceMetrics(
                "Mock metrics".to_string(),
            ))
        }
        ...
        async fn execute_admin_command(
            ...
            Ok(AdminResponse::OperationResult(format!(
                "Executed: {}",
                command
            )))
        }
    ...
    async fn test_admin_handler_stats_detection() {
        ...
        assert_eq!(
            handler.classify_admin_request(&context.message),
            "system_stats"
        );
    }
