...
use crate::agent_traits::{
    AgentResponse, CircuitBreakerTrait, ComponentLifecycleTrait, MemoryManagementTrait,
    RequestContext,
};
    ...
    C: CircuitBreakerTrait,
{
    ...
    pub async fn store_user_message(&self, context: &RequestContext) -> Result<AgentResponse> {
        ...
        self.circuit_breaker
            .execute(async {
                self.memory_service
                    .store_message(&context.message, &context.metadata)
                    .await
            })
            .await?;
        ...
        Ok(AgentResponse::ToolExecution(
            "Сообщение сохранено в память".to_string(),
        ))
    }
    ...
    pub async fn search_memory(&self, query: &str, limit: usize) -> Result<AgentResponse> {
        ...
        let results = self
            .circuit_breaker
            .execute(async { self.memory_service.search_memory(query, limit).await })
            .await?;
        ...
        info!(
            "MemoryHandler: поиск завершен, найдено {} результатов",
            results.len()
        );
    ...
    pub async fn run_memory_promotion(&self) -> Result<AgentResponse> {
        ...
        let result = self
            .circuit_breaker
            .execute(async { self.memory_service.run_promotion().await })
            .await?;
    ...
    pub async fn get_memory_statistics(&self) -> Result<AgentResponse> {
        ...
        let stats = self
            .circuit_breaker
            .execute(async { self.memory_service.get_memory_stats().await })
            .await?;
    ...
    pub async fn check_memory_health(&self) -> Result<String> {
        ...
        self.circuit_breaker
            .execute(async { self.memory_service.health_check().await })
            .await?;
