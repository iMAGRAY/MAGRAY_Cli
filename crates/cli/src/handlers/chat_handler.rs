...
use crate::agent_traits::{
    LlmServiceTrait, ComponentLifecycleTrait, CircuitBreakerTrait,
    ...
    C: CircuitBreakerTrait,
{
    ...
    pub async fn handle_chat(&self, context: &RequestContext) -> Result<AgentResponse> {
        ...
        debug!("ChatHandler: обработка запроса длиной {}", context.message.len());
        
        // Используем Circuit Breaker для защиты от сбоев LLM
        let response = self.circuit_breaker.execute(async {
            self.llm_service.chat(&context.message).await
        }).await?;
    ...
    pub async fn handle_chat_with_context(&self, context: &RequestContext) -> Result<AgentResponse> {
        if !self.initialized {
            return Err(anyhow::anyhow!("ChatHandler не инициализирован"));
        }
        
        debug!("ChatHandler: обработка запроса с контекстом: {:?}", context.metadata);
        
        let response = self.circuit_breaker.execute(async {
            self.llm_service.chat_with_context(&context.message, &context.metadata).await
        }).await?;
    ...
    pub async fn can_handle(&self, context: &RequestContext) -> bool {
        ...
        let has_tool_indicators = tool_indicators.iter()
            .any(|&indicator| message_lower.contains(indicator));
        
        // Исключаем административные команды
        let admin_indicators = [
            "статистика", "stats", "здоровье", "health", "метрики", "metrics",
            "система", "system", "производительность", "performance"
        ];
        
        let has_admin_indicators = admin_indicators.iter()
            .any(|&indicator| message_lower.contains(indicator));
