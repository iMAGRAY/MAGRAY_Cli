...
mod tests {
    ...
    async fn test_unified_agent_initialization() {
        ...
        if let Ok(_agent) = result {
            ...
            assert!(e.to_string().contains("LLM") || e.to_string().contains("env"), 
                   "Expected LLM initialization error, got: {}", e);
        }
    }
    ...
    async fn test_agent_initialization_with_fallback() {
        ...
        if let Err(e) = result {
            assert!(e.to_string().contains("API") || e.to_string().contains("env"),
    ...
    async fn test_process_tool_message() {
        ...
        assert!(message.contains("файл") || message.contains("список"), 
    ...
    async fn test_simple_heuristic_detection() {
        ...
        for msg in chat_messages {
            let has_indicator = ["файл", "git", "создай", "прочитай", "найди"]
                .iter()
                .any(|&word| msg.contains(word));
            assert!(!has_indicator, "Should not detect tool indicator in: {}", msg);
        }
    }
    ...
    async fn test_fallback_heuristic_priority() {
        ...
        for (msg, should_be_tool) in messages_with_priority {
            ...
            if should_be_tool {
                ...
                assert!(msg.contains("?") || msg.contains("объясни") || msg.contains("что"),
                       "Questions should not trigger tools: {}", msg);
            }
        }
    }
    ...
    async fn test_integration_with_memory_service() {
        ...
        assert_eq!(std::mem::size_of::<DIMemoryService>() > 0, true,
    ...
    async fn test_integration_with_router() {
        ...
        for cmd in tool_commands {
            ...
            assert!(cmd.split_whitespace().count() >= 1, "Command should have parts");
        }
    }
    ...
    async fn test_circuit_breaker_on_repeated_failures() {
        ...
        for _ in 0..10 {
            ...
            if current_failures >= max_failures {
                ...
                assert!(current_failures >= max_failures, 
                       "Circuit should be open after {} failures", max_failures);
                break;
            }
