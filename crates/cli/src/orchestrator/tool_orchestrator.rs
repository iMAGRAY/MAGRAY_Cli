...
use crate::orchestrator::{AdaptiveTaskOrchestrator, OrchestrationConfig, OrchestrationResult};
use tools::enhanced_tool_system::{
    EnhancedToolResult, EnhancedToolSystem, EnhancedToolSystemConfig,
};
...
impl ToolOrchestrator {
    ...
    pub async fn new(config: ToolOrchestratorConfig) -> Result<Self> {
        ...
        let task_orchestrator = Arc::new(AdaptiveTaskOrchestrator::new(
            config.orchestration_config.clone(),
        ));
        let tool_system =
            Arc::new(EnhancedToolSystem::new(config.tool_system_config.clone()).await?);
    ...
    pub async fn register_tool(&self, tool: Arc<dyn Tool>) -> Result<()> {
        ...
        info!(
            "📝 Registering tool with integrated orchestrator: {}",
            tool_spec.name
        );
    ...
    pub async fn process_request(
        ...
        info!(
            "🎯 Processing integrated orchestration request: '{}'",
            content
        );
        ...
        let orchestration_result = self
            .task_orchestrator
            .orchestrate_task(content, context.clone())
            .await?;
        let orchestration_time = orchestration_start.elapsed();

        debug!(
            "📊 Orchestration phase completed in {:?}",
            orchestration_time
        );
        ...
        let (tool_result, optimization_applied) = if should_use_tool_system {
            ...
            let enhanced_result = self
                .tool_system
                .execute_request(content, Some(context))
                .await?;
            ...
            let optimization = if self.config.enable_cross_system_optimization {
                self.apply_cross_system_optimization(&orchestration_result, &enhanced_result)
                    .await
            } else {
                false
            };
            ...
            debug!(
                "⚡ Cross-system optimization completed in {:?}",
                optimization_time
            );
        ...
        // Update success metrics
        {
            ...
            let new_avg = Duration::from_nanos(
                (((current_avg.as_nanos() * (requests - 1) as u128) + overhead.as_nanos())
                    / requests as u128) as u64,
            );
        ...
        let resource_efficiency = self
            .calculate_resource_efficiency(&orchestration_result)
            .await;
        ...
        let result = IntegratedOrchestrationResult {
            ...
            performance_metrics: IntegrationPerformanceMetrics {
                ...
                orchestration_time,
                tool_execution_time,
                optimization_time: if optimization_applied {
        ...
        info!(
            "✅ Integrated orchestration completed in {:?}",
            total_execution_time
        );
        Ok(result)
    }
    ...
    async fn should_delegate_to_tool_system(
        &self,
    ...
    async fn apply_cross_system_optimization(
        ...
        let execution_time_diff = tool_result.execution_result.execution_time.as_millis() as i64
            - orchestration_result.execution_time.as_millis() as i64;

        if execution_time_diff.abs() > 500 {
            // More than 500ms difference
            debug!(
                "⚡ Applying timing optimization based on execution difference: {}ms",
                execution_time_diff
            );
    ...
    async fn calculate_alignment_score(
        ...
        let success_alignment =
            if orchestration_result.success == tool_result.execution_result.output.success {
                1.0
            } else {
                0.0
            };
    ...
    pub async fn get_comprehensive_stats(&self) -> String {
        ...
        // Integration metrics
        {
            ...
            let success_rate = if metrics.total_orchestrated_requests > 0 {
                (metrics.successful_orchestrations as f32
                    / metrics.total_orchestrated_requests as f32)
                    * 100.0
            } else {
                0.0
            };
            ...
            let tool_delegation_rate = if metrics.total_orchestrated_requests > 0 {
                (metrics.tool_system_handoffs as f32 / metrics.total_orchestrated_requests as f32)
                    * 100.0
            } else {
                0.0
            };
        ...
        for line in orchestrator_stats.lines().skip(2) {
            // Skip header
            stats.push_str(&format!("  {}\n", line));
        }
        stats.push('\n');
        ...
        for line in tool_stats.lines().skip(1) {
            // Skip header
            stats.push_str(&format!("  {}\n", line));
        }
