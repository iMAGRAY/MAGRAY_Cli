...
impl ResourceManager {
    ...
    pub fn with_limits(limits: ResourceLimits) -> Self {
        info!("🔧 Initializing ResourceManager with limits: CPU={:.0}%, Memory={:.0}%, Concurrent={}",
    ...
    pub async fn can_allocate_resources(&self, requirements: &ResourceRequirements) -> Result<bool> {
        debug!("🔍 Checking resource availability for task requiring: CPU={:.1}%, Memory={:.1}%",
        ...
        let can_allocate = 
            projected_cpu <= self.resource_limits.max_cpu_usage &&
        ...
        if can_allocate {
            ...
            debug!("✅ Resources can be allocated: projected usage CPU={:.1}%, Memory={:.1}%",
    ...
    pub async fn reserve_resources(&self, task_id: &str, requirements: ResourceRequirements) -> Result<()> {
    ...
    pub async fn get_utilization_trends(&self) -> HashMap<String, f32> {
        ...
        if !recent_samples.is_empty() {
            let avg_cpu = recent_samples.iter().map(|s| s.cpu_usage).sum::<f32>() / recent_samples.len() as f32;
            let avg_memory = recent_samples.iter().map(|s| s.memory_usage).sum::<f32>() / recent_samples.len() as f32;
            let avg_io = recent_samples.iter().map(|s| s.io_usage).sum::<f32>() / recent_samples.len() as f32;
            let avg_network = recent_samples.iter().map(|s| s.network_usage).sum::<f32>() / recent_samples.len() as f32;
    ...
    pub async fn get_resource_stats(&self) -> String {
        ...
        stats.push_str(&format!("  • CPU: {:.1}% (limit: {:.0}%)\n", 
                               usage.cpu_usage * 100.0, 
                               self.resource_limits.max_cpu_usage * 100.0));
        stats.push_str(&format!("  • Memory: {:.1}% (limit: {:.0}%)\n",
                               usage.memory_usage * 100.0,
                               self.resource_limits.max_memory_usage * 100.0));
        stats.push_str(&format!("  • IO: {:.1}% (limit: {:.0}%)\n",
                               usage.io_usage * 100.0,
                               self.resource_limits.max_io_usage * 100.0));
        stats.push_str(&format!("  • Network: {:.1}% (limit: {:.0}%)\n",
                               usage.network_usage * 100.0,
                               self.resource_limits.max_network_usage * 100.0));
        
        stats.push_str("\n📋 Resource Allocation:\n");
        stats.push_str(&format!("  • Active tasks: {} (limit: {})\n",
                               reservations.len(),
                               self.resource_limits.max_concurrent_tasks));
        stats.push_str(&format!("  • Total requests: {}\n", allocation_stats.total_requests));
        stats.push_str(&format!("  • Approved: {} ({:.1}%)\n",
                               allocation_stats.approved_requests,
                               if allocation_stats.total_requests > 0 {
                                   (allocation_stats.approved_requests as f32 / allocation_stats.total_requests as f32) * 100.0
                               } else { 0.0 }));
        stats.push_str(&format!("  • Denied: {} ({:.1}%)\n",
                               allocation_stats.denied_requests,
                               if allocation_stats.total_requests > 0 {
                                   (allocation_stats.denied_requests as f32 / allocation_stats.total_requests as f32) * 100.0
                               } else { 0.0 }));
        
        stats.push_str("\n⚡ Performance Features:\n");
        stats.push_str(&format!("  • Priority boosting: {} (threshold: {:.0}%)\n",
                               if self.should_prioritize_high_priority().await { "Active" } else { "Inactive" },
                               self.resource_limits.priority_boost_threshold * 100.0));
        stats.push_str(&format!("  • Resource warnings: {}\n", allocation_stats.resource_warnings));
        stats.push_str(&format!("  • High-priority boosts: {}\n", allocation_stats.high_priority_boosts));
    ...
    pub async fn cleanup_expired_reservations(&self) -> usize {
        ...
        if cleaned_count > 0 {
            debug!("🧹 Cleaned up {} expired resource reservations", cleaned_count);
        }
