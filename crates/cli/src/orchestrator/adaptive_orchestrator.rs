...
impl LoadBalancer {
    ...
    fn select_handler(&mut self, task: &OrchestrationTask, available_handlers: &[String]) -> String {
    ...
    fn update_load(&mut self, handler: &str, execution_time: Duration, success: bool) {
        ...
        let load_delta = if success { time_factor * 0.1 } else { time_factor * 0.3 };
        ...
        self.adaptive_weights.insert(handler.to_string(), new_weight);
    }
}
...
impl AdaptiveTaskOrchestrator {
    ...
    pub async fn orchestrate_task(&self, content: &str, context: HashMap<String, String>) -> Result<OrchestrationResult> {
        ...
        let resource_requirements = self.task_analyzer.estimate_resources(&complexity, &priority).await;
        ...
        let can_execute = self.resource_manager.can_allocate_resources(&resource_requirements).await?;
        if !can_execute {
            return Err(anyhow::anyhow!("Insufficient resources available for task execution"));
        }
        ...
        info!("📋 Task {} routed to handler: {}", task_id, selected_handler);
        
        // Step 5: Execute task
        let execution_result = self.execute_task_on_handler(&orchestration_task, &selected_handler).await;
        
        let execution_time = start_time.elapsed();
        
        // Step 6: Update metrics and cleanup
        let success = execution_result.is_ok();
        self.update_handler_metrics(&selected_handler, execution_time, success).await;
        ...
        let result = match execution_result {
            Ok(response) => {
                info!("✅ Task {} completed successfully in {:?}", task_id, execution_time);
                OrchestrationResult {
                    task_id,
                    response,
                    execution_time,
        ...
        // Update statistics
        {
            ...
            *total_tasks += 1;
            ...
            let new_avg = (current_avg * (tasks_count - 1.0) + execution_time.as_millis() as f64) / tasks_count;
    ...
    async fn execute_task_on_handler(&self, task: &OrchestrationTask, handler: &str) -> Result<String> {
        ...
        Ok(format!("Task '{}' executed by {} handler", task.content, handler))
    }
    ...
    async fn update_handler_metrics(&self, handler: &str, execution_time: Duration, success: bool) {
        ...
        debug!("📊 Updated metrics for {}: {} tasks, {:.1}% success rate", 
               handler, metrics.total_tasks, 
    ...
    pub async fn submit_task(&self, task_id: &str, content: &str, context: HashMap<String, String>) -> Result<super::ExecutionStrategy> {
        ...
        if !self.resource_manager.can_allocate_resources(&resource_requirements).await? {
            return Err(anyhow::anyhow!("Insufficient resources available for task"));
        }
        
        // Reserve resources
        self.resource_manager.reserve_resources(task_id, resource_requirements.clone()).await?;
        ...
        let strategy = self.strategy_selector.lock().await.select_strategy(&selection_criteria).await?;
        
        // Create orchestration task
        let task_uuid = Uuid::parse_str(&task_id.replace("task_", "")).unwrap_or_else(|_| Uuid::new_v4());
        let orchestration_task = OrchestrationTask {
            id: task_uuid,
            content: content.to_string(),
            priority,
        ...
        // Add to active tasks
        self.active_tasks.write().await.insert(task_uuid, orchestration_task);
        
        info!("✅ Task {} orchestrated with strategy: {:?}", task_id, strategy);
        Ok(strategy)
    }
    
    /// Complete task and update metrics
    pub async fn complete_task(&self, task_id: &str, success: bool, duration: std::time::Duration) -> Result<()> {
    ...
    pub async fn shutdown(&self) -> Result<()> {
        ...
        if remaining_tasks > 0 {
            warn!("⚠️ Orchestrator shutdown with {} active tasks remaining", remaining_tasks);
        }
    ...
    pub async fn get_orchestration_stats(&self) -> String {
        ...
        stats.push_str(&format!("  • Currently active tasks: {}\n", active_tasks_count));
        stats.push_str(&format!("  • Max concurrent tasks: {}\n", self.config.max_concurrent_tasks));
        ...
        // Chat handler stats
        {
            ...
            stats.push_str(&format!("  • Chat Handler: {} tasks ({:.1}% success, {:?} avg)\n",
                metrics.total_tasks,
                if metrics.total_tasks > 0 { (metrics.successful_tasks as f32 / metrics.total_tasks as f32) * 100.0 } else { 0.0 },
        ...
        // Tools handler stats
        {
            ...
            stats.push_str(&format!("  • Tools Handler: {} tasks ({:.1}% success, {:?} avg)\n",
                metrics.total_tasks,
                if metrics.total_tasks > 0 { (metrics.successful_tasks as f32 / metrics.total_tasks as f32) * 100.0 } else { 0.0 },
                metrics.average_execution_time
            ));
        ...
        // Memory handler stats
        {
            ...
            stats.push_str(&format!("  • Memory Handler: {} tasks ({:.1}% success, {:?} avg)\n",
                metrics.total_tasks,
                if metrics.total_tasks > 0 { (metrics.successful_tasks as f32 / metrics.total_tasks as f32) * 100.0 } else { 0.0 },
                metrics.average_execution_time
            ));
        ...
        // Admin handler stats
        {
            ...
            stats.push_str(&format!("  • Admin Handler: {} tasks ({:.1}% success, {:?} avg)\n",
                metrics.total_tasks,
                if metrics.total_tasks > 0 { (metrics.successful_tasks as f32 / metrics.total_tasks as f32) * 100.0 } else { 0.0 },
                metrics.average_execution_time
            ));
        ...
        // Load balancer stats
        {
            ...
            for (handler, load) in &balancer.handler_loads {
                let weight = balancer.adaptive_weights.get(handler).copied().unwrap_or(1.0);
                stats.push_str(&format!("  • {}: load={:.2}, weight={:.2}\n", handler, load, weight));
            }
        }
    ...
    pub async fn initialize(&self) -> Result<()> {
        ...
        if !self.is_healthy().await {
            return Err(anyhow::anyhow!("Some orchestrator components are unhealthy"));
        }
