...
impl StrategySelector {
    ...
    pub async fn select_strategy(&mut self, criteria: &SelectionCriteria) -> Result<ExecutionStrategy> {
        debug!("🎯 Selecting execution strategy for task: complexity={:?}, priority={:?}, load={:.1}%",
               criteria.task_complexity, criteria.task_priority, criteria.system_load * 100.0);
        ...
        let count = self.performance_history.strategy_usage
            .entry(strategy.clone())
            .or_insert(0);
        *count += 1;
        
        info!("✅ Selected strategy: {:?} (confidence: {:.1}%)", 
              strategy, self.calculate_confidence(&strategy, criteria) * 100.0);
    ...
    async fn analyze_optimal_strategy(&self, criteria: &SelectionCriteria) -> ExecutionStrategy {
        ...
        scores.insert(ExecutionStrategy::Direct, self.score_direct_strategy(criteria));
        scores.insert(ExecutionStrategy::Batched, self.score_batched_strategy(criteria));
        scores.insert(ExecutionStrategy::Prioritized, self.score_prioritized_strategy(criteria));
        scores.insert(ExecutionStrategy::Parallel, self.score_parallel_strategy(criteria));
        scores.insert(ExecutionStrategy::LoadBalanced, self.score_load_balanced_strategy(criteria));
        scores.insert(ExecutionStrategy::Optimized, self.score_optimized_strategy(criteria));
    ...
    fn calculate_confidence(&self, strategy: &ExecutionStrategy, criteria: &SelectionCriteria) -> f32 {
        ...
        match (strategy, &criteria.task_priority, &criteria.task_complexity) {
            ...
            (ExecutionStrategy::Batched, _, _) if criteria.similar_tasks_count > 3 => confidence += 0.4,
            (ExecutionStrategy::Prioritized, TaskPriority::Emergency, _) => confidence += 0.4,
            (ExecutionStrategy::Parallel, _, TaskComplexity::Expert) if criteria.resource_availability > 0.7 => confidence += 0.4,
            (ExecutionStrategy::LoadBalanced, _, _) if criteria.system_load > 0.8 => confidence += 0.3,
            (ExecutionStrategy::Optimized, TaskPriority::Critical, TaskComplexity::Expert) => confidence += 0.5,
            _ => confidence += 0.1,
        }
    ...
    pub async fn update_metrics(&mut self, strategy: &ExecutionStrategy, success: bool, execution_time: Duration) {
        if success {
        ...
        let total_executions = self.performance_history.successful_executions + self.performance_history.failed_executions;
        
        if total_executions > 1 {
            self.performance_history.avg_execution_time_ms = 
                ((self.performance_history.avg_execution_time_ms * (total_executions - 1) as f32) + new_time_ms) 
        ...
        if self.adaptive_learning && total_executions % 10 == 0 {
            self.adjust_strategy_weights(strategy, success, execution_time).await;
        }
        
        debug!("📊 Updated metrics for strategy {:?}: success={}, time={:.1}ms", 
               strategy, success, new_time_ms);
    }
    
    /// Adjust strategy weights based on performance (adaptive learning)
    async fn adjust_strategy_weights(&mut self, strategy: &ExecutionStrategy, success: bool, execution_time: Duration) {
        ...
        debug!("🧠 Adaptive learning: adjusted {:?} weight from {:.2} to {:.2}", 
    ...
    pub fn get_strategy_stats(&self) -> String {
        ...
        let mut stats = format!(
            ...
            success_rate,
            self.performance_history.avg_execution_time_ms,
            if self.adaptive_learning { "Enabled" } else { "Disabled" }
        );
        ...
        if self.adaptive_learning && total > 50 {
            ...
            let best_strategy = usage_vec_clone.first()
                .map(|(s, _)| s)
                .unwrap_or(&&ExecutionStrategy::Direct);
    ...
    pub fn set_adaptive_learning(&mut self, enabled: bool) {
        ...
        info!("🧠 Adaptive learning {}", if enabled { "enabled" } else { "disabled" });
    }
}
