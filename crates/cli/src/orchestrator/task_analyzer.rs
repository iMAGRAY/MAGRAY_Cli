...
impl TaskAnalyzer {
    pub fn new() -> Self {
        ...
        complexity_keywords.insert("искусственный интеллект".to_string(), TaskComplexity::Expert);
        complexity_keywords.insert("artificial intelligence".to_string(), TaskComplexity::Expert);
    ...
    pub async fn analyze_task(&self, content: &str, context: &HashMap<String, String>) -> Result<(TaskPriority, TaskComplexity)> {
        ...
        info!("📊 Task analysis complete: priority={:?}, complexity={:?}", priority, complexity);
        
        Ok((priority, complexity))
    }
    
    /// Analyze task complexity using keyword matching and heuristics
    async fn analyze_complexity(&self, content: &str, context: &HashMap<String, String>) -> TaskComplexity {
        ...
        let has_technical_terms = content.contains("api") || content.contains("database") || 
                                 content.contains("algorithm") || content.contains("optimization");
        let has_multiple_steps = content.contains(" и ") || content.contains(" then ") || 
                               content.contains("after") || content.contains("потом");
        ...
        let context_complexity = if context.contains_key("urgent") || context.contains_key("critical") {
        ...
        complexities.into_iter().max_by_key(|c| match c {
            TaskComplexity::Simple => 1,
            TaskComplexity::Medium => 2,
            TaskComplexity::Complex => 3,
            TaskComplexity::Expert => 4,
    ...
    async fn analyze_priority(&self, content: &str, context: &HashMap<String, String>) -> TaskPriority {
        ...
        } else if context.get("retry_count").and_then(|s| s.parse::<u32>().ok()).unwrap_or(0) > 0 {
        ...
        if priority_score == 0 {
            ...
            match (matched_priority, context_priority) {
                (TaskPriority::Emergency, _) | (_, TaskPriority::Emergency) => TaskPriority::Emergency,
    ...
    pub async fn estimate_resources(&self, complexity: &TaskComplexity, priority: &TaskPriority) -> ResourceRequirements {
