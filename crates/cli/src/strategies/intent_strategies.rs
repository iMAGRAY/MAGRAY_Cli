...
use crate::agent_traits::{IntentDecisionStrategy, RequestContext, IntentDecision, LlmServiceTrait};
...
impl HeuristicIntentStrategy {
    ...
    fn analyze_keywords(&self, message: &str) -> (String, f32, HashMap<String, String>) {
        ...
        let chat_patterns = [
            ("привет", 0.9), ("здравствуй", 0.9), ("как дела", 0.8),
            ("что нового", 0.7), ("расскажи", 0.6), ("объясни", 0.5),
            ("как", 0.4), ("что", 0.3), ("почему", 0.5), ("зачем", 0.5),
        ];
        
        let tools_patterns = [
            ("файл", 1.0), ("file", 1.0), ("папка", 0.9), ("folder", 0.9),
            ("git", 1.0), ("commit", 0.9), ("status", 0.8),
            ("создай", 0.9), ("create", 0.9), ("прочитай", 0.9), ("read", 0.9),
            ("запиши", 0.9), ("write", 0.9), ("выполни", 0.8), ("execute", 0.8),
            ("команда", 0.8), ("command", 0.8), ("shell", 0.8),
        ];
        
        let memory_patterns = [
            ("память", 1.0), ("memory", 1.0), ("сохрани", 0.9), ("save", 0.9),
            ("найди", 0.8), ("search", 0.8), ("запомни", 0.9), ("remember", 0.9),
            ("promotion", 0.9), ("слои", 0.8), ("layers", 0.8),
        ];
        
        let admin_patterns = [
            ("статистика", 1.0), ("stats", 1.0), ("здоровье", 0.9), ("health", 0.9),
            ("метрики", 1.0), ("metrics", 1.0), ("система", 0.7), ("system", 0.7),
            ("производительность", 0.9), ("performance", 0.9),
        ];
        
        // Подсчет очков для каждого типа
        scores.insert("chat".to_string(), self.calculate_score(&message_lower, &chat_patterns));
        scores.insert("tools".to_string(), self.calculate_score(&message_lower, &tools_patterns));
        scores.insert("memory".to_string(), self.calculate_score(&message_lower, &memory_patterns));
        scores.insert("admin".to_string(), self.calculate_score(&message_lower, &admin_patterns));
        ...
        context.insert("analysis_method".to_string(), "heuristic_keywords".to_string());
...
impl IntentDecisionStrategy for HeuristicIntentStrategy {
    async fn analyze_intent(&self, context: &RequestContext) -> Result<IntentDecision> {
        debug!("HeuristicIntentStrategy: анализ намерения для '{}'", context.message);
        ...
        info!("HeuristicIntentStrategy: результат '{}' с уверенностью {:.2}", 
...
{
    async fn analyze_intent(&self, context: &RequestContext) -> Result<IntentDecision> {
        debug!("LlmIntentStrategy: анализ намерения через LLM для '{}'", context.message);
        
        let prompt = format!("{}\n\nСообщение пользователя: {}", self.system_prompt, context.message);
        ...
        analysis_context.insert("analysis_method".to_string(), "llm_classification".to_string());
        analysis_context.insert("reasoning".to_string(), reasoning);
        
        info!("LlmIntentStrategy: результат '{}' с уверенностью {:.2}", action_type, confidence);
    ...
    L: LlmServiceTrait,
{
    async fn analyze_intent(&self, context: &RequestContext) -> Result<IntentDecision> {
        ...
        if heuristic_result.confidence >= self.heuristic_threshold {
            info!("HybridIntentStrategy: используем результат эвристики (уверенность: {:.2})", 
                  heuristic_result.confidence);
            return Ok(heuristic_result);
        }
        
        // Иначе обращаемся к LLM для более точного анализа
        debug!("HybridIntentStrategy: эвристика дала низкую уверенность ({:.2}), используем LLM", 
               heuristic_result.confidence);
        ...
        if let Some(ref mut ctx) = llm_result.context {
            ...
            ctx.insert("heuristic_confidence".to_string(), heuristic_result.confidence.to_string());
            ctx.insert("heuristic_result".to_string(), heuristic_result.action_type);
        }
        
        info!("HybridIntentStrategy: финальный результат '{}' с уверенностью {:.2}", 
...
mod tests {
    ...
    impl LlmServiceTrait for MockLlmService {
        ...
        async fn chat_with_context(&self, message: &str, _context: &HashMap<String, String>) -> Result<String> {
    ...
    async fn test_hybrid_strategy_high_confidence() {
        ...
        assert!(result.context.as_ref().unwrap().get("analysis_method").unwrap().contains("heuristic"));
    }
    ...
    async fn test_hybrid_strategy_low_confidence() {
        ...
        assert!(result.context.as_ref().unwrap().contains_key("hybrid_analysis"));
    }
}