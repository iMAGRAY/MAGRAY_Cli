...
use crate::agent_traits::{FallbackStrategy, RequestContext, AgentResponse, LlmServiceTrait};
...
impl SimpleFallbackStrategy {
    ...
    fn classify_error(&self, error: &anyhow::Error) -> String {
        ...
        if error_string.contains("network") || error_string.contains("connection") {
            ...
            "timeout_error".to_string()
        } else if error_string.contains("llm") || error_string.contains("openai") || error_string.contains("anthropic") {
            "llm_error".to_string()
...
impl FallbackStrategy for SimpleFallbackStrategy {
    async fn handle_fallback(&self, context: &RequestContext, error: &anyhow::Error) -> Result<AgentResponse> {
        warn!("SimpleFallbackStrategy: обработка ошибки для '{}'", context.message);
        debug!("Ошибка: {}", error);
        
        let error_type = self.classify_error(error);
        let response_text = self.default_responses.get(&error_type)
            .unwrap_or(&self.default_responses["unknown_error"])
            .clone();
        
        info!("SimpleFallbackStrategy: возвращаем fallback ответ для типа '{}'", error_type);
    ...
    L: LlmServiceTrait,
{
    ...
    async fn try_alternative_approach(&self, context: &RequestContext, error: &anyhow::Error) -> Result<AgentResponse> {
        let error_string = error.to_string().to_lowercase();
        
        // Если LLM недоступен, пробуем локальную обработку
        if error_string.contains("llm") || error_string.contains("openai") || error_string.contains("anthropic") {
    ...
    async fn handle_llm_fallback(&self, context: &RequestContext) -> Result<AgentResponse> {
        debug!("SmartFallbackStrategy: LLM fallback для '{}'", context.message);
        ...
        if message_lower.contains("статус") || message_lower.contains("status") {
            return Ok(AgentResponse::Chat(
                "LLM недоступен. Статус системы можно проверить командой 'health check'.".to_string()
            ));
        }
    ...
    async fn handle_tools_fallback(&self, context: &RequestContext) -> Result<AgentResponse> {
        debug!("SmartFallbackStrategy: Tools fallback для '{}'", context.message);
        ...
        if message_lower.contains("git") {
            return Ok(AgentResponse::ToolExecution(
                "Git команда не выполнена. Проверьте, что находитесь в git репозитории.".to_string()
            ));
        }
    ...
    async fn handle_memory_fallback(&self, context: &RequestContext) -> Result<AgentResponse> {
        debug!("SmartFallbackStrategy: Memory fallback для '{}'", context.message);
    ...
    L: LlmServiceTrait,
{
    async fn handle_fallback(&self, context: &RequestContext, error: &anyhow::Error) -> Result<AgentResponse> {
        ...
        match self.try_alternative_approach(context, error).await {
            ...
            Err(fallback_error) => {
                error!("SmartFallbackStrategy: альтернативный подход тоже не сработал: {}", fallback_error);
                
                // Последняя попытка через LLM если доступен
                if let Some(ref llm) = self.llm_service {
                    match llm.chat(&format!("Пользователь пытался: {}. Произошла ошибка: {}. Как лучше ответить?", 
                                          context.message, error)).await {
                        Ok(llm_response) => {
...
impl FallbackStrategy for CircuitBreakerFallbackStrategy {
    async fn handle_fallback(&self, context: &RequestContext, error: &anyhow::Error) -> Result<AgentResponse> {
        ...
        if error_string.contains("circuit") && error_string.contains("open") {
            info!("CircuitBreakerFallbackStrategy: circuit breaker открыт, возвращаем быстрый ответ");
...
impl FallbackStrategy for CompositeFallbackStrategy {
    async fn handle_fallback(&self, context: &RequestContext, error: &anyhow::Error) -> Result<AgentResponse> {
        debug!("CompositeFallbackStrategy: пробуем {} стратегий", self.strategies.len());
        
        for (i, strategy) in self.strategies.iter().enumerate() {
            if strategy.can_handle(error) {
                debug!("CompositeFallbackStrategy: пробуем стратегию {} (приоритет {})", 
                       i, strategy.priority());
                
                match strategy.handle_fallback(context, error).await {
                    Ok(response) => {
                        info!("CompositeFallbackStrategy: стратегия {} успешно обработала ошибку", i);
                        return Ok(response);
                    }
                    Err(strategy_error) => {
                        warn!("CompositeFallbackStrategy: стратегия {} не смогла обработать: {}", 
                              i, strategy_error);
                        continue;
                    }
                }
