...
fn cli_memory_backup_creates_file() {
    ...
    cmd.current_dir(&temp)
        ...
        .env(
            "ORT_DYLIB_PATH",
            "scripts/onnxruntime/lib/libonnxruntime.so",
        )
...
async fn api_backup_and_restore_roundtrip() {
    use memory::api::{MemoryServiceTrait, UnifiedMemoryAPI};
    ...
    let _ = api.remember(
        ...
        memory::api::MemoryContext::new("test").with_layer(Layer::Interact),
    );
    let _ = api.remember(
