...
use cli::services::resilience::{
    DefaultResilienceService, ResilienceService, ResilienceStats, RetryConfig,
};
...
async fn test_operation_succeeds_after_retries() -> Result<()> {
    ...
    let mock_op =
        Arc::new(MockOperation::new("success after retries".to_string()).with_failure_count(2)); // Fail первые 2 попытки, succeed на 3-й
    let mock_op_clone = mock_op.clone();
...
async fn test_operation_fails_after_max_retries() -> Result<()> {
    ...
    let mock_op = Arc::new(MockOperation::new("never succeeds".to_string()).with_always_fail());
    let mock_op_clone = mock_op.clone();
...
async fn test_operation_timing() -> Result<()> {
    ...
    let mock_op = Arc::new(
        MockOperation::new("timed operation".to_string())
...
async fn test_custom_retry_config() -> Result<()> {
    ...
    let mock_op =
        Arc::new(MockOperation::new("custom config test".to_string()).with_failure_count(3)); // Fail 3 times, succeed on 4th
    let mock_op_clone = mock_op.clone();
...
async fn test_single_attempt_config() -> Result<()> {
    ...
    let mock_op = Arc::new(MockOperation::new("single attempt".to_string()).with_always_fail());
    let mock_op_clone = mock_op.clone();
...
async fn test_zero_delay_config() -> Result<()> {
    ...
    let mock_op = Arc::new(MockOperation::new("zero delay test".to_string()).with_failure_count(1));
    let mock_op_clone = mock_op.clone();
...
async fn test_exponential_backoff_timing() -> Result<()> {
    ...
    let mock_op = Arc::new(MockOperation::new("backoff test".to_string()).with_failure_count(3)); // Fail 3 times, succeed on 4th
    let mock_op_clone = mock_op.clone();
...
async fn test_operation_result_fields() -> Result<()> {
    ...
    let mock_op = Arc::new(MockOperation::new("field test".to_string()).with_failure_count(1));
    let mock_op_clone = mock_op.clone();
