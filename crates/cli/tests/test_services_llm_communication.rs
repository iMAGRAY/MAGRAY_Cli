...
use cli::services::llm_communication::{
    LlmCommunicationService, DefaultLlmCommunicationService, LlmCommunicationConfig,
    ChatOptions, LlmUsageStats, LlmHealthStatus, ProviderInfo, RateLimits
};
use cli::services::types::{RequestContext, OperationResult};
...
impl DefaultLlmCommunicationService {
    ...
    #[cfg(test)]
    pub fn with_config_for_testing(mock_client: MockLlmClient, config: LlmCommunicationConfig) -> Self {
        Self {
            llm_client: TestLlmClientWrapper::new(mock_client),
            stats: parking_lot::RwLock::new(LlmUsageStats::default()),
            response_cache: parking_lot::RwLock::new(HashMap::new()),
...
async fn test_timeout_handling() -> Result<()> {
    let mock_client = MockLlmClient::new()
        .with_delay(Duration::from_secs(2)); // Дольше timeout'а
...
async fn test_usage_stats_tracking() -> Result<()> {
    ...
    let options = ChatOptions { use_cache: true, ..Default::default() };
...
fn test_custom_config() {
    ...
    let mock_client = MockLlmClient::new();
    let service = DefaultLlmCommunicationService::with_config_for_testing(mock_client, custom_config);
