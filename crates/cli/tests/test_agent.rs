use cli::agent::{UnifiedAgent, AgentResponse};
use tokio;
...
async fn test_agent_complex_message() {
    ...
    if let Ok(agent) = agent_result {
        let response = agent.process_message("analyze the current system status and provide recommendations").await;
...
async fn test_agent_error_recovery() {
    ...
    if let Ok(agent) = agent_result {
        ...
        let _ = agent.process_message("invalid query that might cause error").await;
