...
fn env_policy_blocks_web_search_with_keyword_secret() {
    ...
    let mut cmd = Command::cargo_bin("magray").expect("binary built");
    cmd.args(["tools","run","--name","web_search","--command","search","--arg","query=find secret docs"])
...
fn env_policy_ask_memory_backup_noninteractive_fails_autoapprove_passes() {
    ...
    let mut cmd = Command::cargo_bin("magray").expect("binary built");
    cmd.args(["memory","backup"]).env("CI","1").env("MAGRAY_CMD_TIMEOUT","20")
        .env("MAGRAY_NONINTERACTIVE","true").env("MAGRAY_POLICY_JSON", ask_backup);
    let status = cmd.status().expect("run ok");
    assert!(!status.success());

    // Auto-approve should pass
    let mut cmd2 = Command::cargo_bin("magray").expect("binary built");
    cmd2.args(["memory","backup"]).env("CI","1").env("MAGRAY_CMD_TIMEOUT","20")
        .env("MAGRAY_AUTO_APPROVE_ASK","true").env("MAGRAY_POLICY_JSON", ask_backup);
...
fn env_policy_ask_memory_restore_noninteractive_fails_autoapprove_passes() {
    ...
    // Non-interactive should fail
    let mut cmd = Command::cargo_bin("magray").expect("binary built");
    cmd.args(["memory","restore","/dev/null"]).env("CI","1").env("MAGRAY_CMD_TIMEOUT","20")
        .env("MAGRAY_NONINTERACTIVE","true").env("MAGRAY_POLICY_JSON", ask_restore);
    let status = cmd.status().expect("run ok");
    assert!(!status.success());

    // Auto-approve should pass
    let mut cmd2 = Command::cargo_bin("magray").expect("binary built");
    cmd2.args(["memory","restore","/dev/null"]).env("CI","1").env("MAGRAY_CMD_TIMEOUT","20")
        .env("MAGRAY_AUTO_APPROVE_ASK","true").env("MAGRAY_POLICY_JSON", ask_restore);
    let status2 = cmd2.status().expect("run ok");
    assert!(status2.success());
}