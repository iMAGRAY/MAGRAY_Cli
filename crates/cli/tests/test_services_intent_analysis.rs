...
use cli::services::intent_analysis::{
    DefaultIntentAnalysisService, IntentAnalysisConfig, IntentAnalysisService, IntentAnalysisStats,
};
...
impl MockIntentAnalyzerAgent {
    pub fn new() -> Self {
        ...
        responses.insert("error_test".to_string(), Err(anyhow!("Simulated ML error")));
    ...
    pub async fn analyze_intent(&self, message: &str) -> Result<MockIntentDecision> {
        ...
        self.responses.get(message).cloned().unwrap_or_else(|| {
            Ok(MockIntentDecision {
...
async fn test_analyze_intent_timeout_fallback() -> Result<()> {
    let mock = MockIntentAnalyzerAgent::new().with_delay(Duration::from_secs(15)); // Дольше timeout'а
...
async fn test_confidence_distribution_tracking() -> Result<()> {
    ...
    service
        .analyze_intent(&create_test_context("medium"))
        .await?;
    ...
    assert_eq!(
        *stats.confidence_distribution.get("medium").unwrap_or(&0),
        1
    );
...
async fn test_stats_reset() -> Result<()> {
    ...
    service
        .analyze_intent(&create_test_context("тест 1"))
        .await?;
    service
        .analyze_intent(&create_test_context("тест 2"))
        .await?;
...
async fn test_average_response_time_calculation() -> Result<()> {
    let mock = MockIntentAnalyzerAgent::new().with_delay(Duration::from_millis(50));
