...
use cli::services::request_routing::{
    DefaultRequestRoutingService, RequestRoutingService, ResourceRequirements, RouteType,
    RoutingConfig, RoutingRecommendation, RoutingStats,
};
use cli::services::types::{AgentResponse, IntentDecision, OperationResult, RequestContext};
...
async fn test_timeout_handling() -> Result<()> {
    let mock_router = MockSmartRouter::new().with_delay(Duration::from_secs(5)); // Дольше timeout'а
...
async fn test_recommend_routing() -> Result<()> {
    ...
    assert_eq!(
        recommendation.resource_requirements.estimated_memory_mb,
        100
    );
...
fn test_config_defaults() {
    ...
    assert!(config
        .blocked_patterns
        .contains(&"delete system32".to_string()));
}
