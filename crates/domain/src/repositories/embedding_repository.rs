...
pub trait EmbeddingRepository: Send + Sync {
    ...
    async fn store_embedding(&self, record_id: RecordId, embedding: EmbeddingVector) -> DomainResult<()>;
    ...
    async fn store_embeddings_batch(&self, embeddings: Vec<(RecordId, EmbeddingVector)>) -> DomainResult<()>;
    
    /// Delete embedding by record ID
    async fn delete_embedding(&self, record_id: RecordId) -> DomainResult<bool>;
    
    /// Update existing embedding
    async fn update_embedding(&self, record_id: RecordId, embedding: EmbeddingVector) -> DomainResult<()>;
