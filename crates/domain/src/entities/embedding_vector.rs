...
impl EmbeddingVector {
    ...
    pub fn new(dimensions: Vec<f32>, expected_dimensions: EmbeddingDimensions) -> DomainResult<Self> {
        ...
        if dimensions.is_empty() {
            return Err(DomainError::InvalidEmbeddingVector("Vector cannot be empty".to_string()));
        }
        
        // Business rule: vectors should be normalized
        if !Self::is_normalized_static(&dimensions) {
            return Err(DomainError::InvalidEmbeddingVector("Vector must be normalized".to_string()));
        }
    ...
    pub fn from_raw(dimensions: Vec<f32>, expected_dimensions: EmbeddingDimensions) -> DomainResult<Self> {
        if dimensions.len() != expected_dimensions {
            return Err(DomainError::EmbeddingDimensionMismatch {
                expected: expected_dimensions,
                actual: dimensions.len(),
    ...
    pub fn cosine_similarity(&self, other: &EmbeddingVector) -> DomainResult<SimilarityScore> {
        ...
        let dot_product: f32 = self.dimensions
            .iter()
    ...
    pub fn l2_distance(&self, other: &EmbeddingVector) -> DomainResult<f32> {
        ...
        let sum_squared: f32 = self.dimensions
            .iter()
            .zip(&other.dimensions)
            .map(|(a, b)| (a - b).powi(2))
            .sum();
    ...
    pub fn normalize(&mut self) -> DomainResult<()> {
        ...
        if magnitude == 0.0 {
            return Err(DomainError::InvalidEmbeddingVector("Cannot normalize zero vector".to_string()));
        }
