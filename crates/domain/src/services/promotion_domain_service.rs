    ...
    M: MemoryRepository,
{
    ...
    pub async fn analyze_promotion_candidates(&self, from_layer: LayerType) -> DomainResult<PromotionAnalysis> {
    ...
    pub async fn execute_batch_promotion(
        ...
        let target_layer = from_layer.next_layer()
    ...
    pub async fn get_promotion_statistics(&self) -> DomainResult<PromotionStatistics> {
        ...
        let interact_candidates = self.memory_repo.find_promotion_candidates(LayerType::Interact).await?;
        let insights_candidates = self.memory_repo.find_promotion_candidates(LayerType::Insights).await?;
    ...
    fn meets_promotion_criteria(&self, record: &MemoryRecord, target_layer: LayerType) -> DomainResult<bool> {
    ...
    fn calculate_promotion_confidence(&self, record: &MemoryRecord) -> f32 {
        ...
        let recency_boost = if access_pattern.hours_since_last_access() < 24 { 0.2 } else { 0.0 };
        let acceleration_boost = if access_pattern.is_accelerating() { 0.1 } else { 0.0 };
...
impl PromotionAnalysis {
    ...
    pub fn recommended_count(&self) -> usize {
        self.recommendations.iter().filter(|r| r.recommended).count()
    }
}
...
pub trait PromotionDomainServiceTrait: Send + Sync {
    async fn analyze_promotion_candidates(&self, from_layer: LayerType) -> DomainResult<PromotionAnalysis>;
