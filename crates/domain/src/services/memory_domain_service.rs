...
use std::sync::Arc;
use crate::entities::{MemoryRecord, RecordId, EmbeddingVector};
use crate::value_objects::LayerType;
use crate::repositories::{MemoryRepository, EmbeddingRepository};
    ...
    E: EmbeddingRepository,
{
    ...
    pub async fn store_record_with_embedding(
        ...
        // Store embedding
        if let Err(e) = self.embedding_repo.store_embedding(record_id, embedding).await {
    ...
    pub async fn update_record_content(
        ...
        // Get existing record
        let mut record = self.memory_repo.find_by_id(record_id).await?
        ...
        self.embedding_repo.update_embedding(record_id, new_embedding).await?;
    ...
    pub async fn store_batch_with_embeddings(
        ...
        let records: Vec<_> = records_with_embeddings.iter().map(|(r, _)| r.clone()).collect();
        ...
        let embedding_pairs: Vec<_> = record_ids.iter()
            .zip(embeddings.into_iter())
            .map(|(&id, (_, embedding))| (id, embedding))
            .collect();
        
        if let Err(e) = self.embedding_repo.store_embeddings_batch(embedding_pairs).await {
    ...
    pub async fn get_promotion_candidates(&self, from_layer: LayerType) -> DomainResult<Vec<MemoryRecord>> {
        let candidates = self.memory_repo.find_promotion_candidates(from_layer).await?;
    ...
    fn validate_embedding_for_record(&self, embedding: &EmbeddingVector) -> DomainResult<()> {
        ...
        if !embedding.is_valid_for_storage() {
            return Err(DomainError::InvalidEmbeddingVector("Invalid vector values".to_string()));
        }
...
pub trait MemoryDomainServiceTrait: Send + Sync {
    ...
    async fn get_promotion_candidates(&self, from_layer: LayerType) -> DomainResult<Vec<MemoryRecord>>;
}
