...
impl RetryManager {
        ...
        E: std::fmt::Display + std::fmt::Debug,
    {
        ...
        for attempt in 1..=self.config.max_attempts {
            debug!("Attempting {} (attempt {}/{})", operation_name, attempt, self.config.max_attempts);
            ...
            match operation().await {
                ...
                Err(err) => {
                    warn!("Operation {} failed on attempt {}: {}", operation_name, attempt, err);
    ...
    pub fn is_retriable_error(error: &anyhow::Error) -> bool {
        ...
        if error_str.contains("hnsw не инициализирован") || error_str.contains("hnsw not initialized") {
            return true;
        }
