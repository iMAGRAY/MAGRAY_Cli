...
/// Оптимизированный promotion engine с time-based индексированием
pub struct OptimizedPromotionEngine {
...
impl OptimizedPromotionEngine {
    pub async fn new(store: Arc<VectorStore>, config: PromotionConfig, db: Arc<Db>) -> Result<Self> {
        info!("🚀 Инициализация OptimizedPromotionEngine с time-based индексами");
    ...
    pub async fn run_optimized_promotion_cycle(&self) -> Result<OptimizedPromotionStats> {
        ...
        let mut stats = OptimizedPromotionStats::default();
        
        info!("🔄 Запуск оптимизированного promotion цикла");
        ...
        stats.interact_to_insights = self.promote_interact_to_insights_optimized().await?;
        stats.insights_to_assets = self.promote_insights_to_assets_optimized().await?;
        ...
        stats.expired_interact = self.expire_records_optimized(Layer::Interact).await?;
        stats.expired_insights = self.expire_records_optimized(Layer::Insights).await?;
        stats.cleanup_time_ms = cleanup_time.elapsed().as_millis() as u64;
        
        stats.total_time_ms = start_time.elapsed().as_millis() as u64;
        
        info!("✅ Оптимизированный promotion цикл завершен за {}ms", stats.total_time_ms);
    ...
    async fn promote_interact_to_insights_optimized(&self) -> Result<usize> {
        let now = Utc::now();
        ...
        let candidates = self.find_candidates_by_time_optimized(
            Layer::Interact,
            threshold_time,
        ...
        if count > 0 {
            info!("🔄 Продвижение {} записей: Interact -> Insights (оптимизировано)", count);
    ...
    async fn promote_insights_to_assets_optimized(&self) -> Result<usize> {
        let now = Utc::now();
        ...
        let count = candidates.len();
        if count > 0 {
            info!("🔄 Продвижение {} записей: Insights -> Assets (оптимизировано)", count);
    ...
    async fn expire_records_optimized(&self, layer: Layer) -> Result<usize> {
        ...
        let expired = self.find_expired_records_optimized(layer, expiry_time).await?;
        let count = expired.len();
        
        if count > 0 {
            info!("🗑️ Удаление {} устаревших записей из {:?} (оптимизировано)", count, layer);
    ...
    async fn find_candidates_by_time_optimized(
        &self,
        layer: Layer,
        before: DateTime<Utc>,
        min_score: f32,
    ...
    async fn find_expired_records_optimized(
        &self,
        layer: Layer,
        before: DateTime<Utc>,
...
pub struct OptimizedPromotionStats {
