...
impl BatchOptimizedStats {
    pub fn throughput_qps(&self) -> f64 {
        ...
        let total_time_s = self.total_processing_time_ns.load(Ordering::Relaxed) as f64 / 1_000_000_000.0;
        if total_time_s > 0.0 {
    ...
    pub fn avg_latency_ms(&self) -> f64 {
        let total_batches = self.total_batches_processed.load(Ordering::Relaxed);
        if total_batches > 0 {
            let total_time_ms = self.total_processing_time_ns.load(Ordering::Relaxed) as f64 / 1_000_000.0;
...
impl AdaptiveBatchingState {
    ...
    fn adjust_batch_size(&mut self, current_latency_ns: u64, _current_batch_size: usize) -> usize {
        ...
        if self.recent_latencies.len() >= 10 {
            let avg_latency = self.recent_latencies.iter().sum::<u64>() as f64 / self.recent_latencies.len() as f64;
            ...
            if avg_latency > target_latency_ns && self.optimal_batch_size > 32 {
                ...
                debug!("Adaptive batching: decreasing batch size to {} (avg latency: {:.2}ms)", 
                       self.optimal_batch_size, avg_latency / 1_000_000.0);
                ...
                self.last_adjustment_time = Instant::now();
                debug!("Adaptive batching: increasing batch size to {} (avg latency: {:.2}ms)", 
                       self.optimal_batch_size, avg_latency / 1_000_000.0);
            }
        }
...
impl BatchOptimizedProcessor {
    ...
    pub fn new(config: BatchOptimizedConfig) -> Result<Self> {
        ...
        let hot_vectors = Arc::new(RwLock::new(AlignedBatchVectors::with_capacity(config.max_batch_size * 2)));
        let memory_pool = Arc::new(Mutex::new(Vec::new()));
        let adaptive_state = Arc::new(Mutex::new(AdaptiveBatchingState::new(config.max_batch_size / 2)));
        ...
        info!("✅ Ultra-optimized batch processor initialized with {} worker threads", processor.config.worker_threads);
    ...
    fn start_workers(&mut self) -> Result<()> {
        for worker_id in 0..self.config.worker_threads {
            ...
            let handle = tokio::task::spawn(async move {
                Self::worker_loop(worker_id, request_rx, stats, hot_vectors, adaptive_state, config).await;
            });
            
            self.worker_handles.push(handle);
        }
        
        info!("✅ Started {} worker threads for batch processing", self.config.worker_threads);
        Ok(())
    }
    ...
    async fn worker_loop(
        ...
        loop {
            ...
            let processed_count = Self::process_request_batch(
                requests, 
                &hot_vectors, 
                &stats, 
                &config
            ...
            stats.total_vectors_processed.fetch_add(processed_count as u64, Ordering::Relaxed);
            stats.total_processing_time_ns.fetch_add(processing_time_ns, Ordering::Relaxed);
            ...
            if config.adaptive_batching {
                ...
                if new_optimal_size != adaptive.optimal_batch_size {
                    stats.adaptive_batch_adjustments.fetch_add(1, Ordering::Relaxed);
                }
            }
            ...
            if batch_size > 0 {
                ...
                if latency_ms > 5.0 {
                    warn!("Worker {}: High latency batch processing: {:.2}ms for {} requests", 
                          worker_id, latency_ms, batch_size);
                } else {
                    debug!("Worker {}: Processed batch of {} requests in {:.2}ms", 
    ...
    async fn process_request_batch(
        ...
        if !insert_requests.is_empty() {
            processed_count += Self::process_insert_batch(insert_requests, hot_vectors, stats).await;
        }
        
        if !search_requests.is_empty() {
            processed_count += Self::process_search_batch(search_requests, hot_vectors, stats).await;
        }
        
        if !batch_search_requests.is_empty() {
            processed_count += Self::process_batch_search_batch(batch_search_requests, hot_vectors, stats).await;
        }
    ...
    async fn process_insert_batch(
        ...
        for request in requests {
            if let BatchRequest::Insert { records, response_tx } = request {
                ...
                stats.simd_operations.fetch_add(record_count as u64, Ordering::Relaxed);
            }
        }
    ...
    async fn process_search_batch(
        ...
        for request in requests {
            if let BatchRequest::Search { query, k, layer, response_tx } = request {
                requests_by_layer.entry(layer).or_default().push((query, k, response_tx));
            }
        }
    ...
    async fn search_in_layer_simd(
        ...
        let mut results: Vec<_> = layer_vectors.iter()
            .zip(distances.iter())
        ...
        stats.simd_operations.fetch_add(layer_vectors.len() as u64, Ordering::Relaxed);
    ...
    async fn search_all_layers_simd(
        ...
        let all_vectors: Vec<_> = hot_vectors.vectors.iter()
        ...
        // Сортируем и возвращаем top-k
        let mut results: Vec<_> = all_vectors.iter()
            .zip(distances.iter())
            .map(|((_, id, _), &distance)| (id.to_string(), distance))
            .collect();
        ...
        stats.simd_operations.fetch_add(all_vectors.len() as u64, Ordering::Relaxed);
        
        results
    }
    
    /// Ultra-optimized SIMD batch distance calculation
    fn batch_cosine_distances_simd(
        query: &[f32], 
        vectors: &[(&Vec<f32>, &str, f32)]
    ) -> Vec<f32> {
            ...
            if is_x86_feature_detected!("avx2") && query.len() % 8 == 0 {
                ...
                for (vector, _, _) in vectors {
                    if vector.len() == query.len() {
                        let distance = unsafe { 
                            cosine_distance_avx2_ultra(query, vector) 
                        };
    ...
    async fn process_batch_search_batch(
        ...
        for request in requests {
            if let BatchRequest::BatchSearch { queries, k, layer, response_tx } = request {
                let hot_vecs = hot_vectors.read().await;
    ...
    pub async fn search(&self, query: Vec<f32>, k: usize, layer: Option<Layer>) -> Result<Vec<(String, f32)>> {
        let (response_tx, response_rx) = tokio::sync::oneshot::channel();
    ...
    pub async fn batch_search(&self, queries: Vec<Vec<f32>>, k: usize, layer: Option<Layer>) -> Result<Vec<Vec<(String, f32)>>> {
        let (response_tx, response_rx) = tokio::sync::oneshot::channel();
    ...
    pub fn get_stats(&self) -> BatchOptimizedStats {
        BatchOptimizedStats {
            ...
            total_vectors_processed: AtomicU64::new(self.stats.total_vectors_processed.load(Ordering::Relaxed)),
            total_processing_time_ns: AtomicU64::new(self.stats.total_processing_time_ns.load(Ordering::Relaxed)),
            ...
            adaptive_batch_adjustments: AtomicU64::new(self.stats.adaptive_batch_adjustments.load(Ordering::Relaxed)),
        }
    }
...
mod tests {
    ...
    async fn test_batch_optimized_processor_creation() {
        ...
        let processor = BatchOptimizedProcessor::new(config).unwrap();
    ...
    async fn test_batch_insert_and_search() {
        ...
        // Insert batch
        let response = processor.insert_batch(records).await.unwrap();
        assert_eq!(response.processed_count, 10);
        assert!(response.used_simd);
        
        // Search
        let query = vec![0.05; 1024]; // Query похожий на первые vectors
        let results = processor.search(query, 5, Some(Layer::Interact)).await.unwrap();
    ...
    async fn test_batch_search_multiple_queries() {
        ...
        processor.insert_batch(records).await.unwrap();
        ...
        let results = processor.batch_search(queries, 2, None).await.unwrap();
    ...
    fn test_adaptive_batching_state() {
        ...
        assert!(new_size < 256, "Should decrease batch size for high latency");
