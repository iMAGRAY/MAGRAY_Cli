...
use crate::{storage::VectorStore, types::Layer};
...
impl LifecycleManager {
        ...
        Fut: std::future::Future<Output = Result<()>>,
    {
...

        {
            ...
            if *state != LifecycleState::Uninitialized {
                return Err(anyhow::anyhow!(
                    "Сервис уже инициализируется или инициализирован"
                ));
            }
        ...
        let init_result = timeout(self.config.initialization_timeout, init_fn()).await;
    ...
    pub async fn initialize_memory_layers(&self, store: Arc<VectorStore>) -> Result<()> {
        ...
        for layer in [Layer::Interact, Layer::Insights, Layer::Assets] {
            let layer_result = timeout(Duration::from_secs(30), store.init_layer(layer)).await;
        ...
        Fut: std::future::Future<Output = Result<()>>,
    {
        ...
        let shutdown_result = timeout(self.config.shutdown_timeout, shutdown_fn()).await;
    ...
    pub async fn get_state_description(&self) -> String {
        ...
        match state {
            ...
            LifecycleState::Initializing => {
                format!("🟡 ИНИЦИАЛИЗАЦИЯ ({:.1}s)", time_in_state.as_secs_f64())
            }
            LifecycleState::Ready => format!("🟢 ГОТОВ ({} активных операций)", active_ops),
            LifecycleState::Degraded => {
                format!("🟠 ДЕГРАДИРОВАН ({:.1}s)", time_in_state.as_secs_f64())
            }
            LifecycleState::ShuttingDown => {
                format!("🔶 ВЫКЛЮЧЕНИЕ ({} операций осталось)", active_ops)
            }
    ...
    async fn drain_active_operations(&self) -> Result<()> {
        ...
        if remaining_ops > 0 {
            warn!(
                "⚠️ Timeout ожидания операций - {} операций будут принудительно прерваны",
                remaining_ops
            );
            return Err(anyhow::anyhow!(
                "Не удалось дождаться завершения {} операций",
                remaining_ops
            ));
        }
...
mod tests {
    ...
    async fn test_lifecycle_failed_initialization() {
        ...
        let result = manager
            .initialize(|| async { Err(anyhow::anyhow!("Initialization failed")) })
            .await;
