...
use crate::{
    health::{HealthMonitor, HealthStatus, SystemHealthStatus},
...
impl HealthManager {
    ...
    pub async fn setup_production_monitoring(&self) -> Result<()> {
        ...
        self.register_component("vector_store", Duration::from_secs(30))
            .await;
        self.register_component("embedding_service", Duration::from_secs(30))
            .await;
        self.register_component("search_coordinator", Duration::from_secs(60))
            .await;
        self.register_component("cache_system", Duration::from_secs(60))
            .await;
        self.register_component("gpu_processor", Duration::from_secs(120))
            .await;
...
impl Coordinator for HealthManager {
    ...
    async fn metrics(&self) -> serde_json::Value {
        ...
        let warning_alerts = alerts
            .iter()
            .filter(|a| a.contains("Предупреждение"))
            .count();
    ...
    async fn health_check(&self) -> Result<()> {
        // Проверяем готовность HealthManager
        if !self.is_ready().await {
            return Err(anyhow::anyhow!("HealthManager не готов"));
        }

        // Проверяем состояние системы
        let system_health = self.health_monitor.overall_health().await?;
        if system_health.overall_status != crate::health::HealthStatus::Healthy {
            return Err(anyhow::anyhow!("Система в нездоровом состоянии"));
        }
        ...
        if !critical_alerts.is_empty() {
            return Err(anyhow::anyhow!(
                "Найдено {} критических алертов",
                critical_alerts.len()
            ));
        }

        // Проверяем SLA метрики
        let sla_metrics = self.sla_metrics.read().await;
        if sla_metrics.availability_percent < 95.0 {
            return Err(anyhow::anyhow!(
                "Доступность системы ниже порога: {:.2}%",
...
impl HealthCoordinator for HealthManager {
    ...
    async fn run_health_check(&self) -> Result<()> {
                        ...
                        if matches!(comp_status, HealthStatus::Healthy) {
                            ...
                            if comp_health.consecutive_failures >= 3 {
                                self.create_alert(
                                    ...
                                    AlertLevel::Critical,
                                    format!(
                                        "Компонент {} недоступен ({} последовательных ошибок)",
                                        comp_name, comp_health.consecutive_failures
                                    ),
                                )
                                .await;
        ...
        if metrics.avg_check_duration_ms == 0.0 {
            ...
            metrics.avg_check_duration_ms = alpha * check_duration.as_millis() as f64
                + (1.0 - alpha) * metrics.avg_check_duration_ms;
        }
...
impl HealthManager {
    ...
    async fn register_component(&self, name: &str, check_interval: Duration) {
        ...
        debug!(
            "📈 Зарегистрирован компонент '{}' с интервалом {:?}",
            name, check_interval
        );
    }
    ...
    async fn create_alert(&self, component: &str, level: AlertLevel, message: String) {
        ...
        warn!(
            "{} Новый алерт: {} - {}",
            level_emoji, alert.component, alert.message
        );
    }
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                if history.len() > 10 {
                    ...
                    sla_guard.availability_percent =
                        (healthy_checks as f64 / history.len() as f64) * 100.0;
                    ...
                    sla_guard.error_rate_percent =
                        (error_checks as f64 / history.len() as f64) * 100.0;

                    // Проверяем SLA нарушения
                    if sla_guard.availability_percent < 99.9 {
                        sla_guard.sla_violations += 1;
                        warn!(
                            "🚨 SLA нарушение: availability {:.2}%",
                            sla_guard.availability_percent
                        );
                    }
                    ...
                    if sla_guard.avg_response_time_ms > 1000.0 {
                        sla_guard.sla_violations += 1;
                        warn!(
                            "🚨 SLA нарушение: response time {:.2}ms",
                            sla_guard.avg_response_time_ms
                        );
                    }
                }
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                let critical_count = alerts
                    .iter()
                    .filter(|a| !a.resolved && matches!(a.level, AlertLevel::Critical))
                    .count();
                if critical_count > 0 {
                ...
                debug!(
                    "🔔 Alert processor: {} активных алертов",
...
impl std::fmt::Debug for HealthManager {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("HealthManager")
            ...
            .field("health_metrics", &"<RwLock<HealthMetrics>>")
            .field("sla_metrics", &"<RwLock<SlaMetrics>>")
            .field("active_alerts", &"<RwLock<Vec<Alert>>>")
            .finish()
    }
}
