...
impl Coordinator for ResourceController {
    async fn initialize(&self) -> Result<()> {
        ...
        info!(
            "📈 Начальное состояние: memory={}%, vectors={}/{}",
    ...
    async fn health_check(&self) -> Result<()> {
        ...
        if !self.is_ready().await {
            return Err(anyhow::anyhow!("ResourceController не готов"));
        }
        ...
        // Критичные пороги
        if usage.vector_usage_percent > 95.0 {
            return Err(anyhow::anyhow!(
                "Критический уровень vectors: {:.1}%",
                usage.vector_usage_percent
            ));
        }

        if usage.cache_usage_percent > 95.0 {
            return Err(anyhow::anyhow!(
                "Критический уровень кэша: {:.1}%",
                usage.cache_usage_percent
            ));
        }
        ...
        let critical_alerts = alerts
            ...
            .filter(|a| {
                matches!(
                    a.alert_type,
                    ResourceAlertType::ResourceExhaustion | ResourceAlertType::CacheOverflow
                )
            })
            .count();
        ...
        if critical_alerts > 0 {
            return Err(anyhow::anyhow!(
                "Найдено {} критических ресурсных алертов",
                critical_alerts
            ));
        }
...
impl ResourceCoordinator for ResourceController {
    ...
    async fn free_resources(&self) -> Result<()> {
        ...
        self.create_alert(
            ...
            format!(
                "Принудительно освобождено: {:.1}MB memory, {} vectors",
                freed_memory as f64 / 1024.0 / 1024.0,
                freed_vectors
            ),
        )
        .await;
...
impl ResourceController {
    ...
    async fn update_resource_metrics(&self, usage: &ResourceUsage) {
        ...
        metrics.avg_memory_usage =
            alpha * usage.cache_usage_percent + (1.0 - alpha) * metrics.avg_memory_usage;
        metrics.avg_vector_count =
            alpha * usage.current_vectors as f64 + (1.0 - alpha) * metrics.avg_vector_count;
    }
            ...
            loop {
                ...
                if usage.cache_usage_percent > 90.0 {
                    let alert = ResourceAlert {
                        ...
                        message: format!(
                            "Критическое использование памяти: {:.1}%",
                            usage.cache_usage_percent
                        ),
                        timestamp: Instant::now(),
                        resolved: false,
                    ...
                    alerts.push(alert);
                    warn!(
                        "🧠 Resource alert: Критическое использование памяти: {:.1}%",
                        usage.cache_usage_percent
                    );
                    ...
                    let alert = ResourceAlert {
                        ...
                        alert_type: ResourceAlertType::MemoryPressure,
                        message: format!(
                            "Высокое использование памяти: {:.1}%",
                            usage.cache_usage_percent
                        ),
                        timestamp: Instant::now(),
                        resolved: false,
                    ...
                    alerts.push(alert);
                    warn!(
                        "🧠 Resource alert: Высокое использование памяти: {:.1}%",
                        usage.cache_usage_percent
                    );
                }
                ...
                if usage.vector_usage_percent > 90.0 {
                    let alert = ResourceAlert {
                        ...
                        message: format!(
                            "Приближение к лимиту векторов: {}/{} ({:.1}%)",
                    ...
                    alerts.push(alert);
                    warn!(
                        "📊 Resource alert: Приближение к лимиту векторов: {}/{} ({:.1}%)",
                        usage.current_vectors, usage.max_vectors, usage.vector_usage_percent
                    );
                }

                debug!(
                    "📊 Resource monitoring: memory={:.1}%, vectors={}/{}",
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                if usage.cache_usage_percent > config.memory_scale_up_threshold {
                    ...
                    new_limits.cache_size_bytes =
                        (current_limits.cache_size_bytes as f64 * scale_factor) as usize;
                    scaling_needed = true;

                    info!(
                        "📈 Memory scale up: {:.1}% -> {}MB",
                        usage.cache_usage_percent,
                        new_limits.cache_size_bytes / 1024 / 1024
                    );
                ...
                if usage.vector_usage_percent > config.vector_scale_up_threshold {
                    ...
                    new_limits.max_vectors =
                        (current_limits.max_vectors as f64 * scale_factor) as usize;
                    scaling_needed = true;

                    info!(
                        "📈 Vector scale up: {:.1}% -> {} vectors",
                ...
                if scaling_needed {
                    ...
                    info!(
                        "✅ Auto-scaling завершён (событие #{})",
                        metrics.scaling_events
                    );
                }

                debug!(
                    "🎯 Auto-scaling check: memory={:.1}%, vectors={:.1}%",
    ...
    async fn start_predictive_scaling_loop(&self) {
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                let memory_values: Vec<f64> = recent_samples
                    .iter()
                    .map(|s| s.memory_usage_percent)
                    .collect();
                let vector_values: Vec<f64> = recent_samples
                    .iter()
                    .map(|s| s.vector_usage_percent)
                    .collect();
...

                {
                    ...
                    predictor.prediction_confidence =
                        if recent_samples.len() >= 10 { 0.8 } else { 0.5 };
                }

                // Если тренд показывает рост, можем заблаговременно увеличить лимиты
                if memory_trend > 2.0 || vector_trend > 2.0 {
                    info!(
                        "🔮 Predictive scaling: memory_trend={:.1}, vector_trend={:.1}",
                        memory_trend, vector_trend
                    );
                }

                debug!(
                    "🔮 Predictive analysis: memory_trend={:.1}, vector_trend={:.1}",
                    memory_trend, vector_trend
                );
            }
        });
