...
impl OrchestrationLifecycleManager {
    ...
    pub fn from_container(container: &crate::di::container_core::ContainerCore) -> Result<Self> {
        let coordinators = CoordinatorRegistry::from_container(container)?;
        Ok(Self::new(coordinators))
    }
    ...
    async fn initialize_coordinator_group(
        ...
        info!("🚀 {}: Инициализация {} координаторов", phase_name, coordinators.len());
        ...
        for (name, coordinator) in coordinators {
            ...
            let init_result = match result {
                Ok(Ok(())) => {
                    info!("✅ {} coordinator инициализирован за {:?}", coordinator_name, duration);
                ...
                Ok(Err(e)) => {
                    error!("❌ {} coordinator ошибка инициализации: {}", coordinator_name, e);
                    InitializationResult {
                        coordinator_name,
                        success: false,
                        duration,
        ...
        if successful == total {
            info!("✅ {}: Все {} координаторов инициализированы успешно", phase_name, total);
        } else {
            let failed_coordinators: Vec<&str> = results.iter()
                .filter(|r| !r.success)
                .map(|r| r.coordinator_name.as_str())
                .collect();
            return Err(anyhow::anyhow!(
                "{}: Ошибка инициализации координаторов: {:?}", 
                phase_name, failed_coordinators
            ));
        }
        ...
        let health_task = {
            ...
            tokio::spawn(async move {
                ...
                while !emergency_shutdown.load(Ordering::Relaxed) {
                    ...
                    if let Err(e) = crate::orchestration::traits::HealthCoordinator::run_health_check(health.as_ref()).await {
...
impl LifecycleManager for OrchestrationLifecycleManager {
    async fn initialize_production(&self) -> Result<()> {
        ...
        info!("🎉 OrchestrationLifecycleManager успешно инициализирован за {:?}", 
    ...
    async fn shutdown_all(&self) -> Result<()> {
        ...
        let shutdown_sequence = vec![
            ("backup", &*self.coordinators.backup as &dyn Coordinator, Duration::from_secs(60)),
            ("promotion", &*self.coordinators.promotion as &dyn Coordinator, Duration::from_secs(30)),
            ("search", &*self.coordinators.search as &dyn Coordinator, Duration::from_secs(15)),
            ("embedding", &*self.coordinators.embedding as &dyn Coordinator, Duration::from_secs(30)),
            ("health", &*self.coordinators.health as &dyn Coordinator, Duration::from_secs(15)),
            ("resources", &*self.coordinators.resources as &dyn Coordinator, Duration::from_secs(15)),
        ];
        ...
        info!("🏁 OrchestrationLifecycleManager успешно остановлен за {:?}", 
            self.start_time.elapsed());
        Ok(())
    }
    ...
    async fn emergency_shutdown(&self) -> Result<()> {
        ...
        let emergency_timeouts = vec![
            ...
            timeout(Duration::from_secs(3), self.coordinators.promotion.shutdown()),
            timeout(Duration::from_secs(2), self.coordinators.search.shutdown()),
            timeout(Duration::from_secs(3), self.coordinators.embedding.shutdown()),
            timeout(Duration::from_secs(2), self.coordinators.health.shutdown()),
            timeout(Duration::from_secs(2), self.coordinators.resources.shutdown())
        ];
