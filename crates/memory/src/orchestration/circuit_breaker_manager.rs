...
impl CircuitBreakerState {
    ...
    fn record_failure(&mut self) {
        ...
        if failures >= self.failure_threshold {
            ...
            debug!("Circuit breaker: {} ошибок из {} допустимых", failures, self.failure_threshold);
        }
    }
...
impl CircuitBreakerManager {
    ...
    pub async fn initialize_standard_coordinators(&self) -> Result<()> {
        ...
        breakers.insert("embedding".to_string(), CircuitBreakerState::new(&CircuitBreakerConfig::standard()));
        breakers.insert("search".to_string(), CircuitBreakerState::new(&CircuitBreakerConfig::fast()));
        breakers.insert("health".to_string(), CircuitBreakerState::new(&CircuitBreakerConfig::standard()));
        breakers.insert("promotion".to_string(), CircuitBreakerState::new(&CircuitBreakerConfig::critical()));
        breakers.insert("resources".to_string(), CircuitBreakerState::new(&CircuitBreakerConfig::standard()));
        breakers.insert("backup".to_string(), CircuitBreakerState::new(&CircuitBreakerConfig::critical()));
    ...
    pub async fn add_component(&self, component: &str, config: Option<CircuitBreakerConfig>) -> Result<()> {
        let config = config.unwrap_or_else(|| self.default_config.clone());
        let mut breakers = self.breakers.write().await;
        
        if breakers.contains_key(component) {
            warn!("Circuit breaker для {} уже существует, пропускаем", component);
            return Ok(());
        }
    ...
    async fn ensure_component_exists(&self, component: &str) {
        ...
        if !breakers.contains_key(component) {
            ...
            if let Err(e) = self.add_component(component, None).await {
                warn!("Не удалось создать circuit breaker для {}: {}", component, e);
            }
        }
    }
}
...
impl CircuitBreakerManagerTrait for CircuitBreakerManager {
    async fn can_execute(&self, component: &str) -> bool {
        ...
        if let Some(breaker) = breakers.get_mut(component) {
            ...
            warn!("Circuit breaker для {} не найден, разрешаем выполнение", component);
            true
        }
    }
...
mod tests {
    ...
    async fn test_circuit_breaker_lifecycle() {
        ...
        manager.initialize_standard_coordinators().await.unwrap();
        ...
        assert_eq!(manager.get_status(component).await, Some(CircuitBreakerStatus::Closed));
        ...
        assert_eq!(manager.get_status(component).await, Some(CircuitBreakerStatus::Open));
        ...
        assert!(manager.can_execute(component).await);
        assert_eq!(manager.get_status(component).await, Some(CircuitBreakerStatus::HalfOpen));
    ...
    async fn test_circuit_breaker_statistics() {
        ...
        manager.add_component(component, None).await.unwrap();
        ...
        let component_stats = stats.get(component).unwrap();
    ...
    async fn test_reset_all_circuit_breakers() {
        ...
        assert_eq!(manager.get_status("search").await, Some(CircuitBreakerStatus::Closed));
        assert_eq!(manager.get_status("embedding").await, Some(CircuitBreakerStatus::Closed));
    }
    ...
    async fn test_different_configurations() {
        ...
        manager.add_component("fast", Some(CircuitBreakerConfig::fast())).await.unwrap();
        manager.add_component("critical", Some(CircuitBreakerConfig::critical())).await.unwrap();
