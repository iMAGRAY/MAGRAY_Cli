...
use crate::{
    orchestration::{
        ...
        orchestration_lifecycle_manager::{OrchestrationLifecycleManager, LifecycleManager},
        operation_executor::{OperationExecutor, OperationExecutorTrait},
        metrics_collector::{MetricsCollector, MetricsCollectorTrait},
        coordinator_registry::{CoordinatorRegistry, CoordinatorRegistryTrait, CoordinatorRegistryFactory},
    },
...
impl OrchestrationFacade {
    ...
    pub async fn from_container(container: &crate::di::container_core::ContainerCore) -> Result<Self> {
        ...
        circuit_breaker_manager.initialize_standard_coordinators().await?;
        
        // Создаем coordinator registry
        let coordinator_registry = Arc::new(CoordinatorRegistryFactory::from_container(container)?);
        ...
        let operation_executor = Arc::new(OperationExecutor::from_container(container, Arc::clone(&circuit_breaker_manager))?);
        
        // Создаем metrics collector
        let metrics_collector = Arc::new(MetricsCollector::from_container(container, Arc::clone(&circuit_breaker_manager))?);
    ...
    pub async fn initialize_all(&self) -> Result<()> {
        tracing::warn!("⚠️ Использование legacy initialize_all(), рекомендуется initialize_production()");
        self.lifecycle_manager.initialize_production().await
    }
    ...
    pub async fn production_health_check(&self) -> Result<SystemHealthStatus> {
        ...
        Ok(SystemHealthStatus {
            overall_status: if readiness_status.all_ready {
    ...
    pub async fn adaptive_optimization(&self) -> Result<()> {
        ...
        if !result.actions_taken.is_empty() {
            tracing::info!("🎯 Адаптивная оптимизация выполнена: {:?}", result.actions_taken);
        }
    ...
    pub async fn get_detailed_readiness(&self) -> crate::orchestration::coordinator_registry::ReadinessStatus {
