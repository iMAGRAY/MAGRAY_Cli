...
use crate::{
    orchestration::{
        ...
        traits::{
            Coordinator, EmbeddingCoordinator as EmbeddingCoordinatorTrait,
...
impl SearchCoordinator {
    pub fn new(store: Arc<VectorStore>, embedding_coordinator: Arc<EmbeddingCoordinator>) -> Self {
...
impl Coordinator for SearchCoordinator {
    async fn initialize(&self) -> Result<()> {
        ...
        let store_ready = timeout(Duration::from_secs(10), self.test_vector_store()).await;
        ...
        if let Err(e) = self.initialize_rerank_model().await {
            info!(
                "⚠️ Reranking модель не загружена: {} (будет работать без reranking)",
                e
            );
        }
    ...
    async fn health_check(&self) -> Result<()> {
        ...
        if !self.is_ready().await {
            return Err(anyhow::anyhow!("SearchCoordinator не готов"));
        }

        // Проверяем vector store
        self.test_vector_store().await?;

        // Проверяем embedding coordinator
        let embedding_metrics = self.embedding_coordinator.metrics().await;
...
impl SearchCoordinatorTrait for SearchCoordinator {
    async fn search(
        ...
        match search_result.into_result() {
            Ok(results) => {
                ...
                self.record_success().await;
                self.update_performance_metrics(total_latency, embedding_latency, true)
                    .await;
                self.cache_result(&cache_key, &results, layer, options)
                    .await;
            ...
            Err(e) => {
                self.record_failure().await;
                self.update_performance_metrics(total_latency, embedding_latency, false)
                    .await;
    ...
    async fn vector_search(
        ...
        debug!(
            "📊 Векторный поиск в слое {:?}, размер вектора: {}",
            layer,
            vector.len()
        );
        ...
        match search_result.into_result() {
            Ok(results) => {
                self.record_success().await;
                self.update_performance_metrics(total_latency, 0.0, true)
                    .await; // Embedding latency = 0 для прямого векторного поиска
            ...
            Err(e) => {
                self.record_failure().await;
                self.update_performance_metrics(total_latency, 0.0, false)
                    .await;
    ...
    async fn search_with_rerank(
        ...
        debug!(
            "🎯 Поиск с reranking для '{}', rerank_top_k={}",
            query, rerank_top_k
        );
...
impl SearchCoordinator {
    ...
    async fn cache_result(
        ...
        if cache.cache.len() >= cache.max_size {
            ...
            let keys_to_remove: Vec<String> = cache.cache.keys().take(to_remove).cloned().collect();
        ...
        let ttl_multiplier = if results.len() < 5 {
            2.0
        ...
        match breaker.state {
            CircuitState::Closed => {
                if breaker.failure_count >= breaker.failure_threshold {
                    ...
                    error!(
                        "🚫 Search circuit breaker открыт после {} ошибок",
    ...
    async fn update_performance_metrics(
        ...
        if metrics.avg_search_latency_ms == 0.0 {
            ...
            metrics.avg_search_latency_ms =
                alpha * total_latency + (1.0 - alpha) * metrics.avg_search_latency_ms;
        }
        ...
        if embedding_latency > 0.0 {
            if metrics.avg_embedding_latency_ms == 0.0 {
                ...
                metrics.avg_embedding_latency_ms =
                    alpha * embedding_latency + (1.0 - alpha) * metrics.avg_embedding_latency_ms;
            }
        }
    }
    ...
    async fn adaptive_cache_cleanup(&self) {
        ...
        debug!(
            "🧹 Адаптивная очистка: удалено {} записей, осталось {}",
            keys_to_remove.len(),
            cache.cache.len()
        );
    }
...
impl std::fmt::Debug for SearchCoordinator {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("SearchCoordinator")
            ...
            .field("embedding_coordinator", &"<EmbeddingCoordinator>")
