...
impl MetricsCollector {
    ...
    pub fn record_batch_operation(
        ...
        debug!(
            "Batch operation '{}': {} records in {:?}",
            operation_type, record_count, duration
        );
    }
    ...
    pub fn export_prometheus(&self) -> String {
        ...
        output.push_str(
            "# HELP memory_vector_search_latency_ms Vector search latency in milliseconds\n",
        );
        output.push_str("# TYPE memory_vector_search_latency_ms histogram\n");
        output.push_str(&format!(
            "memory_vector_search_latency_ms_sum {}\n",
            metrics.vector_search_latency_ms.sum_ms
        ));
        output.push_str(&format!(
            "memory_vector_search_latency_ms_count {}\n",
            metrics.vector_search_latency_ms.count
        ));
        output.push_str(&format!(
            "memory_vector_search_latency_ms{{quantile=\"0.5\"}} {}\n",
            metrics.vector_search_latency_ms.p50_ms
        ));
        output.push_str(&format!(
            "memory_vector_search_latency_ms{{quantile=\"0.9\"}} {}\n",
            metrics.vector_search_latency_ms.p90_ms
        ));
        output.push_str(&format!(
            "memory_vector_search_latency_ms{{quantile=\"0.99\"}} {}\n",
        ...
        for (layer, layer_metrics) in &metrics.layer_sizes {
            ...
            output.push_str(&format!(
                "memory_layer_record_count{{layer=\"{}\"}} {}\n",
                layer, layer_metrics.record_count
            ));

            output.push_str(&format!(
                "memory_layer_size_bytes{{layer=\"{}\"}} {}\n",
    ...
    pub fn log_summary(&self) {
        ...
        info!(
            "  Interact → Insights: {}",
            metrics.promotions_interact_to_insights
        );
        info!(
            "  Insights → Assets: {}",
            metrics.promotions_insights_to_assets
        );
        ...
        if metrics.error_count > 0 {
            info!(
                "Errors: {} (last: {:?})",
                metrics.error_count, metrics.last_error
            );
        }
    }
}
