...
pub fn cosine_distance_safe(a: &[f32], b: &[f32]) -> f32 {
    ...
    let (dot_product, norm_a, norm_b) = a
        .iter()
        .zip(b.iter())
        .fold((0.0f32, 0.0f32, 0.0f32), |(dot, norm_a, norm_b), (&a_val, &b_val)| {
            (
                dot + a_val * b_val,
                norm_a + a_val * a_val, 
                norm_b + b_val * b_val,
...
pub fn check_simd_compatibility() -> SIMDCompatibility {
    SIMDCompatibility {
        ...
        recommended_chunk_size: if is_x86_feature_detected!("avx2") { 8 } else { 4 },
    }
}
...
pub fn benchmark_safe_algorithms(vector_size: usize, num_iterations: usize) -> BenchmarkResult {
    let a: Vec<f32> = (0..vector_size).map(|i| (i as f32) / vector_size as f32).collect();
    let b: Vec<f32> = (0..vector_size).map(|i| ((i + 1) as f32) / vector_size as f32).collect();
