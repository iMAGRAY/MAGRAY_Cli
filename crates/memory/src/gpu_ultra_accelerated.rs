...
use crate::simd_ultra_optimized::{cosine_distance_ultra_optimized, AlignedVector};
use anyhow::Result;
...
impl GpuCosineProcessor {
    ...
    pub fn initialize_buffers(&mut self, batch_size: usize) -> Result<()> {
        ...
        self.target_buffer = Some(GpuMemoryBuffer::allocate(self.device.id, self.vector_dim)?);

        self.result_buffer = Some(GpuMemoryBuffer::allocate(self.device.id, batch_size)?);
    ...
    fn mock_gpu_compute_batch(
        ...
        println!(
            "🚀 Mock GPU processed {} vectors in {:?} ({:.0} ops/sec)",
            queries.len(),
            duration / 10,
            ops_per_second * 10.0
        );
...
pub fn benchmark_gpu_vs_cpu(
    ...
    for device in device_manager.devices() {
        ...
        println!(
            "    Max Batch Size: {}",
            device.estimate_max_batch_size(vector_dim)
        );
    }
    ...
    if let Some(mut gpu_processor) = device_manager.create_cosine_processor(vector_dim)? {
        ...
        for &batch_size in batch_sizes {
            ...
            let queries: Vec<_> = (0..batch_size)
                .map(|i| {
                    let data: Vec<f32> = (0..vector_dim).map(|j| ((i + j) as f32).sin()).collect();
                    crate::simd_ultra_optimized::AlignedVector::new(data)
                })
                .collect();

            let target_data: Vec<f32> = (0..vector_dim).map(|i| (i as f32).cos()).collect();
            ...
            for _ in 0..iterations {
                let _cpu_results =
                    crate::simd_ultra_optimized::batch_cosine_distance_ultra(&queries, &target);
            }
            ...
            for _ in 0..iterations {
                let _gpu_results =
                    gpu_processor.compute_batch_cosine_distance(&queries, &target)?;
            }
            ...
            // Results
            println!(
                "  CPU Ultra-SIMD:   {:.2}ms ({:.0} ops/sec)",
                cpu_time.as_secs_f64() * 1000.0 / iterations as f64,
                cpu_ops_per_sec
            );
            println!(
                "  GPU Mock:         {:.2}ms ({:.0} ops/sec)",
                gpu_time.as_secs_f64() * 1000.0 / iterations as f64,
                gpu_ops_per_sec
            );
        ...
        println!(
            "  Memory: {}GB",
            mock_device.memory_total / (1024 * 1024 * 1024)
        );
        println!(
            "  Max Batch: {}",
            mock_device.estimate_max_batch_size(vector_dim)
        );
        ...
        println!(
            "  Shared Memory: {}KB",
            kernel_config.shared_memory_per_block / 1024
        );
    }
