...
impl BackupManager {
    ...
    pub async fn create_backup(
        ...
        for layer in [Layer::Interact, Layer::Insights, Layer::Assets] {
            ...
            let (count, size, layer_checksum) = self
                .export_layer_with_checksum(&store, layer, &layer_file)
                .await?;
            ...
            total_records += count;
            info!(
                "✅ Exported {} records from layer {:?} (checksum: {}...)",
                count,
                layer,
                &layer_checksum[..16]
            );
        ...
        info!(
            "🔒 Backup integrity: master checksum {}",
            &master_checksum[..16]
        );
        ...
        info!(
            "Backup created successfully: {:?} ({} records)",
            backup_path, total_records
        );
        Ok(backup_path)
    }
    ...
    pub async fn restore_backup(
        ...
        info!(
            "📦 Restoring backup: version {}, {} total records",
        ...
        if let Some(ref expected_checksum) = metadata.checksum {
            info!(
                "🔒 Verifying backup integrity (checksum: {}...)",
                &expected_checksum[..16]
            );
            let verified = self.verify_backup_integrity(temp_path, &metadata).await?;
            if !verified {
                return Err(anyhow!(
                    "❌ Backup integrity check FAILED - corrupted data detected"
                ));
            }
        ...
        info!(
            "Backup metadata: {} records from {}",
            metadata.total_records, metadata.created_at
        );
        ...
        for layer_info in &metadata.layers {
            ...
            if layer_file.exists() {
                let count = self
                    .import_layer(&store, layer_info.layer, &layer_file)
                    .await?;
    ...
    async fn export_layer(
        ...
        let (count, file_size, _checksum) = self
            .export_layer_with_checksum(store, layer, output_path)
            .await?;
    ...
    async fn verify_backup_integrity(
            ...
            for layer_info in &metadata.layers {
                ...
                if let Some(expected_checksum) = layer_checksums.get(layer_name) {
                    if &actual_checksum == expected_checksum {
                        ...
                        error!(
                            "❌ Layer {} checksum mismatch: expected {}, got {}",
        ...
        if let Some(ref expected_master) = metadata.checksum {
            ...
            if &actual_master == expected_master {
                ...
                error!(
                    "❌ Master checksum mismatch: expected {}, got {}",
        ...
        if verified_layers > 0 {
            info!(
                "✅ {} layer checksums verified (no master checksum)",
                verified_layers
            );
            return Ok(true);
        }
    ...
    pub async fn verify_backup(&self, backup_path: impl AsRef<Path>) -> Result<bool> {
        ...
        if metadata.checksum.is_none() && metadata.layer_checksums.is_none() {
            warn!(
                "Backup не содержит контрольных сумм для верификации: {:?}",
                backup_path
            );
        ...
        if let Some(layer_checksums) = &metadata.layer_checksums {
            for (layer_name, expected_checksum) in layer_checksums {
                ...
                if layer_file.exists() {
                    ...
                    if actual_checksum != *expected_checksum {
                        warn!(
                            "Контрольная сумма слоя {} не совпадает: ожидается {}, получено {}",
                            layer_name, expected_checksum, actual_checksum
                        );
                        return Ok(false);
                    }
...
mod tests {
    ...
    fn test_metadata_serialization() -> Result<()> {
        ...
        let parsed: BackupMetadata = serde_json::from_str(&json)
