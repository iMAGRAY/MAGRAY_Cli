...
pub trait CoreMemoryServiceTrait: Send + Sync {
    ...
    async fn batch_insert(&self, records: Vec<Record>) -> Result<crate::service_di::BatchInsertResult>;
    
    /// Батчевый поиск
    async fn batch_search(&self, queries: Vec<String>, layer: Layer, options: SearchOptions) 
