...
impl SimdLevel {
    ...
    fn detect_cpu_features() {
        ...
        info!("   Cache Line: {} bytes", CACHE_LINE_SIZE.load(Ordering::Relaxed));
        info!("   L1 Cache: {} KB", L1_CACHE_SIZE.load(Ordering::Relaxed) / 1024);
        info!("   L3 Cache: {} MB", L3_CACHE_SIZE.load(Ordering::Relaxed) / 1024 / 1024);
    }
...
impl CpuInfo {
    ...
    pub fn print_info(&self) {
        ...
        info!("   Cores: {} physical, {} logical", self.num_cores, self.num_logical_cores);
    ...
    fn detect_alignment(dim: usize) -> usize {
        ...
        match simd_level {
            SimdLevel::Avx512F | SimdLevel::Avx512VL => {
                if dim % 16 == 0 { 64 } else { 32 }  // 64-byte align for AVX-512
            }
            SimdLevel::Avx2 | SimdLevel::Avx2Fma => {
                if dim % 8 == 0 { 32 } else { 16 }   // 32-byte align for AVX2
            }
...
impl AdaptiveAlgorithmSelector {
    ...
    pub fn select_algorithm(&self, workload: &WorkloadProfile) -> AlgorithmStrategy {
        ...
        info!("🎯 Selected algorithm: {:?} for workload dimension={}, batch={}", 
    ...
    pub fn record_performance(
        ...
        debug!("📊 Recorded performance: {:?} -> {:.2} ops/sec", 
