...
impl PromotionMetrics for MLPromotionMetricsCollector {
    fn record_inference(&mut self, inference_time_ms: u64, accuracy: f32) {
        ...
        if self.config.detailed_logging {
            debug!("📊 Inference recorded: {}ms, accuracy: {:.2}%", 
                   inference_time_ms, accuracy * 100.0);
        }
    }
    ...
    fn get_stats(&self) -> MLPromotionStats {
        ...
        let avg_feature_extraction_time = if stats.feature_extraction_times.is_empty() {
            ...
            stats.feature_extraction_times.iter().sum::<u64>() / stats.feature_extraction_times.len() as u64
        };
        ...
        let gpu_utilization = if stats.gpu_utilization_samples.is_empty() {
            ...
            stats.gpu_utilization_samples.iter().sum::<f32>() / stats.gpu_utilization_samples.len() as f32
        };
        ...
        MLPromotionStats {
            ...
            promoted_records: stats.promoted_interact_to_insights + stats.promoted_insights_to_assets,
...
impl MLPromotionMetricsCollector {
    pub fn new(config: MetricsConfig) -> Self {
        ...
        info!("  - Aggregation interval: {}s", config.aggregation_interval_secs);
    ...
    pub fn record_promotion(&mut self, from_layer: crate::types::Layer, to_layer: crate::types::Layer) {
        let mut stats = self.stats.lock().unwrap();
    ...
    pub fn get_performance_breakdown(&self) -> PerformanceBreakdown {
        ...
        PerformanceBreakdown {
            ...
            feature_extraction_avg: if stats.feature_extraction_times.is_empty() {
                ...
                stats.feature_extraction_times.iter().sum::<u64>() as f32 / stats.feature_extraction_times.len() as f32
            },
    ...
    fn calculate_trend(&self, values: &VecDeque<f32>) -> f32 {
        ...
        let second_half_avg: f32 = values.iter().skip(mid).sum::<f32>() / (values.len() - mid) as f32;
    ...
    fn take_historical_snapshot(&mut self, stats: &InternalStats) {
        let snapshot = HistoricalSnapshot {
            ...
            avg_inference_time: if stats.inference_times.is_empty() {
                ...
                stats.inference_times.iter().sum::<u64>() as f32 / stats.inference_times.len() as f32
            },
            ...
            promotion_rate: if stats.total_analyzed > 0 {
                (stats.promoted_interact_to_insights + stats.promoted_insights_to_assets) as f32 / stats.total_analyzed as f32
            } else {
                0.0
            },
        };
