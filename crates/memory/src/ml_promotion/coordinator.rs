...
use super::data_processor::{
    DataProcessorConfig, MLDataProcessor, SimpleSemanticAnalyzer, SimpleUsageTracker,
};
...
impl PromotionCoordinatorBuilder {
    ...
    pub async fn build(mut self) -> Result<PromotionCoordinator> {
        ...
        let store = self
            .store
            .ok_or_else(|| anyhow::anyhow!("VectorStore is required"))?;
...
impl PromotionCoordinator {
    ...
    pub async fn run_promotion_cycle(&mut self) -> Result<MLPromotionStats> {
        ...
        info!(
            "🔬 Отфильтровано {} кандидатов после rules",
        ...
        info!(
            "✅ ML promotion cycle завершен: проанализировано {}, продвинуто {}",
    ...
    async fn analyze_candidates_with_ml(
        &mut self,
        candidates: Vec<Record>,
    ) -> Result<Vec<PromotionDecision>> {
            ...
            for record in batch {
                ...
                if promotion_score >= self.config.promotion_threshold {
                    ...
                    let target_layer = self
                        .rules_engine
                        .determine_target_layer(record, promotion_score);
    ...
    async fn execute_promotions(
        ...
        info!(
            "📈 Promotions выполнены: {}/{} успешно",
            promoted_count, analyzed_count
        );
    ...
    async fn execute_single_promotion(&self, decision: &PromotionDecision) -> Result<bool> {
        ...
        self.store
            .delete_by_id(&decision.record.id, decision.current_layer)
            .await?;
