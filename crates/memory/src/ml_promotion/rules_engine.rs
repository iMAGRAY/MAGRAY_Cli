...
pub enum BusinessRule {
    ...
    LayerBalance { min_interact_ratio: f32, max_assets_ratio: f32 },
...
impl PromotionRulesEngine for ConfigurableRulesEngine {
    ...
    async fn filter_candidates(&self, candidates: Vec<Record>) -> Result<Vec<Record>> {
        ...
        info!("🔬 Фильтрация {} кандидатов по business rules", total_candidates);
    ...
    fn validate_promotion(&self, decision: &PromotionDecision) -> bool {
        ...
        if decision.confidence < self.get_min_confidence_for_layer(decision.current_layer) {
            warn!("❌ Недостаточный confidence: {} < {}", 
                  decision.confidence, self.get_min_confidence_for_layer(decision.current_layer));
            return false;
        }
...
impl ConfigurableRulesEngine {
    pub fn new(config: RulesConfig) -> Self {
        ...
        info!("  - Max system load: {:.1}%", config.max_system_load_for_promotion * 100.0);
        info!("  - Min repromotion interval: {}h", config.min_repromotion_interval_hours);
        
        let mut layer_strategies = HashMap::new();
        
        // Настройка стратегий для каждого layer
        layer_strategies.insert(Layer::Interact, LayerStrategy {
            min_access_count: 2,
            ...
            special_conditions: vec![
                ...
                SpecialCondition::TimeOfDayRestriction { start_hour: 6, end_hour: 22 },
            ],
        });
        
        layer_strategies.insert(Layer::Insights, LayerStrategy {
            min_access_count: 5,
        ...
        layer_strategies.insert(Layer::Assets, LayerStrategy {
            min_access_count: 10,
    ...
    fn check_basic_requirements(&self, record: &Record) -> bool {
        ...
        if age_in_layer.num_hours() < self.config.min_layer_residence_time_hours as i64 {
            debug!("Запись {} слишком новая в layer: {}h < {}h", 
                   record.id, age_in_layer.num_hours(), self.config.min_layer_residence_time_hours);
            return false;
        }
    ...
    fn check_layer_strategy(&self, record: &Record) -> bool {
        ...
        if let Some(max_age) = strategy.max_age_hours {
            ...
            if age.num_hours() > max_age as i64 {
                debug!("Запись слишком старая: {}h > {}h", 
                       age.num_hours(), max_age);
                return false;
            }
        }
            ...
            match condition {
                SpecialCondition::MinTextLength(min_len) => {
                    if record.text.len() < *min_len {
                        debug!("Текст слишком короткий: {} < {}", record.text.len(), min_len);
                        return false;
                    }
                },
                SpecialCondition::RequiredKeywords(keywords) => {
                    let text_lower = record.text.to_lowercase();
                    let has_keyword = keywords.iter().any(|kw| text_lower.contains(&kw.to_lowercase()));
                    if !has_keyword {
                ...
                SpecialCondition::BlacklistKeywords(keywords) => {
                    let text_lower = record.text.to_lowercase();
                    let has_blacklisted = keywords.iter().any(|kw| text_lower.contains(&kw.to_lowercase()));
                    if has_blacklisted {
                ...
                SpecialCondition::MinSemanticImportance(min_importance) => {
                    ...
                    if semantic_score < *min_importance {
                        debug!("Недостаточная семантическая важность: {} < {}", 
                               semantic_score, min_importance);
                        return false;
                    }
                },
                SpecialCondition::TimeOfDayRestriction { start_hour, end_hour } => {
                    let current_hour = Utc::now().time().hour() as u8;
                    if current_hour < *start_hour || current_hour > *end_hour {
                        debug!("Вне разрешенного времени: {}h не в [{}h, {}h]", 
    ...
    async fn check_business_rules(&self, record: &Record) -> bool {
        for rule in &self.business_rules {
            let rule_passed = match rule {
                ...
                BusinessRule::LayerBalance { min_interact_ratio: _, max_assets_ratio: _ } => {
    ...
    async fn check_system_conditions(&self) -> bool {
        ...
        if current_system_load > self.config.max_system_load_for_promotion {
            debug!("Системная нагрузка слишком высока: {:.2} > {:.2}", 
    ...
    pub fn record_promotion(&mut self, record_id: uuid::Uuid) {
        ...
        self.promotion_history.recent_promotions
            .entry(record_id)
            .or_insert_with(Vec::new)
            .push(now);
        
        // Увеличиваем дневной счетчик
        *self.promotion_history.daily_counts.entry(record_id).or_insert(0) += 1;
...
mod tests {
    ...
    async fn test_basic_promotion_rules() {
        ...
        let record = create_test_record(Layer::Interact, 5, "This is a test record with sufficient text");
    ...
    async fn test_filter_candidates() {
        ...
        let candidates = vec![
            ...
            create_test_record(Layer::Interact, 3, "Another good record with sufficient content"),
        ];
