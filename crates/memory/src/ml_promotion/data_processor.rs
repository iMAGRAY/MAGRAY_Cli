...
use super::traits::{DataProcessor, UsageTracker, SemanticAnalyzer, TrainingExample};
use super::types::{PromotionFeatures, AccessPattern, SemanticContext, MLPromotionConfig};
...
impl DataProcessor for MLDataProcessor {
    ...
    async fn prepare_training_data(&self) -> Result<Vec<TrainingExample>> {
        ...
        if training_data.len() < 1000 {
            ...
            info!("🔄 Применена data augmentation, новый размер: {}", training_data.len());
        }
        
        let preparation_time = start_time.elapsed();
        info!("✅ Training data подготовлена за {:?}: {} примеров", 
              preparation_time, training_data.len());
    ...
    fn normalize_features(&self, features: &mut PromotionFeatures) {
        ...
        features.access_count = (features.access_count - stats.access_count_mean) / stats.access_count_std;
        features.access_frequency = (features.access_frequency - stats.access_frequency_mean) / stats.access_frequency_std;
        features.semantic_importance = (features.semantic_importance - stats.semantic_importance_mean) / stats.semantic_importance_std;
...
impl MLDataProcessor {
    pub async fn new(
        ...
        info!("  - Feature engineering: {}", config.enable_feature_engineering);
    ...
    fn apply_feature_engineering(&self, features: &mut PromotionFeatures, record: &Record) {
        ...
        debug!("🧬 Applied feature engineering: age_bucket={:.2}, access_velocity={:.2}", 
    ...
    async fn collect_positive_examples(&self) -> Result<Vec<TrainingExample>> {
        ...
        for record in assets_records.into_iter().take(500) {
            ...
            if age.num_hours() >= 24 {  // Достаточно старые для обучения
                let features = self.extract_features(&record).await?;
                examples.push(TrainingExample { features, label: 1.0 });
            }
        }
        ...
        for record in insights_records.into_iter().take(700) {
            ...
            if age.num_hours() >= 12 {
                let features = self.extract_features(&record).await?;
                examples.push(TrainingExample { features, label: 0.7 });
            }
        }
    ...
    async fn collect_negative_examples(&self) -> Result<Vec<TrainingExample>> {
        ...
        for record in interact_records.into_iter().take(800) {
            ...
            if age.num_hours() >= 48 && record.access_count < 3 {
                let features = self.extract_features(&record).await?;
                examples.push(TrainingExample { features, label: 0.1 });
            }
        }
    ...
    async fn augment_training_data(&self, mut data: Vec<TrainingExample>) -> Result<Vec<TrainingExample>> {
        ...
        for example in data.clone().iter().take(original_size / 2) {
            ...
            augmented_features.semantic_importance = augmented_features.semantic_importance.clamp(0.0, 1.0);
    ...
    fn get_cached_features(&self, record_id: &Uuid, current_access_count: u32) -> Option<CachedFeatures> {
    ...
    fn cache_features(&self, record_id: &Uuid, features: &PromotionFeatures, access_count: u32) {
        ...
        if cache.len() >= self.config.max_cache_size {
            ...
            if cache.len() >= self.config.max_cache_size {
                let keys_to_remove: Vec<_> = cache.keys().take(cache.len() - self.config.max_cache_size + 1).cloned().collect();
        ...
        cache.insert(*record_id, CachedFeatures {
    ...
    async fn compute_normalization_stats(store: &Arc<VectorStore>) -> Result<NormalizationStats> {
        ...
        let age_variance = age_values.iter().map(|x| (x - age_mean).powi(2)).sum::<f32>() / age_values.len() as f32;
        ...
        let access_variance = access_counts.iter().map(|x| (x - access_mean).powi(2)).sum::<f32>() / access_counts.len() as f32;
        let access_std = access_variance.sqrt().max(1.0);
        
        info!("✅ Статистика вычислена: age_mean={:.1}h, access_mean={:.1}", age_mean, access_mean);
...
impl UsageTracker for SimpleUsageTracker {
    fn record_access(&mut self, record_id: &Uuid) {
        let pattern = self.access_patterns.entry(*record_id).or_insert(AccessPattern {
            total_accesses: 0,
            recent_accesses: 0,
            access_velocity: 0.0,
            last_access: Utc::now(),
    ...
    fn get_temporal_pattern_score(&self, record_id: &Uuid) -> f32 {
        self.access_patterns.get(record_id)
