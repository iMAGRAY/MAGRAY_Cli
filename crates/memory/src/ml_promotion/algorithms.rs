...
use super::traits::{AlgorithmConfig, PromotionAlgorithm, TrainingExample};
...
impl PromotionAlgorithm for FrequencyAlgorithm {
    ...
    async fn train(&mut self, training_data: &[TrainingExample]) -> Result<f32> {
        ...
        info!(
            "🎯 Тренировка FrequencyAlgorithm на {} примерах",
            training_data.len()
        );
        ...
        for epoch in 0..self.config.epochs {
            ...
            if epoch % 20 == 0 {
                debug!(
                    "FrequencyAlgorithm epoch {}: accuracy={:.2}%",
                    epoch,
                    accuracy * 100.0
                );
            }
...
impl FrequencyAlgorithm {
    ...
    fn clamp_weights(&mut self) {
        ...
        self.weights.access_frequency_weight =
            self.weights.access_frequency_weight.clamp(-3.0, 3.0);
    ...
    fn evaluate_accuracy(&self, test_set: &[TrainingExample]) -> f32 {
        ...
        for example in test_set {
            ...
            let true_class = if example.label >= threshold {
                1.0f32
            } else {
                0.0f32
            };
...
impl PromotionAlgorithm for SemanticAlgorithm {
    ...
    async fn train(&mut self, training_data: &[TrainingExample]) -> Result<f32> {
        ...
        info!(
            "🧠 Тренировка SemanticAlgorithm на {} примерах",
            training_data.len()
        );
        ...
        for epoch in 0..self.config.epochs {
            ...
            if epoch % 20 == 0 {
                debug!(
                    "SemanticAlgorithm epoch {}: accuracy={:.2}%",
                    epoch,
                    accuracy * 100.0
                );
            }
...
impl SemanticAlgorithm {
    ...
    fn update_weights(&mut self, gradients: &SemanticGradients, learning_rate: f32) {
        self.weights.semantic_importance_weight -=
            learning_rate * gradients.semantic_importance_grad;
    ...
    fn clamp_weights(&mut self) {
        self.weights.semantic_importance_weight =
            self.weights.semantic_importance_weight.clamp(-3.0, 3.0);
...
impl PromotionAlgorithm for HybridAlgorithm {
    ...
    async fn train(&mut self, training_data: &[TrainingExample]) -> Result<f32> {
        ...
        info!(
            "🚀 Тренировка HybridAlgorithm на {} примерах",
            training_data.len()
        );
        ...
        for epoch in 0..self.config.epochs {
            ...
            if epoch % 10 == 0 {
                debug!(
                    "HybridAlgorithm epoch {}: accuracy={:.2}%",
                    epoch,
                    accuracy * 100.0
                );
            }
        ...
        info!(
            "✅ HybridAlgorithm тренировка завершена: accuracy={:.1}%",
            best_accuracy * 100.0
        );
        Ok(best_accuracy)
    }
...
impl AlgorithmFactory {
    pub fn create(
        algorithm_name: &str,
        config: AlgorithmConfig,
    ) -> Result<Box<dyn PromotionAlgorithm>> {
