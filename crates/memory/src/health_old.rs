...
mod tests {
    ...
    async fn test_health_monitor_basic() {
        ...
        // Записываем операцию
        monitor.record_operation(ComponentType::VectorStore, true, 150.0, None);
        
        let health = monitor.get_system_health();
        assert_eq!(health.overall_status, HealthStatus::Healthy);
        
        // Проверяем метрики
        let metrics = monitor.get_component_metrics(ComponentType::VectorStore, \"search_latency\", None);
    ...
    async fn test_alert_generation() {
        ...
        // Метрика превышающая критический порог
        let critical_metric = health_metric!(
        ...
        monitor.record_metric(critical_metric).unwrap();
        
        // Даем время на обработку alert
        sleep(TokioDuration::from_millis(100)).await;
        
        let health = monitor.get_system_health();
        assert!(!health.active_alerts.is_empty());
        assert_eq!(health.active_alerts[0].severity, AlertSeverity::Critical);
    }
    ...
    fn test_component_performance_tracking() {
        ...
        monitor.record_operation(ComponentType::RerankingService, true, 100.0, None);
        monitor.record_operation(ComponentType::RerankingService, true, 200.0, None);
        monitor.record_operation(ComponentType::RerankingService, false, 300.0, Some(\"Timeout\".to_string()));
        
        let perf = monitor.get_component_performance(ComponentType::RerankingService).unwrap();
        assert_eq!(perf.total_requests, 3);
        assert_eq!(perf.failed_requests, 1);
        assert!(perf.success_rate > 0.6 && perf.success_rate < 0.7);
        assert_eq!(perf.avg_response_time_ms, 200.0);
        assert_eq!(perf.last_error, Some(\"Timeout\".to_string()));
    }
}