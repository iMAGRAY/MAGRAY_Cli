...
pub unsafe fn cosine_distance_avx512(a: &[f32], b: &[f32]) -> f32 {
    ...
    debug_assert_eq!(
        a.len() % 16,
        0,
        "Vector length must be multiple of 16 for AVX512"
    );
...
pub fn run_comprehensive_benchmark() {
    ...
    #[cfg(target_arch = "x86_64")]
    {
        if is_x86_feature_detected!("avx2") {
            ...
            println!(
                "  Avg per operation: {:.2} μs",
                avx2_duration.as_micros() as f64 / ITERATIONS as f64
            );
            ...
            let aligned_speedup =
                scalar_duration.as_nanos() as f64 / aligned_duration.as_nanos() as f64;
        ...
        if is_x86_feature_detected!("avx512f") {
            ...
            let avx512_speedup =
                scalar_duration.as_nanos() as f64 / avx512_duration.as_nanos() as f64;
    ...
    println!(
        "  Per vector: {:.2} μs",
        batch_duration.as_micros() as f64 / 100.0
    );
    println!();
