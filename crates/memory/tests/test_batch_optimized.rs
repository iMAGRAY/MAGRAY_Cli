...
mod tests {
    use memory::{
        ...
        Layer, Record, AlignedBatchVectors,
    };
    use std::sync::Arc;
    ...
    async fn test_large_batch_processing() {
        ...
        assert!(duration.as_millis() < 500, "Large batch should be under 500ms");
    ...
    async fn test_simd_processing() {
        ...
        assert!(simd_duration <= no_simd_duration * 2, 
    ...
    async fn test_memory_pool_reuse() {
        ...
        assert!(stats.memory_pool_efficiency > 0.5, "Pool should be efficient");
    }
    ...
    async fn test_sub_5ms_sla() {
        ...
        println!("Avg latency: {:.2}ms, Max: {:.2}ms", avg_latency, max_latency);
        
        // SLA проверки
        assert!(avg_latency < 10.0, "Average should be under 10ms for small batches");
    ...
    async fn test_graceful_shutdown() {
        ...
        let stats = processor.get_stats().await;
        assert!(stats.total_batches > 0, "Should have processed some batches");
    }
    ...
    async fn test_performance_metrics() {
        ...
        // Efficiency метрики
        assert!(stats.cpu_efficiency >= 0.0 && stats.cpu_efficiency <= 1.0,
