...
mod tests {
    use memory::{
        GpuBatchProcessor, GpuBatchConfig, GpuBatchStats,
        Layer, Record, GpuMemoryPool,
        GpuError, GpuDevice,
    };
    use std::sync::Arc;
    ...
    fn generate_gpu_batch(size: usize, dimension: usize) -> Vec<Record> {
        (0..size).map(|i| create_gpu_test_record(i, dimension)).collect()
    }
    ...
    async fn test_gpu_processor_initialization() {
        ...
        assert!(stats.gpu_device_info.is_some() || stats.fallback_active, 
    ...
    async fn test_gpu_device_detection() {
        ...
        if gpu_available {
            ...
            for device in devices {
                ...
                assert!(device.compute_capability >= 6.0, "Should support modern CUDA");
    ...
    async fn test_gpu_memory_pool_initialization() {
        ...
        if pool.is_ok() {
            ...
            assert!(stats.total_size_mb >= 64, "Pool should have allocated memory");
    ...
    async fn test_large_gpu_batch_processing() {
        ...
        if !stats.fallback_active {
            ...
            assert!(duration.as_millis() < 200, "Large GPU batch should be under 200ms");
        }
    }
    ...
    async fn test_mixed_precision_processing() {
        ...
        if !stats.fallback_active {
            ...
            assert!(fp16_duration <= fp32_duration * 2, "FP16 should not be much slower");
        }
    }
    ...
    async fn test_fallback_performance_comparison() {
        ...
        assert_eq!(gpu_stats.total_records, batch_size, "GPU should process all");
        assert_eq!(cpu_stats.total_records, batch_size, "CPU should process all");
    }
    ...
    async fn test_gpu_memory_alignment() {
        ...
        for size in sizes {
            ...
            let result = processor.process_batch(batch).await;
            assert!(result.is_ok(), "Size {} should process with proper alignment", size);
        }
    ...
    async fn test_cuda_context_management() {
        ...
        let handle1 = tokio::spawn(async move {
            processor1.process_batch(batch1).await
        });
        
        let handle2 = tokio::spawn(async move {
            processor2.process_batch(batch2).await
        });
    ...
    async fn test_concurrent_gpu_batch_processing() {
        ...
        assert_eq!(success_count, 8, "All concurrent GPU batches should succeed");
    ...
    async fn test_gradual_performance_degradation() {
        ...
        for i in 1..avg_latencies.len() {
            ...
            assert!(growth < 5.0, "Latency should not grow too dramatically: {}", growth);
        }
    }
    ...
    async fn test_gpu_resource_cleanup() {
        ...
        assert_eq!(stats_after.total_batches, stats_before.total_batches, 
                  "Stats should persist after cleanup");
        
        if let (Some(pool_before), Some(pool_after)) = 
            (stats_before.memory_pool_stats, stats_after.memory_pool_stats) {
            assert!(pool_after.used_size_mb <= pool_before.used_size_mb,
    ...
    async fn test_gpu_statistics_tracking() {
        ...
        assert!(stats.avg_batch_size > 0.0, "Should track average batch size");
        
        // GPU specific метрики
        if !stats.fallback_active {
            assert!(stats.gpu_memory_usage_mb.is_some(), "Should track GPU memory");
            assert!(stats.cuda_kernel_time_ms.is_some(), "Should track kernel time");
            assert!(stats.gpu_utilization.is_some(), "Should track GPU utilization");
        }
    }
    ...
    async fn test_gpu_performance_metrics() {
        ...
        if !stats.fallback_active {
            ...
            assert!(stats.gpu_efficiency >= 0.0 && stats.gpu_efficiency <= 1.0,
                   "GPU efficiency should be 0-1");
            assert!(stats.memory_transfer_efficiency >= 0.0,
