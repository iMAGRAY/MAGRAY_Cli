...
async fn test_cache_service_creation() -> Result<()> {
    ...
    assert_eq!(
        ...
        1024,
        "Default embedding dimension должно быть 1024"
    );
    ...
    assert_eq!(
        service_with_coordinator.get_embedding_dimension(),
        1024,
        "Embedding dimension с coordinator должно быть 1024"
    );

    // Test creation with custom dimension
    let custom_service = CacheService::new_with_dimension(container, 512);
    assert_eq!(
        custom_service.get_embedding_dimension(),
        512,
        "Custom embedding dimension должно быть 512"
    );
...
async fn test_fallback_embedding_generation() -> Result<()> {
    ...
    assert_eq!(
        embedding.len(),
        1024,
        "Fallback embedding должен иметь размерность 1024"
    );
    ...
    assert!(
        (norm - 1.0).abs() < 0.001,
        "Embedding должен быть нормализован (norm ≈ 1.0)"
    );
    ...
    assert_ne!(
        ...
        "Разные входы должны давать разные embeddings"
    );
...
async fn test_get_or_create_embedding() -> Result<()> {
    ...
    assert_eq!(
        embedding.len(),
        1024,
        "Generated embedding должен иметь правильную размерность"
    );
    ...
    assert_eq!(
        embedding, embedding2,
        "Repeated calls должны возвращать идентичные embeddings"
    );
...
async fn test_get_or_create_embedding_with_coordinator() -> Result<()> {
    ...
    assert_eq!(
        embedding.len(),
        1024,
        "Embedding с coordinator должен иметь правильную размерность"
    );
...
async fn test_cache_hit_rate() -> Result<()> {
    ...
    assert_eq!(
        hit_rate, 0.0,
        "Hit rate должен быть 0.0 при отсутствии операций"
    );
...
async fn test_clear_cache() -> Result<()> {
    ...
    assert!(
        result.is_err(),
        "Clear cache должен завершаться с ошибкой без реального cache"
    );
...
async fn test_set_cache_size() -> Result<()> {
    ...
    assert!(
        result.is_err(),
        "Set cache size должен завершаться с ошибкой без реального cache"
    );
...
async fn test_detailed_cache_stats() -> Result<()> {
    ...
    assert_eq!(
        ...
        "Embedding dimension должно быть 1024"
    );
    assert!(
        stats.coordinator_available,
        "Coordinator должен быть доступен"
    );
...
async fn test_embedding_dimension_modification() -> Result<()> {
    ...
    assert_eq!(
        service.get_embedding_dimension(),
        1024,
        "Initial dimension должно быть 1024"
    );
    ...
    assert_eq!(
        service.get_embedding_dimension(),
        768,
        "Dimension должно обновиться до 768"
    );
    ...
    assert_eq!(
        embedding.len(),
        768,
        "New embedding должен иметь обновленную размерность"
    );
...
async fn test_concurrent_embedding_generation() -> Result<()> {
    ...
    let tasks: Vec<_> = (0..20)
        .map(|i| {
            ...
            tokio::spawn(async move {
                service_clone
                    .get_or_create_embedding(&format!("query {}", i))
                    .await
            })
        })
        .collect();
    ...
    for (i, result) in results.into_iter().enumerate() {
        ...
        assert_eq!(
            embedding.len(),
            1024,
            "Embedding {} должен иметь правильную размерность",
            i
        );
    }
...
async fn test_concurrent_cache_operations() -> Result<()> {
    ...
    for (i, result) in results.into_iter().enumerate() {
        assert!(
            result.is_ok(),
            "Concurrent operation {} должна завершиться без panic",
            i
        );
    }
...
async fn test_mixed_concurrent_operations() -> Result<()> {
    ...
    let tasks: Vec<_> = (0..50)
        .map(|i| {
            ...
            if i % 3 == 0 {
                tokio::spawn(async move {
                    service_clone
                        .get_or_create_embedding(&format!("text {}", i))
                        .await
                        .map(|_| ())
                })
            } else if i % 3 == 1 {
    ...
    for (i, result) in results.into_iter().enumerate() {
        assert!(
            result.is_ok(),
            "Mixed operation {} должна завершиться без panic",
            i
        );
    }
...
async fn test_edge_cases() -> Result<()> {
    ...
    assert_eq!(
        empty_embedding.len(),
        1024,
        "Empty text должен генерировать embedding правильной размерности"
    );
    ...
    assert_eq!(
        long_embedding.len(),
        1024,
        "Long text должен генерировать embedding правильной размерности"
    );
    ...
    assert_eq!(
        special_embedding.len(),
        1024,
        "Special characters должны генерировать embedding правильной размерности"
    );
    ...
    assert_eq!(
        unicode_embedding.len(),
        1024,
        "Unicode text должен генерировать embedding правильной размерности"
    );
...
async fn test_extreme_dimensions() -> Result<()> {
    ...
    assert!(
        (small_embedding[0].abs() - 1.0).abs() < 0.001,
        "Single value должен быть нормализован до ±1"
    );
    ...
    assert_eq!(
        large_embedding.len(),
        8192,
        "Large dimension должен работать"
    );
    ...
    assert!(
        (norm - 1.0).abs() < 0.001,
        "Large embedding должен быть нормализован"
    );
...
async fn test_cache_service_with_real_coordinator() -> Result<()> {
    ...
    let health_monitor = Arc::new(HealthMonitor::new(
        memory::health::HealthMonitorConfig::default(),
    ));
    ...
    let resource_manager = Arc::new(parking_lot::RwLock::new(
        memory::resource_manager::ResourceManager::new(
            memory::resource_manager::ResourceConfig::default(),
        )?,
    ));
    ...
    let embedding = cache_service
        .get_or_create_embedding("integration test")
        .await?;
    assert_eq!(
        embedding.len(),
        1024,
        "Integration test embedding должен работать"
    );

    let stats = cache_service.get_detailed_cache_stats().await;
    assert!(
        stats.coordinator_available,
        "Coordinator должен быть доступен в integration тесте"
    );
...
async fn test_cache_service_memory_safety() -> Result<()> {
    ...
    for i in 0..50 {
        ...
        let embedding = service
            .get_or_create_embedding(&format!("test text {}", i))
            .await?;
        assert_eq!(
            embedding.len(),
            1024 + i * 10,
        ...
        for j in 0..10 {
            ...
            assert_eq!(
                fallback.len(),
                1024 + i * 10,
                "Fallback embedding должен иметь правильную размерность"
            );
        }
    }
...
async fn test_cache_service_consistency_across_instances() -> Result<()> {
    ...
    assert_eq!(
        ...
        "Different service instances должны генерировать идентичные embeddings"
    );
...
async fn stress_test_cache_service() -> Result<()> {
    ...
    let tasks: Vec<_> = (0..1000)
        .map(|i| {
            ...
            tokio::spawn(async move {
                ...
                if i % 4 == 0 {
                    service_clone
                        .get_or_create_embedding(&format!("stress test {}", i))
                        .await
                        .map(|_| ())
                } else if i % 4 == 1 {
    ...
    for (i, result) in results.into_iter().enumerate() {
        assert!(
            result.is_ok(),
            "Stress test operation {} должна завершиться без panic",
            i
        );
        assert!(
            result.unwrap().is_ok(),
