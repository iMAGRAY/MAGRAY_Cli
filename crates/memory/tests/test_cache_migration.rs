use anyhow::Result;
use memory::{
    EmbeddingCache, EmbeddingCacheLRU, CacheConfig, 
    migrate_cache_to_lru, recommend_cache_config
};
...
async fn test_full_migration_workflow() -> Result<()> {
    ...
    let migration_result = migrate_cache_to_lru(&old_cache_path, &new_cache_path, recommended_config.clone()).await;
...
fn test_config_for_different_embedding_sizes() -> Result<()> {
    ...
    println!("  📊 Qwen3 embeddings: {} entries поместится в cache", qwen3_entries);
    ...
    println!("  📊 Large embeddings (1536d): {} entries поместится", large_entries);
    ...
    assert!(config.max_entries >= qwen3_entries / 4, 
        "Config entries {} должно поместить хотя бы 1/4 от Qwen3 {}", 
        config.max_entries, qwen3_entries / 4);
    
    println!("  📊 Config max_entries: {}, реально поместится Qwen3: {}, large: {}", 
