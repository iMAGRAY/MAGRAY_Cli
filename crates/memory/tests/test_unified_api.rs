use anyhow::Result;
use memory::{
    UnifiedMemoryAPI, MemoryContext, create_di_memory_service,
    Layer, 
    api::SearchOptions,
};
use std::sync::Arc;
...
async fn test_unified_api_basic_operations() -> Result<()> {
    ...
    let id = api.remember("Test memory for API".to_string(), context).await?;
    ...
    println!("  ✅ Recall операция: найдено {} результатов", results.len());
    
    // Тест 3: Health check
    let health = api.health_check().await?;
    println!("  📊 Health status: {}", health.status);
    assert!(matches!(health.status, "healthy" | "degraded"), "Система должна быть здорова");
    ...
    println!("  🔄 Optimization time: {}ms", optimization_result.total_time_ms);
    assert!(optimization_result.total_time_ms >= 0, "Время оптимизации должно быть положительным");
...
async fn test_unified_api_search_scenarios() -> Result<()> {
    ...
    let test_data = vec![
        ("Machine learning algorithms overview", Layer::Interact, vec!["ai", "ml"]),
        ("Deep neural networks implementation", Layer::Insights, vec!["ai", "implementation"]),
        ("Python programming best practices", Layer::Assets, vec!["python", "programming"]),
        ("Rust systems programming guide", Layer::Assets, vec!["rust", "systems"]),
    ];
    ...
    assert!(interact_results.iter().all(|r| r.layer == Layer::Interact), "Все результаты должны быть из Interact");
    println!("  ✅ Поиск по одному слою: {} результатов", interact_results.len());
    ...
    println!("  ✅ Поиск по всем слоям: {} результатов", all_results.len());
    
    // Тест 3: Поиск с лимитом
    let options = SearchOptions::new().limit(2);
    let limited_results = api.recall("guide", options).await?;
    assert!(limited_results.len() <= 2, "Результаты должны быть ограничены");
    println!("  ✅ Ограничение результатов работает: {} результатов", limited_results.len());
    ...
    println!("  📊 Результатов для несуществующего запроса: {}", empty_results.len());
...
async fn test_unified_api_error_handling() -> Result<()> {
    ...
    assert!(!forget_result, "Forget несуществующего ID должен возвращать false");
    ...
    println!("  📊 Результатов для пустого запроса: {}", empty_results.len());
    ...
    assert!(stats.total_records >= 0, "Total records должен быть неотрицательным");
...
async fn test_unified_api_performance() -> Result<()> {
    ...
    assert!(remember_time.as_secs() < 5, "Remember операции должны быть быстрыми");
    assert!(search_time.as_secs() < 3, "Поиск должен быть быстрым");
    assert!(monitoring_time.as_secs() < 2, "Мониторинг должен быть быстрым");
