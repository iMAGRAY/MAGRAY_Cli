...
use memory::{
    services::{CoreMemoryService, traits::CoreMemoryServiceTrait},
    ...
    container.register(
        move |_container: &DIContainer| -> Result<VectorStore, anyhow::Error> {
            tokio::runtime::Handle::current().block_on(async {
                VectorStore::new(&temp_dir_clone).await
            })
        },
        Lifetime::Singleton,
...
async fn test_insert_single_record() -> Result<()> {
    ...
    let results = service.search("Test record 1", Layer::Interact, default_search_options()).await?;
...
async fn test_insert_batch_records() -> Result<()> {
    ...
    for i in 1..=5 {
        let results = service.search(&format!("Test record {}", i), Layer::Interact, default_search_options()).await?;
...
async fn test_search_functionality() -> Result<()> {
    ...
    let interact_results = service.search("Test record", Layer::Interact, default_search_options()).await?;
    assert_eq!(interact_results.len(), 2, "Должны найти 2 записи в Interact");
    
    let insights_results = service.search("Test record", Layer::Insights, default_search_options()).await?;
    assert_eq!(insights_results.len(), 1, "Должны найти 1 запись в Insights");
    
    // Test search with different top_k
    let mut limited_options = default_search_options();
    limited_options.top_k = 1;
    let limited_results = service.search("Test record", Layer::Interact, limited_options).await?;
    assert!(limited_results.len() <= 1, "Должны получить максимум 1 результат");
...
async fn test_update_record() -> Result<()> {
    ...
    let results = service.search("Updated test record", Layer::Interact, default_search_options()).await?;
...
async fn test_delete_record() -> Result<()> {
    ...
    let results_before = service.search("Test record 1", Layer::Interact, default_search_options()).await?;
    assert!(!results_before.is_empty(), "Запись должна существовать до удаления");
    ...
    let results_after = service.search("Test record 1", Layer::Interact, default_search_options()).await?;
...
async fn test_batch_insert_with_results() -> Result<()> {
    ...
    assert!(result.total_time_ms > 0, "Время выполнения должно быть больше 0");
...
async fn test_batch_search_functionality() -> Result<()> {
    ...
    let result = service.batch_search(queries.clone(), Layer::Interact, default_search_options()).await?;
    
    assert_eq!(result.queries, queries, "Queries должны совпадать");
    assert_eq!(result.results.len(), 3, "Должны получить результаты для 3 запросов");
    assert!(result.total_time_ms > 0, "Время выполнения должно быть больше 0");
    
    // Verify each query returned results
    for (i, query_results) in result.results.iter().enumerate() {
        assert!(!query_results.is_empty(), "Запрос {} должен вернуть результаты", i + 1);
    }
...
async fn test_concurrent_operations() -> Result<()> {
    ...
    for (i, result) in results.into_iter().enumerate() {
        assert!(result.is_ok(), "Task {} должна завершиться без panic", i + 1);
        assert!(result.unwrap().is_ok(), "Insert {} должен быть успешным", i + 1);
    }
    ...
    for i in 1..=5 {
        ...
        search_tasks.push(tokio::spawn(async move {
            service_clone.search(&format!("Test record {}", i), Layer::Interact, default_search_options()).await
        }));
    }
    
    let search_results = futures::future::join_all(search_tasks).await;
    for (i, result) in search_results.into_iter().enumerate() {
        assert!(result.is_ok(), "Search task {} должна завершиться без panic", i + 1);
        assert!(result.unwrap().is_ok(), "Search {} должен быть успешным", i + 1);
    }
...
async fn test_error_handling() -> Result<()> {
    ...
    let empty_search = service.search("", Layer::Interact, default_search_options()).await;
    assert!(empty_search.is_ok(), "Поиск с пустым запросом должен обрабатываться корректно");
...
async fn test_embedding_generation() -> Result<()> {
    ...
    let results = service.search("test query", Layer::Interact, default_search_options()).await?;
    // Embedding должен генерироваться внутри search метода
    
    // Test consistency of embedding generation
    let results1 = service.search("consistent query", Layer::Interact, default_search_options()).await?;
    let results2 = service.search("consistent query", Layer::Interact, default_search_options()).await?;
...
async fn test_operation_limiter_concurrency() -> Result<()> {
    ...
    assert!(elapsed >= Duration::from_millis(250), "Concurrency limiter должен работать");
...
async fn test_metrics_integration() -> Result<()> {
    ...
    service.search("Test record 1", Layer::Interact, default_search_options()).await?;
...
async fn test_edge_cases() -> Result<()> {
    ...
    assert!(result.is_ok(), "Empty batch должен обрабатываться корректно");
    ...
    let special_chars_search = service.search("!@#$%^&*()", Layer::Interact, default_search_options()).await;
    assert!(special_chars_search.is_ok(), "Поиск с спецсимволами должен работать");
