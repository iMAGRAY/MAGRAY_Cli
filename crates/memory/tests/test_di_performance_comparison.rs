...
fn test_registration_performance_comparison() -> Result<()> {
    ...
    let improvement = original_registration_time.as_nanos() as f64 / optimized_registration_time.as_nanos() as f64;
    println!("    Improvement: {:.2}x", improvement);

    // Оптимизированный должен быть не медленнее оригинального
    assert!(optimized_registration_time <= original_registration_time * 2, 
...
fn test_singleton_resolution_performance() -> Result<()> {
    ...
    let cache_improvement = original_cached_resolves.as_nanos() as f64 / optimized_cached_resolves.as_nanos() as f64;
...
fn test_concurrent_performance() -> Result<()> {
    ...
    println!("  📊 {} concurrent разрешений ({} threads × {}):", 
    ...
    let improvement = original_concurrent_time.as_nanos() as f64 / optimized_concurrent_time.as_nanos() as f64;
    println!("    Improvement: {:.2}x", improvement);
...
fn test_builder_performance() -> Result<()> {
    ...
    let improvement = original_builder_time.as_nanos() as f64 / optimized_builder_time.as_nanos() as f64;
    println!("    Improvement: {:.2}x", improvement);
...
fn test_comprehensive_performance_comparison() -> Result<()> {
    ...
    for cycle in 0..resolution_cycles {
        for i in 0..10 { // Resolve 10 random services per cycle
    ...
    println!("  📊 Comprehensive test ({} services, {} resolutions):", 
             service_count, total_resolutions);
    println!("    Original total: {:?}", original_mixed_time);
    println!("    Optimized total: {:?}", optimized_mixed_time);
    
    let overall_improvement = original_mixed_time.as_nanos() as f64 / optimized_mixed_time.as_nanos() as f64;
    ...
    assert!(metrics.cache_hits > 0, "Should have cache hits");
    assert!(overall_improvement >= 0.8, "Overall performance should not significantly regress");
