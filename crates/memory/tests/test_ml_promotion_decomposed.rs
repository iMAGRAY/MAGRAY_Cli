...
use memory::ml_promotion::{
    MLPromotionConfig, MLPromotionStats, PromotionDecision, PromotionFeatures,
    PromotionCoordinator, PromotionCoordinatorBuilder, CoordinatorInfo,
    FrequencyAlgorithm, SemanticAlgorithm, HybridAlgorithm, AlgorithmFactory,
    MLPromotionMetricsCollector, MetricsConfig, PerformanceBreakdown,
    ConfigurableRulesEngine, RulesConfig, LayerStrategy, SpecialCondition, BusinessRule,
    MLDataProcessor, DataProcessorConfig, SimpleUsageTracker, SimpleSemanticAnalyzer,
    traits::{PromotionAlgorithm, PromotionMetrics, PromotionRulesEngine, DataProcessor, TrainingExample}
};
...
mod test_config_variants {
    ...
    fn test_rules_config_variants() {
        ...
        assert!(production_config.min_repromotion_interval_hours > default_config.min_repromotion_interval_hours);
        assert!(production_config.min_layer_residence_time_hours > default_config.min_layer_residence_time_hours);

        // Development более гибкий
        assert!(development_config.min_repromotion_interval_hours < default_config.min_repromotion_interval_hours);
        assert!(development_config.max_promotions_per_record_per_day > default_config.max_promotions_per_record_per_day);
    }
}
...
mod test_algorithms {
    ...
    fn test_algorithm_factory() {
        ...
        for algorithm_name in algorithms {
            ...
            assert!(algorithm.is_ok(), "Failed to create algorithm: {}", algorithm_name);
        }
...
mod test_rules_engine {
    ...
    async fn test_can_promote_basic() {
        ...
        let good_record = create_test_record(Layer::Interact, 5, "Good record with sufficient text content", 2);
    ...
    async fn test_filter_candidates() {
        ...
        let candidates = vec![
            ...
            create_test_record(Layer::Interact, 3, "Another decent record with good content", 1),
        ];
...
mod test_data_processor {
    ...
    async fn test_semantic_analyzer() {
        ...
        let importance = analyzer.analyze_importance("This is a critical error warning").await.unwrap();
        ...
        // Тест без keywords
        let no_keywords = analyzer.analyze_importance("normal text without special words").await.unwrap();
...
mod test_integration {
    ...
    fn test_coordinator_builder_creation() {
        ...
        assert_eq!(std::mem::size_of_val(&builder), std::mem::size_of_val(&prod_builder));
        assert_eq!(std::mem::size_of_val(&builder), std::mem::size_of_val(&dev_builder));
    }
    ...
    fn test_factory_functions() {
        ...
        use memory::ml_promotion::{create_production_coordinator, create_development_coordinator};
...
mod test_solid_principles {
    ...
    fn test_interface_segregation_principle() {
        ...
        use memory::ml_promotion::traits::{PromotionAlgorithm, PromotionMetrics, PromotionRulesEngine};
        
        // PromotionAlgorithm - только для ML prediction
        let _: fn(&dyn PromotionAlgorithm, &PromotionFeatures) -> f32 = |algo, features| {
            algo.predict_score(features)
        };
        
        // PromotionMetrics - только для metrics
        let _: fn(&dyn PromotionMetrics) -> MLPromotionStats = |metrics| {
            metrics.get_stats()
        };
    ...
    fn test_dependency_inversion_principle() {
        ...
        let _builder_with_deps = builder
