...
async fn test_qwen3_memory_bridge_basic() -> Result<()> {
    ...
    match Qwen3MemoryBridge::new(config).await {
        Ok(bridge) => {
            ...
            println!("✅ Embedding dimension корректный: {}", bridge.embedding_dim());
            ...
            match init_result {
                Ok(()) => {
                    ...
                    match test_result {
                        Ok(embedding) => {
                            println!("✅ Single embedding получен (размерность: {})", embedding.len());
                    ...
                    match batch_result {
                        Ok(embeddings) => {
                            println!("✅ Batch embedding получен ({} embeddings)", embeddings.len());
                ...
                Err(e) => {
                    ...
                    match fallback_result {
                        Ok(embedding) => {
                            println!("✅ Fallback embedding работает (размерность: {})", embedding.len());
                            assert_eq!(embedding.len(), 1024);
                        }
                        Err(e) => {
...
#[cfg(all(not(feature = "minimal"), feature = "embeddings", feature = "gpu-acceleration"))]
#[tokio::test]
async fn test_qwen3_gpu_batch_processor_integration() -> Result<()> {
    use std::sync::Arc;
    use memory::{EmbeddingCache, CacheConfig, gpu_accelerated::{GpuBatchProcessor, BatchProcessorConfig}};
    ...
    match GpuBatchProcessor::with_qwen3_bridge(config, cache).await {
        Ok(processor) => {
            ...
            match test_result {
                Ok(embedding) => {
                    println!("✅ Processor embedding получен (размерность: {})", embedding.len());
