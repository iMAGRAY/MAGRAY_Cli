...
use anyhow::Result;
use memory::di::{
    UnifiedDIContainer, UnifiedDIContainerBuilder, ContainerConfiguration,
    DIResolver, DIRegistrar, Lifetime,
    DIMemoryServiceMigrationFacade, LegacyMemoryConfig,
};
use std::{
...
async fn test_unified_container_creation_and_basic_operations() -> Result<()> {
    ...
    container.register(
        |_| Ok(SimpleTestService::new("test")),
        Lifetime::Singleton
    )?;
...
async fn test_builder_pattern() -> Result<()> {
    ...
    assert_eq!(container.configuration.instance_creation_timeout, Duration::from_secs(5));
...
async fn test_instance_registration() -> Result<()> {
    ...
    assert_eq!(resolved1.counter.load(Ordering::SeqCst), original_counter_value);
...
async fn test_complex_dependency_graph() -> Result<()> {
    ...
    container.register(
        |_| Ok(SimpleTestService::new("root")),
        Lifetime::Singleton
    )?;
...
async fn test_duplicate_registration_error() -> Result<()> {
    ...
    container.register(
        |_| Ok(SimpleTestService::new("first")),
        Lifetime::Singleton
    )?;
...
async fn test_concurrent_access() -> Result<()> {
    ...
    for i in 0..10 {
        ...
        let handle = tokio::spawn(async move {
            let service = container_clone.resolve::<SimpleTestService>()
...
async fn test_full_integration_scenario() -> Result<()> {
    ...
    for i in 1..services.len() {
        if let (Ok(s1), Ok(s2)) = (services[0].downcast_ref::<DependentTestService>(), services[i].downcast_ref::<DependentTestService>()) {
            if std::any::TypeId::of::<DependentTestService>() == std::any::TypeId::of::<DependentTestService>() {
...
async fn test_edge_case_very_large_dependency_graph() -> Result<()> {
    ...
    for i in 0..100 {
        ...
        container.register(
            ...
            if i % 2 == 0 { Lifetime::Singleton } else { Lifetime::Transient }
        )?;
...
async fn test_stress_concurrent_high_load() -> Result<()> {
    ...
    for batch in 0..20 {
        for thread in 0..50 {
            ...
            let handle = tokio::spawn(async move {
                let service = container_clone.resolve::<SimpleTestService>()
                    .expect(&format!("Failed to resolve in batch {} thread {}", batch, thread));
