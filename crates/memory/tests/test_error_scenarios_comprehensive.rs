...
use memory::{
    service_di::{default_config, DIMemoryService, MemoryServiceConfig},
    ...
    match result {
        Ok(_) => {
            ...
            let search_results = service
                .search("Valid content", Layer::Interact, SearchOptions::default())
                .await?;
...
async fn test_error_invalid_search_options() -> Result<()> {
    ...
    let result = service
        .search("Test", Layer::Interact, invalid_options)
        .await;
    ...
    let result = service
        .search("Test", Layer::Interact, huge_limit_options)
        .await;
...
async fn test_error_boundary_conditions() -> Result<()> {
    ...
    let empty_search = service
        .search("", Layer::Interact, SearchOptions::default())
        .await;
    match empty_search {
    ...
    let long_search = service
        .search(&long_query, Layer::Interact, SearchOptions::default())
        .await;
    match long_search {
        Ok(results) => assert!(results.len() >= 0),
    ...
    let unicode_search = service
        .search(unicode_query, Layer::Interact, SearchOptions::default())
        .await;
    ...
    for i in 0..10 {
        ...
        let handle = tokio::spawn(async move {
            let problematic_record =
                create_problematic_record(&format!("health_test_{}", i), "invalid_embedding");
