...
use memory::{
    service_di::{
        default_config, BatchInsertResult, BatchSearchResult, DIMemoryService, MemoryServiceConfig,
    },
    types::{Layer, Record, SearchOptions},
    CacheConfigType, DIContainer, Lifetime,
};
...
async fn test_basic_crud_operations() -> Result<()> {
    ...
    let search_results = service
        .search("Test content", Layer::Interact, SearchOptions::default())
        .await?;
    ...
    let search_results = service
        .search("Updated content", Layer::Interact, SearchOptions::default())
        .await?;
    assert_eq!(search_results.len(), 1);
...
async fn test_batch_insert_operations() -> Result<()> {
    ...
    for (i, record) in records.iter().enumerate() {
        let search_results = service
            .search(&record.content, Layer::Interact, SearchOptions::default())
            .await?;
...
async fn test_batch_search_operations() -> Result<()> {
    ...
    for query in queries.clone() {
        let search_result = service
            .search(&query, Layer::Interact, SearchOptions::default())
            .await?;
...
async fn test_multi_layer_operations() -> Result<()> {
    ...
    let wrong_layer_results = service
        .search("Interact", Layer::Insights, SearchOptions::default())
        .await?;
...
async fn test_metrics_collection() -> Result<()> {
    ...
    service.insert(record.clone()).await?;
    service
        .search("Metrics", Layer::Interact, SearchOptions::default())
        .await?;
...
async fn test_concurrent_access() -> Result<()> {
    ...
    let all_results = service
        .search("Concurrent", Layer::Interact, SearchOptions::default())
        .await?;
...
async fn test_error_handling_invalid_layer() -> Result<()> {
    ...
    let results = service
        .search("Test content", Layer::Insights, SearchOptions::default())
        .await?;
...
async fn test_search_options_functionality() -> Result<()> {
    ...
    let limited_results = service
        .search("Test content", Layer::Interact, limited_options)
        .await?;
    ...
    let scored_results = service
        .search("Test content", Layer::Interact, scored_options)
        .await?;
...
async fn test_memory_service_lifecycle() -> Result<()> {
    ...
    let results = service
        .search("Lifecycle", Layer::Interact, SearchOptions::default())
        .await?;
...
async fn test_large_batch_operations() -> Result<()> {
    ...
    let start_time = std::time::Instant::now();
    let search_result = service
        .batch_search(queries, Layer::Interact, SearchOptions::default())
        .await?;
...
async fn test_memory_service_with_custom_config() -> Result<()> {
    ...
    let results = service
        .search("Custom config", Layer::Interact, SearchOptions::default())
        .await?;
    assert_eq!(results.len(), 1);
        ...
        let handle = tokio::spawn(async move {
            match i % 4 {
                ...
                2 => {
                    ...
                    let record = create_test_record(
                        &format!("stress_{}", i),
                        &format!("Updated stress content {}", i),
                        Layer::Interact,
                    );
                ...
                3 => {
                    // Delete operation
                    service_clone
                        .delete(&format!("stress_{}", i % 10), Layer::Interact)
                        .await
                }
                _ => unreachable!(),
            }
