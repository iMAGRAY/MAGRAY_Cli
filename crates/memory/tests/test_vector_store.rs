use memory::{VectorStore, Layer, Record, HnswRsConfig};
use uuid::Uuid;
use chrono::Utc;
...
async fn test_layer_initialization() {
    ...
    for layer in [Layer::Interact, Layer::Insights, Layer::Assets] {
        ...
        assert!(result.is_ok(), "Layer {} should initialize successfully", layer.as_str());
    }
}
...
async fn test_vector_search_single() {
    ...
    let results = store.search(&query_embedding, Layer::Interact, 5).await.unwrap();
...
async fn test_vector_search_multiple() {
    ...
    let results = store.search(&query_embedding, Layer::Interact, 2).await.unwrap();
    ...
    if results.len() > 1 {
        ...
        assert!(score_ascending || score_descending, 
...
async fn test_delete_by_id() {
    ...
    // Удаляем запись
    let deleted = store.delete_by_id(&record.id, Layer::Interact).await.unwrap();
    ...
    let not_deleted = store.delete_by_id(&record.id, Layer::Interact).await.unwrap();
...
async fn test_delete_expired() {
    ...
    let deleted_count = store.delete_expired(Layer::Interact, cutoff_time).await.unwrap();
    
    assert!(deleted_count > 0, "Should delete at least one expired record");
    
    // Проверяем что старая запись удалена
    let old_not_found = store.get_by_id(&old_record.id, Layer::Interact).await.unwrap();
    assert!(old_not_found.is_none(), "Old record should be deleted");
    
    // Проверяем что новая запись осталась
    let new_found = store.get_by_id(&new_record.id, Layer::Interact).await.unwrap();
...
async fn test_search_with_invalid_embedding() {
    ...
    let result = store.search(&wrong_size_embedding, Layer::Interact, 5).await;
    
    // Должна вернуться ошибка
    assert!(result.is_err(), "Search with wrong embedding size should fail");
}
...
async fn test_concurrent_operations() {
    ...
    assert!(search_result.is_ok(), "Search after concurrent inserts should work");
}
...
async fn test_change_tracking() {
    ...
    assert!(new_version >= initial_version, "Version should increment after changes");
    
    // Получаем изменения с начальной версии
    let changes = store.get_changes_since(initial_version).await.unwrap();
    assert!(!changes.is_empty(), "Should track changes since initial version");
}