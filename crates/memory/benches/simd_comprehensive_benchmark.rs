...
use memory::{
    ...
    simd_ultra_optimized::{
        batch_cosine_distance_ultra, batch_cosine_distance_auto,
        cosine_distance_ultra_optimized, cosine_distance_auto_ultra,
        cosine_distance_scalar_optimized, cosine_distance_scalar,
        benchmark_horizontal_sum_variants, test_ultra_optimized_performance,
    },
};
...
fn bench_cosine_distance_variants(c: &mut Criterion) {
    ...
    for &dim in &gen.vector_sizes {
        ...
        group.bench_with_input(
            ...
            |bench, _| {
                bench.iter(|| {
                    black_box(cosine_distance_scalar_optimized(
                        black_box(a), 
                        black_box(b)
                    ))
                })
        ...
        group.bench_with_input(
            BenchmarkId::new("auto_ultra", dim),
            &dim,
            |bench, _| {
                bench.iter(|| {
        ...
        if is_x86_feature_detected!("avx2") && is_x86_feature_detected!("fma") {
            group.bench_with_input(
                BenchmarkId::new("avx2_ultra", dim),
                &dim,
                |bench, _| {
                    bench.iter(|| unsafe {
                        black_box(cosine_distance_ultra_optimized(
                            black_box(a), 
                            black_box(b)
                        ))
                    })
        ...
        if is_x86_feature_detected!("avx512f") && dim >= 64 && dim % 16 == 0 {
            group.bench_with_input(
                BenchmarkId::new("avx512_ultra", dim),
                &dim,
                |bench, _| {
                    bench.iter(|| {
                        black_box(unsafe {
...
fn bench_batch_cosine_distance(c: &mut Criterion) {
    ...
    for &batch_size in &gen.batch_sizes {
        for &dim in &[1024] {  // Focus на 1024D для embedding use case
...
fn bench_hnsw_search_performance(c: &mut Criterion) {
    ...
    for &k in &k_values {
        group.bench_with_input(
            BenchmarkId::new("single_search", k),
            &k,
            |bench, &k| {
                bench.iter(|| {
        ...
        group.bench_with_input(
            BenchmarkId::new("batch_search", k),
            &k,
            |bench, &k| {
                bench.iter(|| {
                    black_box(index.parallel_search(black_box(batch_queries), k))
                })
            },
        );
    }
...
fn bench_memory_alignment_performance(c: &mut Criterion) {
    ...
    for &dim in &[1024, 2048] {
        ...
        if dim % 8 == 0 {
            ...
            group.bench_with_input(
                BenchmarkId::new("simd_distance", dim),
                &dim,
                |bench, _| {
                    bench.iter(|| {
                        black_box(cosine_distance_auto_ultra(
...
fn bench_comprehensive_performance_validation(c: &mut Criterion) {
    ...
    group.bench_function("ultra_optimized_performance_test", |bench| {
        bench.iter(|| {
            black_box(test_ultra_optimized_performance().unwrap())
        })
    });
                ...
                bench.iter(|| {
                    pool.install(|| {
                        let results: Vec<f32> = vectors.par_iter()
                            .map(|query| {
                                cosine_distance_auto_ultra(query, target)
                            })
                            .collect();
                        black_box(results)
                    })
