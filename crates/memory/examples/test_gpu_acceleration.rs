...
use anyhow::Result;
use chrono::Utc;
use memory::{default_config, Layer, MemoryConfig, MemoryService, Record};
...
async fn test_memory_gpu_integration() -> Result<()> {
    ...
    info!(
        "  Cache stats - Hits: {}, Misses: {}, Size: {} bytes",
        hits, misses, size
    );
...
async fn test_performance_comparison() -> Result<()> {
    ...
    let cpu_records: Vec<Record> = test_data
        .iter()
        .map(|text| Record {
            id: Uuid::new_v4(),
            text: text.clone(),
    ...
    info!(
        "  CPU время: {:?} ({:.1} texts/sec)",
        cpu_time,
        test_data.len() as f64 / cpu_time.as_secs_f64()
    );
    ...
    let gpu_records: Vec<Record> = test_data
        .iter()
        .map(|text| Record {
            id: Uuid::new_v4(),
            text: text.clone(),
    ...
    info!(
        "  GPU время: {:?} ({:.1} texts/sec)",
        gpu_time,
        test_data.len() as f64 / gpu_time.as_secs_f64()
    );
