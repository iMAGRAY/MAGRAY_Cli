    ...
    #[cfg(target_arch = "x86_64")]
    {
        if std::arch::is_x86_feature_detected!("avx2") {
            unsafe {
                ...
                let speedup1 = original_duration.as_nanos() as f64 / optimized_duration.as_nanos() as f64;
                let speedup2 = original_duration.as_nanos() as f64 / permute_duration.as_nanos() as f64;
...
fn test_memory_alignment_impact() {
    ...
    let unaligned_a: Vec<f32> = (0..DIMENSION).map(|_| rand::random::<f32>() * 2.0 - 1.0).collect();
    let unaligned_b: Vec<f32> = (0..DIMENSION).map(|_| rand::random::<f32>() * 2.0 - 1.0).collect();
...
fn test_prefetching_effectiveness() {
    ...
    let large_vector_a: Vec<f32> = (0..DIMENSION).map(|_| rand::random::<f32>() * 2.0 - 1.0).collect();
    let large_vector_b: Vec<f32> = (0..DIMENSION).map(|_| rand::random::<f32>() * 2.0 - 1.0).collect();
    ...
    #[cfg(target_arch = "x86_64")]
    {
        if std::arch::is_x86_feature_detected!("avx2") && DIMENSION % 8 == 0 {
            let start = Instant::now();
            for _ in 0..ITERATIONS {
                let _result = unsafe { cosine_distance_avx2_ultra(&large_vector_a, &large_vector_b) };
            }
            ...
            if prefetch_duration < no_prefetch_duration {
                let speedup = no_prefetch_duration.as_nanos() as f64 / prefetch_duration.as_nanos() as f64;
...
fn test_different_vector_sizes() {
    ...
    for &size in &sizes {
        if size % 8 != 0 { continue; } // Skip sizes not divisible by 8 for AVX2
        
        let vector_a: Vec<f32> = (0..size).map(|_| rand::random::<f32>() * 2.0 - 1.0).collect();
        let vector_b: Vec<f32> = (0..size).map(|_| rand::random::<f32>() * 2.0 - 1.0).collect();
