...
use crate::dtos::{
    StoreMemoryRequest, StoreMemoryResponse, BatchStoreMemoryRequest, BatchStoreMemoryResponse,
    PromoteRecordsRequest, PromoteRecordsResponse, AnalyzePromotionRequest, AnalyzePromotionResponse,
};
use crate::use_cases::{
    StoreMemoryUseCase, PromoteRecordsUseCase,
};
...
pub trait MemoryApplicationService: Send + Sync {
    ...
    async fn store_memory_record(&self, request: StoreMemoryRequest, context: RequestContext) -> ApplicationResult<StoreMemoryResponse>;
    
    /// Store batch of memory records with transaction handling
    async fn store_memory_batch(&self, request: BatchStoreMemoryRequest, context: RequestContext) -> ApplicationResult<BatchStoreMemoryResponse>;
    
    /// Promote records with analysis and validation
    async fn promote_memory_records(&self, request: PromoteRecordsRequest, context: RequestContext) -> ApplicationResult<PromoteRecordsResponse>;
    
    /// Analyze promotion opportunities
    async fn analyze_promotion_opportunities(&self, request: AnalyzePromotionRequest, context: RequestContext) -> ApplicationResult<AnalyzePromotionResponse>;
    
    /// Complete memory workflow: store → analyze → promote (if beneficial)
    async fn complete_memory_workflow(&self, request: StoreMemoryRequest, context: RequestContext) -> ApplicationResult<CompleteWorkflowResponse>;
...
impl MemoryApplicationService for MemoryApplicationServiceImpl {
    ...
    async fn store_memory_record(&self, request: StoreMemoryRequest, context: RequestContext) -> ApplicationResult<StoreMemoryResponse> {
        ...
        match store_result {
            Ok(response) => {
                ...
                self.record_operation_success("store_memory", total_time.as_millis() as u64).await?;
    ...
    async fn store_memory_batch(&self, request: BatchStoreMemoryRequest, context: RequestContext) -> ApplicationResult<BatchStoreMemoryResponse> {
        ...
        info!("Processing batch store request: {} records", request.records.len());
        ...
        match batch_result {
            ...
            Err(e) => {
                ...
                // Record failure
                self.record_operation_failure("store_batch", &e, total_time.as_millis() as u64).await?;
    ...
    async fn promote_memory_records(&self, request: PromoteRecordsRequest, context: RequestContext) -> ApplicationResult<PromoteRecordsResponse> {
        ...
        info!("Processing promotion request with {} criteria", request.criteria.len());
        ...
        match promotion_result {
            ...
            Err(e) => {
                ...
                // Record failure
                self.record_operation_failure("promote_records", &e, total_time.as_millis() as u64).await?;
    ...
    async fn analyze_promotion_opportunities(&self, request: AnalyzePromotionRequest, context: RequestContext) -> ApplicationResult<AnalyzePromotionResponse> {
        ...
        match analysis_result {
            Ok(response) => {
                ...
                self.record_analysis_metrics(&response, total_time.as_millis() as u64).await?;
    ...
    async fn complete_memory_workflow(&self, request: StoreMemoryRequest, context: RequestContext) -> ApplicationResult<CompleteWorkflowResponse> {
        ...
        if self.config.auto_promotion_enabled {
            if self.should_analyze_for_promotion(&request, &store_response) {
                ...
                match self.analyze_promotion_opportunities(analysis_request, context.clone()).await {
                    Ok(analysis) => {
                        ...
                        if let Some(ref analysis) = promotion_analysis {
                            if self.should_auto_promote(&analysis) {
                                ...
                                match self.promote_memory_records(promotion_request, context.clone()).await {
                                    Ok(response) => {
                                        promotion_response = Some(response);
                                        workflow_recommendations.push("Record automatically promoted based on analysis".to_string());
                                    }
                                    Err(e) => {
                                        workflow_recommendations.push(format!("Auto-promotion failed: {}", e));
                                    }
                                }
                            } else {
                                workflow_recommendations.push("Record not suitable for immediate promotion".to_string());
                            }
                        }
                    }
                    Err(e) => {
                ...
                workflow_recommendations.push("Record does not qualify for promotion analysis".to_string());
            }
        } else {
    ...
    async fn health_check(&self, context: RequestContext) -> ApplicationResult<MemorySystemHealth> {
        ...
        let overall_status = self.determine_overall_health(&store_health, &promotion_health, &cache_health);
        let health_score = self.calculate_health_score(&store_health, &promotion_health, &cache_health);
        ...
        self.record_health_check_metrics(&system_health, total_time.as_millis() as u64).await?;
...
impl MemoryApplicationServiceImpl {
    ...
    fn validate_store_request(&self, request: &StoreMemoryRequest) -> ApplicationResult<()> {
        ...
        if request.content.len() > 100_000 {
            return Err(ApplicationError::validation("Content exceeds maximum size for single record"));
        }
        
        if request.tags.len() > 100 {
            return Err(ApplicationError::validation("Too many tags for single record"));
        }
    ...
    fn validate_batch_request(&self, request: &BatchStoreMemoryRequest) -> ApplicationResult<()> {
        if request.records.len() > self.config.batch_size_limit {
            return Err(ApplicationError::validation(
                format!("Batch size {} exceeds limit {}", request.records.len(), self.config.batch_size_limit)
            ));
        }
    ...
    fn validate_promotion_request(&self, request: &PromoteRecordsRequest) -> ApplicationResult<()> {
        if request.criteria.is_empty() {
            return Err(ApplicationError::validation("No promotion criteria specified"));
        }
    ...
    async fn begin_transaction(&self, context: &RequestContext) -> ApplicationResult<TransactionGuard> {
    ...
    fn create_analysis_request(&self, request: &StoreMemoryRequest, response: &StoreMemoryResponse) -> ApplicationResult<AnalyzePromotionRequest> {
        Ok(AnalyzePromotionRequest {
            layers: Some(vec![response.layer]),
            time_window_hours: Some(24),
            min_access_frequency: Some(1),
            min_score_threshold: Some(0.7),
            max_hours_since_access: Some(168), // 1 week
        })
    }
    ...
    fn should_auto_promote(&self, analysis: &AnalyzePromotionResponse) -> bool {
        ...
        analysis.candidates.iter().any(|c| c.confidence_score >= self.config.promotion_threshold)
    }
    
    /// Create promotion request from analysis
    fn create_promotion_request(&self, analysis: &AnalyzePromotionResponse) -> ApplicationResult<PromoteRecordsRequest> {
        let high_confidence_candidates: Vec<_> = analysis.candidates
            .iter()
            .filter(|c| c.confidence_score >= self.config.promotion_threshold)
            .take(10) // Limit auto-promotions
            .collect();
        
        if high_confidence_candidates.is_empty() {
            return Err(ApplicationError::business_logic("No suitable candidates for auto-promotion"));
        }
        
        // Create criteria based on analysis
        let criteria = vec![crate::dtos::PromotionCriterion {
            min_access_frequency: 1,
            min_score_threshold: self.config.promotion_threshold,
            max_hours_since_access: Some(168),
            target_layers: vec![domain::value_objects::layer_type::LayerType::Cache],
            project_filter: None,
        }];
    ...
    fn determine_overall_health(&self, store: &SubsystemHealth, promotion: &SubsystemHealth, cache: &SubsystemHealth) -> HealthStatus {
        let subsystems = [store, promotion, cache];
        
        if subsystems.iter().any(|s| matches!(s.status, HealthStatus::Critical)) {
            HealthStatus::Critical
        } else if subsystems.iter().any(|s| matches!(s.status, HealthStatus::Unhealthy)) {
            HealthStatus::Unhealthy
        } else if subsystems.iter().any(|s| matches!(s.status, HealthStatus::Degraded)) {
    ...
    fn calculate_health_score(&self, store: &SubsystemHealth, promotion: &SubsystemHealth, cache: &SubsystemHealth) -> f64 {
        let subsystems = [store, promotion, cache];
        let total_score: f64 = subsystems.iter().map(|s| match s.status {
    ...
    async fn record_operation_success(&self, operation: &str, duration_ms: u64) -> ApplicationResult<()> {
        self.metrics_collector.increment_counter(
            &format!("{}_operations_successful", operation),
            1,
            None,
        ).await?;
    ...
    async fn record_operation_failure(&self, operation: &str, error: &ApplicationError, duration_ms: u64) -> ApplicationResult<()> {
        self.metrics_collector.increment_counter(
            &format!("{}_operations_failed", operation),
            1,
            None,
        ).await?;
    ...
    async fn record_batch_metrics(&self, response: &BatchStoreMemoryResponse, duration_ms: u64) -> ApplicationResult<()> {
        ...
        self.metrics_collector.record_gauge(
            "batch_store_size",
            response.total_requested as f64,
            None,
        ).await?;
    ...
    async fn record_promotion_metrics(&self, response: &PromoteRecordsResponse, duration_ms: u64) -> ApplicationResult<()> {
        self.metrics_collector.record_gauge(
            "promotion_records_count",
            response.promoted_records.len() as f64,
            None,
        ).await?;
    ...
    async fn record_analysis_metrics(&self, response: &AnalyzePromotionResponse, duration_ms: u64) -> ApplicationResult<()> {
        self.metrics_collector.record_gauge(
    ...
    async fn record_workflow_metrics(&self, analysis_performed: bool, promotion_performed: bool, duration_ms: u64) -> ApplicationResult<()> {
        self.metrics_collector.increment_counter(
            "complete_workflow_operations",
            1,
            None,
        ).await?;
    ...
    async fn record_health_check_metrics(&self, health: &MemorySystemHealth, duration_ms: u64) -> ApplicationResult<()> {
        self.metrics_collector.record_gauge(
    ...
    async fn send_error_notification(&self, operation: &str, error: &ApplicationError, context: &RequestContext) -> ApplicationResult<()> {
    ...
    async fn send_batch_success_notification(&self, response: &BatchStoreMemoryResponse, context: &RequestContext) -> ApplicationResult<()> {
        use crate::ports::{Notification, NotificationLevel};
        
        let notification = Notification::info(
            "Large Batch Store Completed",
            &format!("Successfully processed {} out of {} records", response.successful, response.total_requested),
        );
    ...
    async fn send_promotion_success_notification(&self, response: &PromoteRecordsResponse, context: &RequestContext) -> ApplicationResult<()> {
        use crate::ports::{Notification, NotificationLevel};
        
        let notification = Notification::info(
            "Bulk Promotion Completed",
            &format!("Promoted {} records between memory layers", response.promoted_records.len()),
        );
