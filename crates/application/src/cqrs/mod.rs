...
/// Implement CommandHandler for Arc<T> where T: CommandHandler<C>
#[async_trait]
impl<C: Command + 'static, T: CommandHandler<C> + Send + Sync + 'static> CommandHandler<C>
    for Arc<T>
{
    async fn handle(&self, command: C, context: RequestContext) -> ApplicationResult<C::Response> {
        (**self).handle(command, context).await
    }

    async fn before_handle(&self, command: &C, context: &RequestContext) -> ApplicationResult<()> {
        (**self).before_handle(command, context).await
    }
    ...
    async fn after_handle(
        ...
        (**self).after_handle(command, response, context).await
    }
}

/// Implement QueryHandler for Arc<T> where T: QueryHandler<Q>
#[async_trait]
impl<Q: Query + 'static, T: QueryHandler<Q> + Send + Sync + 'static> QueryHandler<Q> for Arc<T> {
    async fn handle(&self, query: Q, context: RequestContext) -> ApplicationResult<Q::Response> {
        (**self).handle(query, context).await
    }

    async fn before_handle(&self, query: &Q, context: &RequestContext) -> ApplicationResult<()> {
        (**self).before_handle(query, context).await
    }
    ...
    async fn after_handle(
        ...
        (**self).after_handle(query, response, context).await
    }
}
...
pub struct CqrsBus {
    ...
    metrics: Arc<dyn crate::ports::MetricsCollector>,
}

impl CqrsBus {
    pub fn new(metrics: Arc<dyn crate::ports::MetricsCollector>) -> Self {
        Self {
    ...
    pub fn register_command_handler<C: Command + 'static, H: CommandHandler<C> + 'static>(
        ...
        self.command_handlers
            .insert(command_id, Box::new(Arc::new(handler)));
    }
    ...
    pub fn register_query_handler<Q: Query + 'static, H: QueryHandler<Q> + 'static>(
        ...
        self.query_handlers
            .insert(query_id, Box::new(Arc::new(handler)));
    }

    /// Send command for execution
    #[instrument(skip(self, command, context), fields(command_id = command.command_id()))]
    pub async fn send_command<C: Command + 'static>(
        &self,
        command: C,
        context: RequestContext,
        ...
        let handler_any = self.command_handlers.get(type_id).ok_or_else(|| {
            crate::ApplicationError::infrastructure(format!(
        ...
        let handler = handler_any
            ...
            .ok_or_else(|| {
                crate::ApplicationError::infrastructure(format!(
                    "Handler type mismatch for command: {}",
                    command_id
                ))
            })?;
        ...
        self.record_command_metrics(command_id, duration.as_millis() as u64, result.is_ok())
            .await?;
    ...
    pub async fn send_query<Q: Query + 'static>(
        &self,
        query: Q,
        context: RequestContext,
    ) -> ApplicationResult<Q::Response> {
        let start_time = std::time::Instant::now();
        ...
        let handler_any = self.query_handlers.get(type_id).ok_or_else(|| {
            crate::ApplicationError::infrastructure(format!(
        ...
        let handler = handler_any
            ...
            .ok_or_else(|| {
                crate::ApplicationError::infrastructure(format!(
                    "Handler type mismatch for query: {}",
                    query_id
                ))
            })?;
        ...
        // Record metrics
        let duration = start_time.elapsed();
        self.record_query_metrics(query_id, duration.as_millis() as u64, result.is_ok())
            .await?;

        result
    }

    async fn record_command_metrics(
        &self,
        command_id: &str,
        duration_ms: u64,
        success: bool,
        ...
        self.metrics
            .increment_counter("cqrs_commands_total", 1, Some(&tags))
            .await?;
        self.metrics
            .record_timing("cqrs_command_duration", duration_ms, Some(&tags))
            .await?;
    ...
    async fn record_query_metrics(
        ...
        self.metrics
            .increment_counter("cqrs_queries_total", 1, Some(&tags))
            .await?;
        self.metrics
            .record_timing("cqrs_query_duration", duration_ms, Some(&tags))
            .await?;
...
macro_rules! impl_cqrs {
    (command $name:ident => $response:ty) => {
        ...
        impl crate::cqrs::Command for $name {
            ...
            ) -> crate::ApplicationResult<Self::Response> {
