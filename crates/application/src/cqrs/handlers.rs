...
use super::commands::*;
use super::queries::*;
use super::{Command, CommandHandler, Query, QueryHandler};
...
impl StoreMemoryCommandHandler {
    pub fn new(use_case: Arc<dyn StoreMemoryUseCase>, metrics: Arc<dyn MetricsCollector>) -> Self {
...
impl CommandHandler<StoreMemoryCommand> for StoreMemoryCommandHandler {
    ...
    async fn handle(
    ...
    ) -> ApplicationResult<crate::dtos::StoreMemoryResponse> {
        let start_time = std::time::Instant::now();

        info!(
            "Handling StoreMemoryCommand for request: {}",
            context.request_id
        );
        ...
        let request = crate::dtos::StoreMemoryRequest {
            ...
            project: command.project.unwrap_or_default(),
            kind: None,    // Default - could be inferred from command if needed
            session: None, // Default - could be set from context if needed
        ...
        match &result {
            Ok(response) => {
                ...
                self.record_success_metrics("store_memory", duration.as_millis() as u64, &response)
                    .await?;
            ...
            Err(e) => {
                ...
                self.record_failure_metrics("store_memory", duration.as_millis() as u64, e)
                    .await?;
    ...
    async fn after_handle(
        &self,
        command: &StoreMemoryCommand,
        response: &crate::dtos::StoreMemoryResponse,
        context: &RequestContext,
    ) -> ApplicationResult<()> {
        // Post-execution actions
        info!(
            "Stored record {} with {} dimensions on layer {:?}",
            response.record_id, response.embedding_dimensions, response.layer
        );
...
impl CommandHandler<BatchStoreMemoryCommand> for BatchStoreMemoryCommandHandler {
    ...
    async fn handle(
        &self,
        command: BatchStoreMemoryCommand,
        context: RequestContext,
    ) -> ApplicationResult<crate::dtos::BatchStoreMemoryResponse> {
        let start_time = std::time::Instant::now();

        info!(
            "Handling BatchStoreMemoryCommand with {} records",
            command.records.len()
        );
        ...
        match &result {
            Ok(response) => {
                ...
                self.record_batch_metrics(response, duration.as_millis() as u64)
                    .await?;
...
impl CommandHandler<PromoteRecordsCommand> for PromoteRecordsCommandHandler {
    ...
    async fn handle(
        &self,
        command: PromoteRecordsCommand,
        context: RequestContext,
    ) -> ApplicationResult<crate::dtos::PromoteRecordsResponse> {
        let start_time = std::time::Instant::now();

        info!(
            "Handling PromoteRecordsCommand (dry_run: {})",
            command.dry_run
        );
        ...
        let request = crate::dtos::PromoteRecordsRequest {
            ...
            force: false,
            from_layer: None,
            record_ids: Some(Vec::new()),
            to_layer: LayerType::Insights,
        };

        let result = self
            .use_case
            .promote_records(request, context.clone())
            .await;
...
impl SearchMemoryQueryHandler {
    pub fn new(use_case: Arc<dyn SearchMemoryUseCase>, metrics: Arc<dyn MetricsCollector>) -> Self {
        Self { use_case, metrics }
    }
}
...
impl QueryHandler<SearchMemoryQuery> for SearchMemoryQueryHandler {
    ...
    async fn handle(
        ...
        context: RequestContext,
    ) -> ApplicationResult<crate::dtos::SearchMemoryResponse> {
        let start_time = std::time::Instant::now();
        ...
        let request = crate::dtos::SearchMemoryRequest {
            query: query.query,
            layers: Some(query.layers),
            limit: Some(query.limit),
            include_embeddings: query.include_embeddings,
            project_filter: query.project_filter,
            filters: None, // Convert tag_filters if needed
...
impl QueryHandler<SimilaritySearchQuery> for SimilaritySearchQueryHandler {
    ...
    async fn handle(
        ...
        context: RequestContext,
    ) -> ApplicationResult<crate::dtos::SimilaritySearchResponse> {
        let start_time = std::time::Instant::now();

        info!(
            "Handling SimilaritySearchQuery with {} dimensions",
        ...
        let result = self
            .use_case
            .similarity_search(request, context.clone())
            .await;
...
impl QueryHandler<RetrieveMemoryQuery> for RetrieveMemoryQueryHandler {
    ...
    async fn handle(
        ...
        context: RequestContext,
    ) -> ApplicationResult<crate::dtos::RetrieveMemoryResponse> {
        let start_time = std::time::Instant::now();

        info!(
            "Handling RetrieveMemoryQuery for record: {}",
            query.record_id
        );
        ...
        let result = self
            .use_case
            .retrieve_memory(request, context.clone())
            .await;
        ...
        match &result {
            Ok(response) => {
                ...
                // Record query metrics
                self.record_query_metrics("retrieve", duration.as_millis() as u64, 1)
                    .await?;
            }
            Err(e) => {
...
impl AnalyzeUsageQueryHandler {
    pub fn new(use_case: Arc<dyn AnalyzeUsageUseCase>, metrics: Arc<dyn MetricsCollector>) -> Self {
        Self { use_case, metrics }
    }
}
...
impl QueryHandler<AnalyzeUsageQuery> for AnalyzeUsageQueryHandler {
    ...
    async fn handle(
        &self,
        query: AnalyzeUsageQuery,
        context: RequestContext,
    ) -> ApplicationResult<crate::dtos::AnalyzeUsageResponse> {
        let start_time = std::time::Instant::now();
        ...
        let request = crate::dtos::AnalyzeUsageRequest {
            time_window_hours: query.time_window_hours.unwrap_or(24) as u32,
            layers: query.layers,
            project_filter: query.project_filter,
        ...
        let result = self
            .use_case
            .analyze_usage_patterns(request, context.clone())
            .await;
...
impl StoreMemoryCommandHandler {
    async fn record_success_metrics(
        ...
        self.metrics
            .increment_counter("command_executions_successful", 1, Some(&tags))
            .await?;
        self.metrics
            .record_timing("command_duration", duration_ms, Some(&tags))
            .await?;
        self.metrics
            .record_gauge(
                "embedding_dimensions",
                response.embedding_dimensions as f64,
                Some(&tags),
            )
            .await?;
    ...
    async fn record_failure_metrics(
        ...
        self.metrics
            .increment_counter("command_executions_failed", 1, Some(&tags))
            .await?;
        self.metrics
            .record_timing("command_duration", duration_ms, Some(&tags))
            .await?;
...
impl BatchStoreMemoryCommandHandler {
    async fn record_batch_metrics(
        &self,
        response: &crate::dtos::BatchStoreMemoryResponse,
        duration_ms: u64,
    ) -> ApplicationResult<()> {
        ...
        self.metrics
            .increment_counter("batch_operations", 1, Some(&tags))
            .await?;
        self.metrics
            .record_gauge("batch_success_rate", success_rate, Some(&tags))
            .await?;
        self.metrics
            .record_gauge("batch_size", response.total_requested as f64, Some(&tags))
            .await?;
        self.metrics
            .record_timing("batch_duration", duration_ms, Some(&tags))
            .await?;
...
impl SearchMemoryQueryHandler {
    async fn record_query_metrics(
        ...
        self.metrics
            .increment_counter("query_executions", 1, Some(&tags))
            .await?;
        self.metrics
            .record_timing("query_duration", duration_ms, Some(&tags))
            .await?;
        self.metrics
            .record_gauge("query_result_count", result_count as f64, Some(&tags))
            .await?;
...
impl HandlerRegistry {
    pub fn new(
        ...
        Self {
            store_memory_handler: Arc::new(StoreMemoryCommandHandler::new(
                store_use_case.clone(),
                metrics.clone(),
            )),
            batch_store_handler: Arc::new(BatchStoreMemoryCommandHandler::new(
                store_use_case.clone(),
                metrics.clone(),
            )),
            promote_records_handler: Arc::new(PromoteRecordsCommandHandler::new(
                promotion_use_case,
                metrics.clone(),
            )),
            search_memory_handler: Arc::new(SearchMemoryQueryHandler::new(
                search_use_case.clone(),
                metrics.clone(),
            )),
            similarity_search_handler: Arc::new(SimilaritySearchQueryHandler::new(
                search_use_case.clone(),
                metrics.clone(),
            )),
            retrieve_memory_handler: Arc::new(RetrieveMemoryQueryHandler::new(
                store_use_case,
                metrics.clone(),
            )),
            analyze_usage_handler: Arc::new(AnalyzeUsageQueryHandler::new(
                analytics_use_case,
                metrics,
            )),
        }
    ...
    pub fn register_all_handlers(&self, bus: &mut super::CqrsBus) {
        ...
        bus.register_command_handler(self.store_memory_handler.clone());
        bus.register_command_handler(self.batch_store_handler.clone());
        bus.register_command_handler(self.promote_records_handler.clone());

        // Register query handlers
        bus.register_query_handler(self.search_memory_handler.clone());
        bus.register_query_handler(self.similarity_search_handler.clone());
        bus.register_query_handler(self.retrieve_memory_handler.clone());
        bus.register_query_handler(self.analyze_usage_handler.clone());
    }
}
