...
use crate::dtos::{
    BatchStoreMemoryRequest, BatchStoreMemoryResponse, RetrieveMemoryRequest,
    RetrieveMemoryResponse, StoreMemoryRequest, StoreMemoryResponse,
};
use crate::ports::{EmbeddingProvider, MetricsCollector, MetricsCollectorExt, NotificationService};
...
impl StoreMemoryUseCase for StoreMemoryUseCaseImpl {
    ...
    async fn store_memory(
        ...
        info!(
            "Starting store memory operation for request: {}",
            context.request_id
        );
        ...
        // Save dimensions before moving embedding
        let embedding_dimensions = embedding.dimensions().len();
        ...
        let memory_record =
            self.create_memory_record(record_id.clone(), &request, embedding, target_layer)?;

        // Store in repository
        let store_start = std::time::Instant::now();
        self.memory_repository
            .store(memory_record.clone())
            .await
            .map_err(|e| {
                ApplicationError::infrastructure_with_source(
                    "Failed to store memory record",
                    Box::new(e),
                )
            })?;
        ...
        // Record metrics
        self.record_store_metrics(&memory_record, total_time, embedding_time, store_time)
            .await?;

        if request.priority.unwrap_or(1) >= 3 {
            self.send_store_notification(&memory_record, &context)
                .await?;
        }
        ...
        info!(
            "Successfully stored memory record: {} in layer: {:?}",
            record_id, target_layer
        );
    ...
    async fn store_batch_memory(
        ...
        info!(
            "Starting batch store operation for {} records",
            request.records.len()
        );
        ...
        if request.options.parallel_processing {
            results = self
                .process_batch_parallel(&request.records, &context)
                .await?;
        } else {
            results = self
                .process_batch_sequential(&request.records, &context)
                .await?;
        }
        ...
        self.record_batch_metrics(request.records.len(), successful, failed, total_time)
            .await?;

        self.send_batch_notification(request.records.len(), successful, failed, &context)
            .await?;
        ...
        info!(
            "Batch store completed: {}/{} successful",
            successful,
            request.records.len()
        );
    ...
    async fn retrieve_memory(
        ...
        // Parse record ID
        let record_id = RecordId::from_string(&request.record_id).map_err(|e| {
            ApplicationError::validation(format!("Invalid record ID format: {}", e))
        })?;
        ...
        let memory_record = self
            ...
            .ok_or_else(|| ApplicationError::not_found("MemoryRecord", &request.record_id))?;

        let total_time = start_time.elapsed();

        // Record metrics
        self.record_retrieve_metrics(&memory_record, total_time, request.include_stats)
            .await?;

        // Build response
        let response = RetrieveMemoryResponse {
            record_id: memory_record.id().to_string(),
            content: memory_record.content().to_string(),
            metadata: None, // TODO: Add metadata support to MemoryRecord
            layer: memory_record.layer(),
            created_at: memory_record.created_at(),
            last_accessed: memory_record.created_at(), // Use created_at as fallback
            access_count: memory_record.access_pattern().access_count() as u64,
            embedding: if request.include_embedding {
                Some(Vec::new()) // TODO: Add embedding support to MemoryRecord
            } else {
                None
            },
            stats: if request.include_stats {
                Some(crate::dtos::RecordStats {
                    ...
                    cache_hit: matches!(memory_record.layer(), LayerType::Interact),
        ...
        info!(
            "Successfully retrieved memory record: {}",
            request.record_id
        );
...
impl StoreMemoryUseCaseImpl {
    ...
    fn validate_batch_request(&self, request: &BatchStoreMemoryRequest) -> ApplicationResult<()> {
        ...
        if request.records.len() > 100 {
            return Err(ApplicationError::validation(
                "Too many records in batch (max 100)",
            ));
        }
    ...
    async fn record_batch_metrics(
        ...
        self.metrics_collector
            .increment_counter("batch_store_operations_total", 1, None)
            .await?;
        ...
        self.metrics_collector
            .record_timing("batch_store_duration", total_time.as_millis() as u64, None)
            .await?;
    ...
    async fn send_store_notification(
        &self,
        record: &MemoryRecord,
        context: &RequestContext,
    ) -> ApplicationResult<()> {
        use crate::ports::{Notification, NotificationCategory, NotificationLevel};

        let notification = Notification::info(
            "High Priority Memory Record Stored",
            &format!(
                "Record {} stored in {:?} layer",
                record.id(),
                record.layer()
            ),
        )
    ...
    async fn is_promotion_candidate(&self, record: &MemoryRecord) -> ApplicationResult<bool> {
        ...
        self.memory_domain_service
            .is_promotion_candidate(
                record,
                record.access_pattern().access_count() as f64,
                &chrono::Utc::now(),
            )
            .await
            .map_err(|e| ApplicationError::Domain(e))
    ...
    fn estimate_retrieval_time(&self, layer: LayerType) -> u64 {
        match layer {
            LayerType::Interact => 5,  // 5ms
            LayerType::Insights => 25, // 25ms
            LayerType::Assets => 100,  // 100ms
        }
    }
}
