...
use domain::services::memory_domain_service::MemoryDomainServiceTrait;
...
pub trait AnalyzeUsageUseCase: Send + Sync {
    ...
    ) -> ApplicationResult<AnalyzeUsageResponse>;
...
impl AnalyzeUsageUseCase for AnalyzeUsageUseCaseImpl {
    ...
    async fn analyze_usage_patterns(
        ...
        info!(
            "Starting usage patterns analysis for request: {}",
            context.request_id
        );
        ...
        let start_time_analysis =
            end_time - std::time::Duration::from_secs(request.time_window_hours as u64 * 3600);
        ...
        let layer_analysis = self
            .analyze_layer_performance(&usage_stats, &request)
            .await?;
        ...
        let recommendations = self
            .generate_optimization_recommendations(&usage_stats, &efficiency_metrics)
            .await?;

        let total_time = start_time.elapsed();

        // Record analysis metrics
        self.record_usage_analysis_metrics(&usage_stats, total_time)
            .await?;
    ...
    async fn generate_insights(
        ...
        info!(
            "Starting insights generation for request: {}",
            context.request_id
        );
        ...
        for insight_type in &request.insight_types {
            match insight_type {
                crate::dtos::analytics::InsightType::Performance
                | crate::dtos::analytics::InsightType::PerformanceTrends => {
                    let performance_insights = self.generate_performance_insights(&request).await?;
                    insights.extend(performance_insights);
                }
                crate::dtos::analytics::InsightType::Usage
                | crate::dtos::analytics::InsightType::UsageAnomalies => {
                    let anomaly_insights = self.generate_anomaly_insights(&request).await?;
                    insights.extend(anomaly_insights);
                }
                crate::dtos::analytics::InsightType::Optimization
                | crate::dtos::analytics::InsightType::OptimizationOpportunities => {
                    let optimization_insights =
                        self.generate_optimization_insights(&request).await?;
                    insights.extend(optimization_insights);
                }
                crate::dtos::analytics::InsightType::Anomaly => {
                    let resource_insights = self.generate_resource_insights(&request).await?;
                    insights.extend(resource_insights);
                }
                crate::dtos::analytics::InsightType::Trend => {
                    let predictive_insights = self.generate_predictive_insights(&request).await?;
                    insights.extend(predictive_insights);
                }
                crate::dtos::analytics::InsightType::Prediction
                | crate::dtos::analytics::InsightType::PredictiveAnalysis => {
                    let predictive_insights = self.generate_predictive_insights(&request).await?;
                    insights.extend(predictive_insights);
                }
                crate::dtos::analytics::InsightType::PerformanceTrends => {
                ...
                crate::dtos::analytics::InsightType::ResourceUtilization => {
                    let resource_insights = self.generate_resource_insights(&request).await?;
                    insights.extend(resource_insights);
                }
            }
        ...
        self.record_insights_metrics(insights.len(), total_time)
            .await?;

        // Calculate confidence before moving insights
        let confidence_score = self.calculate_overall_confidence(&insights);
    ...
    async fn get_system_statistics(
        ...
        info!(
            "Collecting system statistics for request: {}",
            context.request_id
        );
        ...
        let statistics = crate::dtos::SystemStatistics {
            ...
            average_search_time_ms: domain_stats.average_response_time_ms,
            memory_usage_mb: domain_stats.memory_utilization_mb,
        ...
        self.record_statistics_collection_metrics(total_time)
            .await?;

        info!(
            "System statistics collected in {}ms",
    ...
    async fn generate_health_report(
        ...
        info!(
            "Generating health report for request: {}",
            context.request_id
        );
        ...
        let health_recommendations = self
            .generate_health_recommendations(&health_data, &critical_issues)
            .await?;
        ...
        if health_score < 0.7 || has_critical_issues {
...
impl AnalyzeUsageUseCaseImpl {
    ...
    fn validate_insights_request(
        ...
        if request.insight_types.len() > 10 {
            return Err(ApplicationError::validation(
                "Too many insight types requested (max 10)",
            ));
        }
    ...
    async fn analyze_layer_performance(
        ...
        for (layer, layer_stats) in &usage_stats.layer_statistics {
            let performance = crate::dtos::LayerPerformance {
                hit_rate: layer_stats.hit_rate as f32,
                average_response_time_ms: layer_stats.average_response_time_ms,
                error_rate: layer_stats.error_rate as f32,
                throughput_ops_per_second: layer_stats.throughput_qps,
                capacity_utilization: layer_stats.utilization_percentage as f32,
                performance_trend: vec![], // Initialize with empty trend data
            };
    ...
    async fn generate_access_patterns(
        ...
        for pattern in &usage_stats.access_patterns {
            let access_pattern = crate::dtos::AccessPattern {
                ...
                frequency: pattern.frequency as u64,
    ...
    async fn calculate_efficiency_metrics(
        ...
        let cache_efficiency = usage_stats
            .layer_statistics
            .get(&LayerType::Interact)
            .map(|stats| stats.hit_rate)
            .unwrap_or(0.0);

        let overall_response_time = usage_stats.overall_average_response_time_ms;

        let resource_utilization = usage_stats
            .layer_statistics
            .values()
            .map(|stats| stats.utilization_percentage)
            .sum::<f64>()
            / usage_stats.layer_statistics.len() as f64;

        let throughput_efficiency =
            usage_stats.total_requests as f64 / usage_stats.time_window_hours as f64;
    ...
    async fn generate_optimization_recommendations(
        ...
        if efficiency_metrics.cache_efficiency < 0.7 {
            recommendations.push(crate::dtos::OptimizationRecommendation {
                ...
                estimated_impact: crate::dtos::ImpactEstimate {
                    performance_impact: "25% improvement expected".to_string(),
                    cost_impact: "15% cost reduction".to_string(),
                    user_experience_impact: "Medium improvement".to_string(),
                    quantified_benefit: Some(25.0),
                },
                action_items: vec![
        ...
        if efficiency_metrics.overall_response_time > 100.0 {
            recommendations.push(crate::dtos::OptimizationRecommendation {
                ...
                estimated_impact: crate::dtos::ImpactEstimate {
                    performance_impact: "40% improvement expected".to_string(),
                    cost_impact: "10% cost reduction".to_string(),
                    user_experience_impact: "High improvement".to_string(),
                    quantified_benefit: Some(40.0),
                },
                action_items: vec![
        ...
        if efficiency_metrics.resource_utilization > 0.9 {
            recommendations.push(crate::dtos::OptimizationRecommendation {
                ...
                estimated_impact: crate::dtos::ImpactEstimate {
                    performance_impact: "20% improvement expected".to_string(),
                    cost_impact: "25% cost increase (more resources needed)".to_string(),
                    user_experience_impact: "Significant improvement".to_string(),
                    quantified_benefit: Some(20.0),
                },
                action_items: vec![
    ...
    async fn generate_performance_insights(
        &self,
        request: &GenerateInsightsRequest,
    ) -> ApplicationResult<Vec<crate::dtos::Insight>> {
        // Implementation would analyze performance trends from metrics collector
        Ok(vec![crate::dtos::Insight {
            insight_type: crate::dtos::InsightType::Performance,
            title: "Search Performance Improving".to_string(),
            description: "Search response times have improved by 15% over the last 24 hours"
                .to_string(),
    ...
    async fn generate_resource_insights(
        &self,
        request: &GenerateInsightsRequest,
    ) -> ApplicationResult<Vec<crate::dtos::Insight>> {
    ...
    async fn identify_critical_issues(
        ...
        for issue in &health_data.critical_issues {
            let critical_issue = crate::dtos::CriticalIssue {
                ...
                severity: crate::dtos::IssueSeverity::Medium,
    ...
    fn calculate_health_score(
        ...
        let base_score = health_data
            .components
            .values()
            .map(|c| c.health_score)
            .sum::<f64>()
            / health_data.components.len() as f64;

        let critical_penalty = critical_issues.len() as f64 * 0.1;

        (base_score - critical_penalty).clamp(0.0, 1.0)
    }
    ...
    async fn send_health_alert(
        &self,
        report: &crate::dtos::HealthReport,
        context: &RequestContext,
    ) -> ApplicationResult<()> {
    ...
    async fn record_usage_analysis_metrics(
        ...
        self.metrics_collector
            .increment_counter("usage_analysis_operations_total", 1, None)
            .await?;
    ...
    async fn record_insights_metrics(
        &self,
        insights_count: usize,
        total_time: std::time::Duration,
    ) -> ApplicationResult<()> {
        self.metrics_collector
            .increment_counter("insights_generation_operations_total", 1, None)
            .await?;

        self.metrics_collector
            .record_gauge("insights_generated", insights_count as f64, None)
            .await?;

        Ok(())
    }

    async fn record_statistics_collection_metrics(
        &self,
        total_time: std::time::Duration,
    ) -> ApplicationResult<()> {
        self.metrics_collector
