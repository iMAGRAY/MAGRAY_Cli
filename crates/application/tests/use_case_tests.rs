...
use application::{RequestContext, RequestSource, ApplicationResult};
...
impl StoreMemoryUseCase for MockStoreMemoryUseCase {
    async fn store_memory(&self, request: StoreMemoryRequest, _context: RequestContext) -> ApplicationResult<StoreMemoryResponse> {
        if self.should_fail {
            return Err(application::ApplicationError::infrastructure("Mock failure"));
        }
    ...
    async fn store_batch_memory(&self, request: BatchStoreMemoryRequest, _context: RequestContext) -> ApplicationResult<BatchStoreMemoryResponse> {
        if self.should_fail {
            return Err(application::ApplicationError::infrastructure("Mock batch failure"));
        }

        let total = request.records.len();
        let results: Vec<BatchStoreResult> = (0..total).map(|i| BatchStoreResult {
            index: i,
            success: true,
    ...
    async fn retrieve_memory(&self, request: RetrieveMemoryRequest, _context: RequestContext) -> ApplicationResult<RetrieveMemoryResponse> {
        if self.should_fail {
...
impl SearchMemoryUseCase for MockSearchMemoryUseCase {
    async fn search_memory(&self, request: SearchMemoryRequest, _context: RequestContext) -> ApplicationResult<SearchMemoryResponse> {
    ...
    async fn similarity_search(&self, request: SimilaritySearchRequest, _context: RequestContext) -> ApplicationResult<SimilaritySearchResponse> {
        Ok(SimilaritySearchResponse {
            results: vec![
                SimilarityResult {
                    ...
                    layer: domain::value_objects::layer_type::LayerType::Index,
                }
            ],
            search_time_ms: 25,
            total_found: 1,
    ...
    async fn get_cached_search(&self, _query_hash: &str, _context: RequestContext) -> ApplicationResult<Option<SearchMemoryResponse>> {
        Ok(None)
    }
}
...
async fn test_search_memory_use_case() {
    ...
    let request = SearchMemoryRequest {
        ...
        layers: vec![
            domain::value_objects::layer_type::LayerType::Cache,
            domain::value_objects::layer_type::LayerType::Index,
        ],
        limit: 10,
...
async fn test_similarity_search_use_case() {
    ...
    let request = SimilaritySearchRequest {
        ...
        limit: 5,
        threshold: Some(0.8),
        layers: vec![domain::value_objects::layer_type::LayerType::Index],
    ...
    assert!(response.results[0].embedding.is_some());
}
