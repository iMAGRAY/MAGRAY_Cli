use crate::{AiError, Result, RerankingConfig};
...
impl OptimizedRerankingService {
    ...
    pub async fn new(config: RerankingConfig) -> Result<Self> {
                ...
                match gpu_config.create_providers() {
                    Ok(providers) => {
                        if !providers.is_empty() {
                            info!("🚀 Добавляем {} GPU провайдеров для реранкера", providers.len());
        ...
        info!("✅ Reranker model loaded: {} inputs, {} outputs", inputs, outputs);
