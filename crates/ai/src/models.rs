...
impl OnnxSession {
    ...
    pub fn new(model_name: String, model_path: PathBuf, use_gpu: bool) -> Result<Self> {
        ...
        match Self::create_real_session(&model_name, &model_path, use_gpu) {
            Ok(session_info) => {
                info!(
                    "✅ Successfully created real ONNX session for: {}",
                    model_name
                );
                Ok(Self {
                    model_name,
        ...
        match OnnxSession::new(model_name.to_string(), model_path.clone(), use_gpu) {
            Ok(session) => {
                if session.is_fallback() {
                    info!(
                        "⚠️  Loaded ONNX model in fallback mode: {} (reason: {:?})",
                        model_name,
                        session.get_fallback_reason()
                    );
                } else {
...
mod tests {
    ...
    fn test_onnx_session_model_not_found() {
        ...
        let result = OnnxSession::new("test_model".to_string(), nonexistent_path, false);
    ...
    fn test_as_ref_implementation() {
        ...
        let session =
            OnnxSession::new_fallback("test".to_string(), model_path.clone(), "Test".to_string());
