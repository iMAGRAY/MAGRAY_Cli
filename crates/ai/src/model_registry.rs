...
impl ModelRegistry {
    ...
    pub fn register_model(&mut self, model: ModelInfo) {
        info!("📝 Регистрируем модель: {} ({:?})", model.name, model.model_type);
    ...
    pub fn diagnose_models(&self) -> Result<()> {
        ...
        if let Some(default_embedding) = self.get_default_model(ModelType::Embedding) {
            if self.is_model_available(&default_embedding.name) {
                info!("✅ Embedding модель по умолчанию доступна: {}", default_embedding.name);
            } else {
                warn!("⚠️ Embedding модель по умолчанию недоступна: {}", default_embedding.name);
            }
        }
        
        if let Some(default_reranker) = self.get_default_model(ModelType::Reranker) {
            if self.is_model_available(&default_reranker.name) {
                info!("✅ Reranker модель по умолчанию доступна: {}", default_reranker.name);
            } else {
                warn!("⚠️ Reranker модель по умолчанию недоступна: {}", default_reranker.name);
            }
        }
        ...
        if old_models_dir.exists() {
            ...
            warn!("   Рекомендуется переместить модели в централизованную папку: {:?}", self.models_dir);
        }
