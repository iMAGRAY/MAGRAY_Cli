...
impl GpuEmbeddingService {
    pub async fn new(config: EmbeddingConfig) -> Result<Self> {
        ...
        let (optimal_batch_size, use_gpu) = if config.use_gpu && detector.available {
            ...
            info!(
                "  - Batch size: {} (рекомендовано: {})",
                gpu_batch_size, optimal_params.batch_size
            );
            info!("  - Max sequence: {}", optimal_params.max_sequence_length);
            info!("  - FP16: {}", optimal_params.use_fp16);
            info!(
                "  - GPU memory: {}MB available",
                detector
                    .devices
                    .first()
    ...
    pub async fn embed_batch(&self, texts: Vec<String>) -> Result<Vec<Vec<f32>>> {
        ...
        debug!(
            "🎯 Обработка {} текстов батчами по {}",
            num_texts, effective_batch_size
        );
        ...
        metrics.avg_batch_size = (metrics.avg_batch_size
            * (metrics.total_requests - num_texts as u64) as f32
            + self.optimal_batch_size as f32 * num_texts as f32)
            / metrics.total_requests as f32;

        info!(
            "⚡ Обработано {} текстов за {}ms ({:.1} tokens/sec)",
            num_texts,
            elapsed,
    ...
    async fn process_batch(&self, texts: Vec<String>) -> Result<Vec<Vec<f32>>> {
        ...
        debug!(
            "🏊 Processing batch of {} texts with memory pooling",
            batch_size
        );
        ...
        let result = GPU_MEMORY_POOL
            .with_buffer_async(tensor_size * 3, |buffer| async move {
                // Используем batch tokenization для эффективности
                let text_refs: Vec<&str> = texts.iter().map(|s| s.as_str()).collect();
                let mut batch_tokenized = self
                    .tokenizer
                ...
                self.tokenizer
                    .pad_batch(&mut batch_tokenized, Some(self.max_length))
                ...
                let mut all_attention_mask: Vec<i64> =
                    Vec::with_capacity(batch_size * self.max_length);
                let mut all_token_type_ids: Vec<i64> =
                    Vec::with_capacity(batch_size * self.max_length);
                ...
                let input_ids_array =
                    Array2::from_shape_vec((batch_size, self.max_length), all_input_ids)?;
                ...
                let token_type_ids_tensor =
                    Tensor::from_array((token_type_ids_shape, token_type_ids_data))?;
