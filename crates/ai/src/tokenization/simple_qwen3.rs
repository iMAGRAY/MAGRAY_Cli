...
impl SimpleQwen3Tokenizer {
    ...
    fn is_good_subword(&self, subword: &str) -> bool {
        ...
        let last_char = subword
            ...
            .unwrap_or(' ');
        let vowels = [
            'a', 'e', 'i', 'o', 'u', 'y', 'а', 'е', 'и', 'о', 'у', 'ы', 'э', 'ю', 'я',
        ];

        // Проверяем общие префиксы/суффиксы
        let common_endings = [
            "ing", "ed", "er", "ly", "tion", "ness", "ment", "ный", "ком", "тся", "ать", "ить",
            "еть",
        ];
        let common_prefixes = [
            "un", "re", "pre", "de", "dis", "не", "пре", "при", "раз", "без",
        ];

        vowels.contains(&last_char)
            || common_endings
                .iter()
                .any(|&ending| subword.ends_with(ending))
            || common_prefixes
                .iter()
                .any(|&prefix| subword.starts_with(prefix))
    }
    ...
    pub fn pad_batch(
        &self,
        batch: &mut super::BatchTokenized,
