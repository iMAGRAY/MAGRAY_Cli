...
use chrono::Utc;
use orchestrator::agents::intent_analyzer::{Intent, IntentContext, IntentType};
...
async fn test_planner_with_intelligent_tool_selection() {
    ...
    assert!(
        ...
        "Planner with intelligent tool selection should be created successfully"
    );

    let planner = planner_result.unwrap();
    assert!(
        planner.has_intelligent_tool_selection(),
        "Planner should have intelligent tool selection enabled"
    );
}
...
async fn test_intelligent_plan_building() {
    ...
    assert!(
        plan.is_ok(),
        "Plan building with intelligent tool selection should succeed"
    );

    let plan = plan.unwrap();
    assert_eq!(
        plan.intent_id, intent.id,
        "Plan should reference the correct intent"
    );
    ...
    assert!(
        plan.metadata.contains_key("intelligent_selection_used"),
        "Plan should include intelligent selection metadata"
    );
...
async fn test_fallback_to_basic_planning() {
    ...
    assert!(
        !planner.has_intelligent_tool_selection(),
        "Basic planner should not have intelligent tool selection"
    );
...
async fn test_plan_validation_with_intelligent_selection() {
    ...
    assert!(
        validation.is_ok(),
        "Plan validation should work with intelligent selection"
    );
    ...
    assert!(
        ...
        "Validation should detect missing tools"
    );
    assert!(
        !validation_result.errors.is_empty(),
        "Validation should report errors for missing tools"
    );
}
