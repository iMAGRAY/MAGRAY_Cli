...
impl SmartRouter {
    ...
    pub async fn process_single_tool_request(&self, user_query: &str) -> Result<String> {
        ...
        let tool_selection = self
            .tool_selector
            .select_tool(user_query, &available_tools)
            .await?;

        println!(
            "[AI] Выбран инструмент: {} (уверенность: {:.1}%)",
            tool_selection.tool_name,
            tool_selection.confidence * 100.0
        );
    ...
    pub async fn process_smart_request(&self, user_query: &str) -> Result<String> {
        ...
        if plan.confidence < 0.7 {
            println!(
                "[⚠️] Низкая уверенность в плане ({:.1}%), продолжить? [y/N]",
