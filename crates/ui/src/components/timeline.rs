...
impl Timeline {
    ...
    pub fn render(&mut self, f: &mut Frame, area: Rect) {
        match self.timeline_data.clone() {
            Some(timeline) => {
                ...
                if self.show_details && self.selected_task_index < timeline.tasks.len() {
                    self.render_task_details(
                        f,
                        chunks[2],
                        &timeline.tasks[self.selected_task_index],
                    );
                } else {
    ...
    fn render_task_list(&mut self, f: &mut Frame, area: Rect, timeline: &TimelineData) {
        let filtered_tasks: Vec<(usize, &TaskEvent)> = timeline
            .tasks
            .iter()
            .enumerate()
            .filter(|(_, task)| match task.status {
        ...
        if self.auto_scroll {
            if let Some((idx, _)) = filtered_tasks
                .iter()
    ...
    fn render_task_details(&self, f: &mut Frame, area: Rect, task: &TaskEvent) {
        let mut details = vec![
            ...
            Line::from(vec![
                ...
                Span::styled(
                    format!("{:?}", task.status),
                    self.get_status_style(&task.status),
                ),
            ]),
            Line::from(vec![
        ...
        if let Some(end_time) = &task.end_time {
            details.push(Line::from(vec![
                Span::styled("Ended: ", Style::default().fg(Color::Cyan)),
                Span::raw(end_time),
            ]));
        }
    ...
    fn render_session_stats(&self, f: &mut Frame, area: Rect, timeline: &TimelineData) {
        ...
        let avg_duration = if completed > 0 {
            let total_duration: u64 = timeline.tasks.iter().filter_map(|t| t.duration).sum();
        ...
        if let Some(avg) = avg_duration {
            ...
            stats.push(Line::from(vec![
                ...
                Span::raw(format!("{avg} seconds")),
            ]));
        }

        if let Some(est_total) = timeline.estimated_total_duration {
            stats.push(Line::from(vec![
                Span::styled("Est. Total: ", Style::default().fg(Color::Cyan)),
                Span::raw(format!("{est_total} seconds")),
            ]));
        }
    ...
    fn render_controls(&self, f: &mut Frame, area: Rect) {
        let controls =
            "↑↓: Navigate, Enter: Toggle details, A: Auto-scroll, C/F: Toggle completed/failed";
