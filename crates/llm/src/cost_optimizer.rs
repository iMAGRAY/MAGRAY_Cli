...
impl Default for CostTable {
    fn default() -> Self {
        ...
        costs.insert(
            (ProviderType::OpenAI, "gpt-4-turbo".to_string()),
            (0.01, 0.03),
        );
        costs.insert(
        ...
        costs.insert(
            (ProviderType::Groq, "llama-3.1-8b-instant".to_string()),
            (0.00005, 0.00008),
        );
        costs.insert(
        ...
        costs.insert(
            (ProviderType::Groq, "mixtral-8x7b-32768".to_string()),
            (0.00024, 0.00024),
        );
...
impl CostTable {
    pub fn get_cost(&self, provider_type: &ProviderType, model: &str) -> (f32, f32) {
        self.costs
            .get(&(provider_type.clone(), model.to_string()))
...
impl CostOptimizer {
    ...
    pub fn select_optimal_provider(
        ...
        debug!(
            "🧮 Selecting optimal provider for task: complexity={:?}, priority={:?}, tokens={}",
        ...
        if suitable_providers.is_empty() {
            info!(
                "⚠️ No suitable providers for complexity level {:?}, using any available",
                task.complexity
            );
        ...
        if task.priority == Priority::Critical {
            let premium = suitable_providers
                .iter()
        ...
        for provider in &suitable_providers {
            let estimated_cost =
                self.estimate_provider_cost(provider, task.tokens, task.tokens / 4);
        ...
        info!(
            "💰 Selected cost-optimal provider: estimated cost ${:.4}",
            best_cost
        );
    ...
    fn estimate_provider_cost(
        ...
        self.cost_table
            .estimate_cost(&provider_type, model, input_tokens, output_tokens)
    }
    ...
    pub fn record_cost(&mut self, cost: f32) {
        ...
        debug!(
            "💸 Recorded cost: ${:.4} (total: ${:.2})",
            cost, self.current_spend
        );

        if let Some(limit) = self.budget_limit {
            if self.current_spend > limit * 0.9 {
                info!(
                    "⚠️ Budget warning: {:.1}% of daily limit used",
    ...
    pub fn reset_daily_spending(&mut self) {
        info!(
            "📊 Daily spending reset. Previous: ${:.2}",
            self.current_spend
        );
...
mod tests {
    ...
    fn test_cost_estimation() {
        ...
        let free_cost =
            optimizer
                .cost_table
                .estimate_cost(&ProviderType::Local, "llama", 1000, 500);
