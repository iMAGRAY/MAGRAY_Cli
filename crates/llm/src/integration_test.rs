...
mod integration_tests {
    use crate::{LlmClient, LlmProvider, CompletionRequest, MultiProviderLlmOrchestrator};
    ...
    fn test_task_complexity_analysis() {
        ...
        // Complex request  
        let complex_request = CompletionRequest::new(&"x".repeat(8000)) // Very long request
        ...
        assert!(complexity.tokens > 1500, "Expected >1500 tokens, got {}", complexity.tokens);
    ...
    fn test_cost_optimization() {
        use crate::{CostOptimizer, TaskComplexity, ComplexityLevel, Priority, LlmProvider};
