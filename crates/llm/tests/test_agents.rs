...
async fn test_tool_selector_simple_commands() {
    ...
    let selection = agent
        .select_tool("read file config.json", &available_tools)
        .await;
...
async fn test_parameter_extractor_file_operations() {
    ...
    let extraction = agent
        .extract_parameters("read file config.json", "file_read", &required_params)
        .await;
...
async fn test_parameter_extractor_missing_parameters() {
    ...
    let extraction = agent
        .extract_parameters("create a directory", "shell_exec", &required_params)
        .await;
    assert!(extraction.is_ok());
...
async fn test_action_planner_simple_task() {
    ...
    let plan = agent
        .create_plan("read the config file", &available_tools)
        .await;
    assert!(plan.is_ok());
...
async fn test_agents_error_handling() {
    ...
    let param_result = param_extractor
        .extract_parameters("test", "file_read", &required_params)
        .await;
