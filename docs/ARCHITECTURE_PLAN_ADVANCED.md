...
crates/
  ...
  tools/         # runtime плагинов (WASI/subprocess), registry, встроенные инструменты
...
— Эта архитектура ориентирована на долгую жизнь: платформа (а не монолитный «скрипт»), сильная память (Qwen3 emb + rerank 0.6B), безопасность и прозрачность, сценарная мощь и выдающийся UX (план‑дифф‑выполнение, таймлайн/панели, flows). Без переобучения — достигаем качества инженерными средствами.